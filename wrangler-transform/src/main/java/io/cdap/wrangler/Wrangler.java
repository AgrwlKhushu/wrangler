 ...
 * <p>
 * This plugin is an implementation of the transformation that are performed
 * in the
...
@Name("Wrangler")
@Description("Wrangler - A interactive tool for data cleansing and " +
        "transformation.")
public class Wrangler extends Transform<StructuredRecord, StructuredRecord> implements LinearRelationalTransform {
    ...
    public static final String DIRECTIVE_METRIC_NAME = "wrangler.directive" +
            ".count";
    ...
    private static final String ERROR_STRATEGY_DEFAULT = "wrangler.error" +
            ".strategy.default";
    ...
    /**
     * Configures the plugin during deployment of the pipeline that uses the
     * plugin.
     *
     * <p>
     *   <ul>
     *     <li>Parses the directives configured. If there are any issues they
     *     will highlighted during deployment</li>
    ...
    public void configurePipeline(PipelineConfigurer configurer) {
        ...
        FailureCollector collector =
                configurer.getStageConfigurer().getFailureCollector();
        ...
        try {
            ...
            if (config.getUDDs() != null && !config.getUDDs().trim().isEmpty()) {
                if (config.containsMacro("directives")) {
                    directives = String.format("#pragma load-directives %s;",
                            config.getUDDs());
                } else {
                    directives = String.format("#pragma load-directives %s;" +
                            "%s", config.getUDDs(), config.getDirectives());
                }
            }
            ...
            if (!config.containsMacro(Config.NAME_PRECONDITION_LANGUAGE)) {
                if (PRECONDITION_LANGUAGE_SQL.equalsIgnoreCase(config.getPreconditionLanguage())) {
                    if (!config.containsMacro(Config.NAME_PRECONDITION_SQL)) {
                        validatePrecondition(config.getPreconditionSQL(),
                                true, collector);
                    }
                    validateSQLModeDirectives(collector);
                } else {
                    if (!config.containsMacro(Config.NAME_PRECONDITION)) {
                        validatePrecondition(config.getPreconditionJEXL(),
                                false, collector);
                    }
                }
            }
            ...
            try {
                ...
                CompileStatus status =
                        compiler.compile(new MigrateToV2(directives).migrate());
                RecipeSymbol symbols = status.getSymbols();
                if (symbols != null) {
                    Set<String> dynamicDirectives =
                            symbols.getLoadableDirectives();
                    for (String directive : dynamicDirectives) {
                        Object directivePlugin =
                                configurer.usePlugin(Directive.TYPE,
                                        directive, directive,
                                        PluginProperties.builder().build());
                        if (directivePlugin == null) {
                            collector.addFailure(
                                            String.format("User Defined " +
                                                    "Directive '%s' is not " +
                                                    "deployed or is not " +
                                                    "available.", directive),
                                            "Ensure the directive is deployed.")
                                    .withPluginNotFound(directive, directive,
                                            Directive.TYPE)
                                    .withConfigElement(Config.NAME_UDD,
                                            directive);
                        }
                    }
                    // If the 'directives' contains macro, then we would not
                    // attempt to compile
                    // it.
                    if (!config.containsMacro(Config.NAME_DIRECTIVES)) {
                        // Create the registry that only interacts with
                        // system directives.
                        ...
                        while (iterator.hasNext()) {
                            ...
                            if (group != null) {
                                String directive =
                                        (String) group.get(0).value();
                                DirectiveInfo directiveInfo = registry.get(""
                                        , directive);
                                if (directiveInfo == null && !dynamicDirectives.contains(directive)) {
                                    collector.addFailure(
                                                    String.format("Wrangler " +
                                                                    "plugin " +
                                                                    "has a " +
                                                                    "directive '%s' " +
                                                                    "that " +
                ...
                collector.addFailure(
                        String.format("Compilation error occurred, %s: %s ",
                                e.getClass().getName(),
                                e.getMessage()), null);
            } catch (DirectiveParseException e) {
                collector.addFailure(
                        String.format("Error parsing directive, %s: %s",
                                e.getClass().getName(),
                                e.getMessage()), null);
            } catch (DirectiveLoadException e) {
                collector.addFailure(
                        String.format("Error loading directive, %s: %s",
                                e.getClass().getName(),
                                e.getMessage()), null);
            }
            ...
            try {
                ...
                collector.addFailure(
                        String.format("Invalid output schema %s: %s",
                                e.getClass().getName(), e.getMessage()),
                        null).withConfigProperty(Config.NAME_SCHEMA).withStacktrace(e.getStackTrace());
            }

            // Check if jexl pre-condition is not null or empty and if so
            // compile expression.
            if (!config.containsMacro(Config.NAME_PRECONDITION) && !config.containsMacro(
                ...
                if (PRECONDITION_LANGUAGE_JEXL.equalsIgnoreCase(config.getPreconditionLanguage())
                    ...
                    try {
                        ...
                        collector.addFailure(String.format("Error compiling " +
                                                "precondition expression, %s:" +
                                                " %s",
                                        e.getClass().getName(),
                                        e.getMessage()), null)
    ...
    /**
     * {@code prepareRun} is invoked by the client once before the job is
     * submitted, but after the resolution
    ...
    public void prepareRun(StageSubmitterContext context) throws Exception {
        ...
        // Validate input schema. If there is no input schema available then
        // there
        ...
        // If there is input schema, but if there is no output schema, there
        // is nothing to apply
        // transformations on. So, there is no point in generating field
        // level lineage.
        ...
        try {
            ...
            String errorReason = "Unable to parse recipe and extract all " +
        ...
        LineageOperations lineageOperations = new LineageOperations(input,
                output, directives);
        context.record(lineageOperations.generate());
    }

    /**
     * Initialize the wrangler by parsing the directives and creating the
     * runtime context.
     *
    ...
    public void initialize(TransformContext context) throws Exception {
        ...
        try {
            ...
            String errorMessage = String.format(
                    "Format of output schema specified is invalid. Please " +
                            "check the format. %s: %s",
                    e.getClass().getName(), e.getMessage());
        ...
        if (!config.containsMacro(Config.NAME_PRECONDITION_LANGUAGE)) {
            if (PRECONDITION_LANGUAGE_JEXL.equalsIgnoreCase(config.getPreconditionLanguage())
                ...
                try {
                    ...
                    String errorReason = "Failed to evaluate precondition due" +
                            " to an invalid JEXL expression.";
                    throw WranglerErrorUtil.getProgramFailureExceptionDetailsFromChain(e, errorReason, null,
                            ErrorType.USER);
                }
            }
        ...
        try {
            ...
            String errorReason = "Unable to compile the recipe and execute " +
                    "directives.";
            String errorMessage = String.format(
        ...
        String defaultStrategy =
                context.getArguments().get(ERROR_STRATEGY_DEFAULT);
        onErrorStrategy =
                (defaultStrategy != null && config.onError == null) ?
                        defaultStrategy : config.getOnError();
    ...
    public void transform(StructuredRecord input,
        ...
        try {
            ...
            if ("*".equalsIgnoreCase(config.getField())) {
                ...
                row.add(config.getField(),
                        StructuredToRowTransformer.getValue(input,
                                config.getField()));
            }
            ...
            if (PRECONDITION_LANGUAGE_JEXL.equalsIgnoreCase(config.getPreconditionLanguage())
                ...
                if (skip) {
                    ...
                    return; // Expression evaluated to true, so we skip the
                    // record.
                }
            }
            ...
            // We now extract errors from the execution and pass it on to the
            // error emitter.
            ...
            if (errors.size() > 0) {
                ...
                for (ErrorRecord error : errors) {
                    emitter.emitError(new InvalidEntry<>(error.getCode(),
                ...
                if (WRANGLER_FAIL_PIPELINE_FOR_ERROR.isEnabled(getContext())
                    ...
                    String errorReason = String.format("Errors in Wrangler " +
                                    "Transformation - %s",
                            errorMessages);
            ...
            if (onErrorStrategy.equalsIgnoreCase(ON_ERROR_PROCEED)) {
                // Emit error record, if the Error flattener or error
                // handlers are not connected, then
            ...
            if (onErrorStrategy.equalsIgnoreCase(ON_ERROR_FAIL_PIPELINE)) {
                emitter.emitAlert(ImmutableMap.of(
                        ...
                        "message", String.format("Stopping pipeline stage %s " +
                                        "on error %s",
                                getContext().getStageName(), e.getMessage()),
                        "value", String.valueOf(errorCounter)
                ));
                String errorReason = "Error occurred while processing input " +
                        "data, possibly due to invalid "
                        + "transformation or schema mismatch.";
                String errorMessage = String.format("Pipeline failed at " +
                                "stage:%s, %s: %s",
                        getContext().getStageName(), e.getClass().getName(),
                        e.getMessage());
                throw WranglerErrorUtil.getProgramFailureExceptionDetailsFromChain(e, errorReason,
                        errorMessage, ErrorType.UNKNOWN);
            }
            // If it's 'skip-on-error' we continue processing and don't emit
            // any error records.
            return;
        } finally {
            getContext().getMetrics().gauge("process.time",
                    System.nanoTime() - start);
        }

        for (StructuredRecord record : records) {
            StructuredRecord.Builder builder =
                    StructuredRecord.builder(oSchema);
            // Iterate through output schema, if the 'record' doesn't have
            // it, then
            // attempt to take if from 'input'.
            for (Schema.Field field : oSchema.getFields()) {
                Object wObject = record.get(field.getName()); // wrangled
                // records
                if (wObject == null) {
                    builder.set(field.getName(), null);
                } else {
                    if (wObject instanceof String) {
                        builder.convertAndSet(field.getName(),
                                (String) wObject);
                    } else {
                        // No need to use specific methods for fields of
                        // logical type - timestamp, date and time. This is
                        // because
                        // the wObject should already have correct values for
    ...
    private void validateInputSchema(@Nullable Schema inputSchema,
        ...
        if (inputSchema != null) {
            ...
            Schema.Field inputSchemaField =
                    inputSchema.getField(config.getField());
            if (inputSchemaField == null) {
                collector.addFailure(String.format("Field '%s' must be " +
    ...
    private void validatePrecondition(String precondition,
    ...
    private void validateSQLModeDirectives(FailureCollector collector) {
        if (!Strings.isNullOrEmpty(config.getDirectives())) {
            collector.addFailure("Directives are not supported for " +
                            "precondition of type SQL", null)
                    .withConfigProperty(Config.NAME_DIRECTIVES);
        }

        if (!Strings.isNullOrEmpty(config.getUDDs())) {
            collector.addFailure("UDDs are not supported for precondition of " +
                            "type SQL", null)
    ...
    /**
     * This method creates a {@link CompositeDirectiveRegistry} and
     * initializes the {@link RecipeParser}
    ...
    private RecipeParser getRecipeParser(StageContext context) {
        ...
        registry =
                new CompositeDirectiveRegistry(SystemDirectiveRegistry.INSTANCE, new UserDirectiveRegistry(context));
        try {
        ...
        String directives = config.getDirectives();
        if (config.getUDDs() != null && !config.getUDDs().trim().isEmpty()) {
            directives = String.format("#pragma load-directives %s;%s",
                    config.getUDDs(), config.getDirectives());
        }

        try {
            return new GrammarBasedParser(context.getNamespace(),
                    new MigrateToV2(directives).migrate(), registry);
        } catch (Exception e) {
    ...
    public Relation transform(RelationalTranformContext relationalTranformContext, Relation relation) {
        if (PRECONDITION_LANGUAGE_SQL.equalsIgnoreCase(config.getPreconditionLanguage())
            ...
            if (!Feature.WRANGLER_PRECONDITION_SQL.isEnabled(relationalTranformContext)) {
                String errorReason = "SQL Precondition feature is not " +
                        "available";
                throw ErrorUtils.getProgramFailureException(
                        new ErrorCategory(ErrorCategory.ErrorCategoryEnum.PLUGIN), errorReason, errorReason,
            ...
            Expression filterExpression =
                    expressionFactory.get().compile(config.getPreconditionSQL());
            return relation.filter(filterExpression);
        }

        return new InvalidRelation("Plugin is not configured for relational " +
                "transformation");
    }
     ...
     * @param metrics    CDAP {@link Metrics} object using which metrics can
     *                   be emitted
     */
    private void emitDirectiveMetrics(List<Directive> directives,
                                      Metrics metrics) {
        for (Directive directive : directives) {
            // skip emitting metrics if the directive is not system directive
            try {
                if (registry.get(Contexts.SYSTEM,
                        directive.define().getDirectiveName()) == null) {
                    continue;
                }
            } catch (Exception e) {
                String errorReason = String.format("Unable to load directive " +
                                "%s",
    ...
    private EntityCountMetric getDirectiveUsageMetric(String directiveName) {
        return new EntityCountMetric(
                DIRECTIVE_METRIC_NAME, DIRECTIVE_ENTITY_TYPE, directiveName,
    ...
    public static class Config extends PluginConfig {
        ...
        @Name(NAME_FIELD)
        @Description("Name of the input field to be wrangled or '*' to " +
                "wrangle all the fields.")
        @Macro
        ...
        @Description("Toggle to configure precondition language between JEXL " +
                "and SQL")
        @Macro
        @Nullable
        private String preconditionLanguage;
        @Name(NAME_PRECONDITION)
        @Description("JEXL Precondition expression specifying filtering " +
                "before applying directives (true to filter)")
        @Macro
        @Nullable
        private String precondition;
        @Name(NAME_PRECONDITION_SQL)
        @Description("SQL Precondition expression specifying filtering before" +
                " applying directives (false to filter)")
        @Macro
        @Nullable
        ...
        @Name(NAME_UDD)
        @Description("List of User Defined Directives (UDD) that have to be " +
                "loaded.")
        @Nullable
        private String udds;

        public Config(String preconditionLanguage, String precondition,
                      String directives, String udds,
