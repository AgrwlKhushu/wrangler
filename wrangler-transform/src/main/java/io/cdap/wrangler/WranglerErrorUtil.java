...
public final class WranglerErrorUtil {
    ...
    private static final Map<String, String> TERMINAL_EXCEPTIONS =
            ImmutableMap.<String, String>builder()
                    .put(DirectiveParseException.class.getName(), "Parsing" +
                            "-Directive")
                    .put(PreconditionException.class.getName(), "Precondition")
                    .put(DirectiveExecutionException.class.getName(),
                            "Executing" +
                                    "-Directive")
                    .put(DirectiveLoadException.class.getName(), "Loading" +
                            "-Directive")
                    .put(DirectiveNotFoundException.class.getName(),
                            "Directive-Not" +
                                    "-Found")
                    .put(RecordConvertorException.class.getName(), "Record" +
                            "-Conversion")
                    .put(ELException.class.getName(), "ExpressionLanguage" +
                            "-Parsing").build();

    private static final Map<String, String> NON_TERMINAL_EXCEPTIONS =
            ImmutableMap.<String, String>builder()
     ...
     * <p>
     * This class is designed to contain only static utility methods for
     * handling exceptions and
     * should not be instantiated. Any attempt to create an instance of this
    ...
    /**
     * Traverses the causal chain of the given Throwable to find specific
     * exceptions. If a terminal
     * exception is found, it returns a corresponding ProgramFailureException
     * . If a non-terminal
     * exception is found, it is stored as a fallback. Otherwise, a generic
     * ProgramFailureException is
     * returned.
     *
     * @param e            the Throwable to analyze
     * @param errorReason  the error reason to tell the cause of error
     * @param errorMessage default error message if no terminal exception is
     *                     found
    ...
    public static ProgramFailureException getProgramFailureExceptionDetailsFromChain(Throwable e,
        ...
        for (Throwable t : causalChain) {
            ...
            if (NON_TERMINAL_EXCEPTIONS.containsKey(t.getClass().getName())) {
                nonTerminalException = t; // Store non-terminal exception as
                // fallback
                continue;
            }
            String errorSubCategory =
                    TERMINAL_EXCEPTIONS.get(t.getClass().getName());
            if (errorSubCategory != null) {
                return getProgramFailureException(t, errorReason,
                        errorSubCategory);
            }
        }
    ...
    /**
     * Constructs a ProgramFailureException using the provided exception
     * details.
     *
