...
public class Precondition {
    ...
    // SimpleScriptContext is pretty expensive to construct due to all
    // PrintWriter creation, so let's cache it
        ...
        try {
            ...
            if (!(result instanceof Boolean)) {
                throw new PreconditionException(
                        String.format("Precondition '%s' does not result in " +
                                "true or false.", condition)
                );
            }
            return (Boolean) result;
        } catch (ScriptException e) {
            // Generally JexlException wraps the original exception, so it's
            // good idea
