...
public class TestSetupHooks {
    ...
    public static void createTempSourceBQTable() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQTableQueryFileCsv"),
                PluginPropertyUtils.pluginProp("InsertBQDataQueryFileCsv"));
    }

    @Before(order = 2, value = "@BQ_SINK_TEST")
    public static void setTempTargetBQTableName() {
        String bqTargetTableName =
                "E2E_TARGET_" + UUID.randomUUID().toString().replaceAll("-",
                        "_");
    ...
    public static void deleteTempTargetBQTable() throws IOException,
            InterruptedException {
        String bqTargetTableName = PluginPropertyUtils.pluginProp(
                "bqTargetTable");
        try {
            BigQueryClient.dropBqQuery(bqTargetTableName);
            BeforeActions.scenario.write(
                    "BQ Target table - " + bqTargetTableName + " deleted " +
                            "successfully");
    ...
    public static void createTempSourceBQTableFxdLen() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQDataQueryFileFxdLen"),
                PluginPropertyUtils.pluginProp("InsertBQDataQueryFileFxdLen"));
    }

    @Before(order = 1, value = "@BQ_SOURCE_HL7_TEST")
    public static void createTempSourceBQTableHl7() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQDataQueryFileHl7"),
                PluginPropertyUtils.pluginProp("InsertBQDataQueryFileHl7"));
    }

    @Before(order = 1, value = "@BQ_SOURCE_TS_TEST")
    public static void createTempSourceBQTableTimestamp() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQDataQueryFileTimestamp"),
                PluginPropertyUtils.pluginProp(
                        "InsertBQDataQueryFileTimestamp"));
    }

    @Before(order = 1, value = "@BQ_SOURCE_DATETIME_TEST")
    public static void createTempSourceBQTableDateTime() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQDataQueryFileDatetime"),
                PluginPropertyUtils.pluginProp("InsertBQDataQueryFileDatetime"
                ));
    }

    @After(order = 2, value = "@BQ_SOURCE_TEST")
    public static void deleteTempSourceBQTable() throws IOException,
            InterruptedException {
        String bqSourceTable = PluginPropertyUtils.pluginProp("bqSourceTable");
        BigQueryClient.dropBqQuery(bqSourceTable);
        BeforeActions.scenario.write("BQ source Table " + bqSourceTable + " " +
                "deleted successfully");
        PluginPropertyUtils.removePluginProp("bqSourceTable");
    }

    @Before(order = 1, value = "@BQ_SOURCE_AVRO_TEST")
    public static void createTempSourceBQTableAvro() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQTableQueryFileAvro"),
                PluginPropertyUtils.pluginProp("InsertBQDataQueryFileAvro"));
    }

    @Before(order = 1, value = "@BQ_SOURCE_LOG_TEST")
    public static void createTempSourceBQTableLog() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQTableQueryFileLog"),
                PluginPropertyUtils.pluginProp("InsertBQDataQueryFileLog"));
    }


    @Before(order = 1, value = "@BQ_SOURCE_JSON_TEST")
    public static void createTempSourceBQTableJson() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQTableQueryFileJson"),
                PluginPropertyUtils.pluginProp("InsertBQDataQueryFileJson"));
    }

    @Before(order = 1, value = "@BQ_SOURCE_XML_TEST")
    public static void createTempSourceBQTableXml() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQDataQueryFileXml"),
                PluginPropertyUtils.pluginProp("InsertBQDataQueryFileXml"));
    }

    @Before(order = 1, value = "@BQ_SOURCE_GRPBY_TEST")
    public static void createTempSourceBQTableGroupBy() throws IOException,
            InterruptedException {
        createSourceBQTableWithQueries(PluginPropertyUtils.pluginProp(
                        "CreateBQTableQueryFile"),
                PluginPropertyUtils.pluginProp("InsertBQDataQueryFile"));
    }

    @Before(order = 1, value = "@GCS_SOURCE_TEST")
    public static void createBucketWithEXCELFile() throws IOException,
            URISyntaxException {
        gcsSourceBucketName =
                createGCSBucketWithFile(PluginPropertyUtils.pluginProp(
                        "testFile"));
        PluginPropertyUtils.addPluginProp("gcsSourceBucket",
                "gs://" + gcsSourceBucketName + "/" +
    ...
    private static String createGCSBucketWithFile(String filePath)
        ...
        String bucketName =
                StorageClient.createBucket("e2e-test-" + UUID.randomUUID()).getName();
    ...
    private static void deleteGCSBucket(String bucketName) {
        try {
            for (Blob blob :
                    StorageClient.listObjects(bucketName).iterateAll()) {
            ...
            if (e.getMessage().contains("The specified bucket does not exist")) {
                BeforeActions.scenario.write("GCS Bucket " + bucketName + " " +
                        "does not exist.");
            } else {
                Assert.fail(e.getMessage());
            }
    ...
    private static void createSourceBQTableWithQueries(String bqCreateTableQueryFile,
        ...
        try {
            createTableQuery =
                    new String(Files.readAllBytes(Paths.get(TestSetupHooks.class.getResource
        ...
        try {
            ...
            // Iterator on TableResult values in getSoleQueryResult method
    ...
    public static void setBQConnectionName() {
        PluginPropertyUtils.addPluginProp("bqConnectionName",
                "BQ-" + UUID.randomUUID());
    }
}
