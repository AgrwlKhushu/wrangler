 ...
 * }
 * @Override public void initialize(Argument args) throws
...
public interface Directive extends Executor<List<Row>, List<Row>>,
     ...
     * definitions. Each token definition consists of {@code name}, {@code
     * TokenType}
     * and {@code optional} field that specifies whether the token specified is
     * optionally present.</p>
     *
     * <p>The {@code UsageDefinition} provides different methods to {@code
     * define},
     * and as well generate {@code usage} based on the definition.</p>
     *
     * <p>This method is invoked by the framework at the time of creating an
     * executable
     * directive that will be added to the {@link RecipePipeline}. It's
     * generally during
     * the configuration phase.</p>.
     *
     * <p>NOTE: As best practice, developer needs to make sure that this
     * class doesn't
     * throw an exception. Also, it should not include external libraries
     * that can
     ...
     * builder.define("expression", TokenType.EXPRESSOION); // exp: { age <
     * 10.0 }
     * }
     * </code>
     * </p>
     * <p>
     * {@code TokenType} supports many different token types that can be used
     * within the
     * usage definition.
     *
     * @return A object of {@code UsageDefinition} containing definitions of
     * each argument
    ...
    /**
     * This method provides a way to emit metrics from the Directive.
     * Metadata about each metric to be emitted can be
