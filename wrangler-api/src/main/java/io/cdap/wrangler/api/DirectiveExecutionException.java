...
/**
 * A Executor specific exception used for communicating issues with execution
 * of pipeline in that step.
 */
public class DirectiveExecutionException extends Exception {
    ...
    public DirectiveExecutionException(String directiveName,
                                       String errorMessage) {
        this(String.format("Error encountered while executing '%s' : %s",
                directiveName, errorMessage));
    }

    public DirectiveExecutionException(String directiveName,
                                       String errorMessage, Throwable e) {
        this(String.format("Error encountered while executing '%s' : %s",
                directiveName, errorMessage), e);
    }
}
