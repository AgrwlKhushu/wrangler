...
public final class TestingRig {
    ...
    public static RecipePipeline pipeline(Class<? extends Directive> directive, TestRecipe recipe)
        ...
        RecipeParser parser = new GrammarBasedParser(Contexts.SYSTEM, migrate
                , registry);
        return new RecipePipelineExecutor(parser, null);
    }

    public static RecipeParser parser(Class<? extends Directive> directive,
                                      String[] recipe)
    ...
    private static void verify(Class<? extends Directive> directive) {
        ...
        if (plugin == null || !plugin.type().equalsIgnoreCase(Directive.TYPE)) {
            throw new IllegalArgumentException(
                    String.format("Class '%s' @Plugin annotation is not of " +
                                    "type '%s', Set it as @Plugin(type=UDD" +
                                    ".Type)",
        ...
        if (name == null) {
            throw new IllegalArgumentException(
                    String.format("Class '%s' is missing @Name annotation. E" +
                            ".g. @Name(\"directive-name\")", classz)
            );
        }
        ...
        if (description == null) {
            throw new IllegalArgumentException(
                    String.format("Class '%s' is missing @Description " +
                            "annotation. " +
                            "E.g. @Description(\"this is what my directive " +
                            "does\")", classz)
            );
        }
    }
