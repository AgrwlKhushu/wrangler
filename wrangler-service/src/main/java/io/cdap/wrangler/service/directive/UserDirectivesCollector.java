...
/**
 * This Visitor collects user directives and helps to add #pragma directive
 * to load them to the
 * script
 */
public class UserDirectivesCollector implements GrammarWalker.Visitor<RuntimeException> {
    ...
    /**
     * If any user directives was found, adds load-directives pragma as the
     * first command.
     */
    public void addLoadDirectivesPragma(List<String> directives) {
        if (!userDirectives.isEmpty()) {
            directives.add(0, "#pragma load-directives " + String.join(",",
                    userDirectives) + ";");
        }
    }
}
