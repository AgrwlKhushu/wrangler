...
public class ConnectionUpgrader {
    private static final Logger LOG =
            LoggerFactory.getLogger(ConnectionUpgrader.class);
    private static final UpgradeState CONN_COMPLETE_STATE =
            new UpgradeState(1L);
    ...
    public ConnectionUpgrader(UpgradeStore upgradeStore,
    ...
    public void upgradeConnections() throws Exception {
        ...
        for (NamespaceSummary ns : namespaces) {
            UpgradeState state = upgradeStore.getEntityUpgradeState(ns,
                    UpgradeEntityType.CONNECTION);
            if (state == null || state.getVersion() == 0L) {
                upgradeConnectionsInNamespace(ns);
            }
        }
        upgradeStore.setEntityUpgradeState(UpgradeEntityType.CONNECTION,
                CONN_COMPLETE_STATE);
    }
    ...
    private void upgradeConnectionsInNamespace(NamespaceSummary namespace) {
        List<Connection> connections = TransactionRunners.run(context, ctx -> {
            ...
            return connStore.list(new Namespace(namespace.getName(),
        ...
        for (Connection connection : connections) {
            ...
            PluginInfo pluginInfo =
                    artifactLoader.getPluginInfo(type.getConnectorName());
            if (pluginInfo == null) {
                LOG.warn("Unable to find the connector for connection type {}" +
                                " with connection name {}, " +
                                "upgrade will not be done for it",
                        type.name().toLowerCase(), connection.getName());
                continue;
            }

            PluginInfo info = new PluginInfo(
                    pluginInfo.getName(), pluginInfo.getType(),
                    pluginInfo.getCategory(),
                    SpecificationUpgradeUtils.getConnectorProperties(type,
                            connection.getProperties()),
                    pluginInfo.getArtifact());
            ConnectionCreationRequest request =
                    new ConnectionCreationRequest(connection.getDescription()
                            , info);
            try {
                discoverer.addConnection(namespace.getName(),
                        connection.getName(), request);
            } catch (ConflictException e) {
                // if there is a conflict exception, it is quite possible
                // this connection is already upgraded before
                LOG.debug("A connection {} already exists, ignoring the " +
                        "upgrade", connection.getName());
            } catch (Exception e) {
                LOG.warn("Failed to upgrade connection {}",
                        connection.getName(), e);
            }
        }
        upgradeStore.setEntityUpgradeState(namespace,
                UpgradeEntityType.CONNECTION, CONN_COMPLETE_STATE);
    }
}
