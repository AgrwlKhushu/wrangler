...
public class ADLSHandler extends AbstractWranglerHandler {
    ...
    private ADLStoreClient initializeAndGetADLSClient(ConnectionMeta connection) {
        ADLSConfiguration configuration =
                new ADLSConfiguration(connection.getProperties());
        ...
        AccessTokenProvider provider =
                new ClientCredsTokenProvider(authTokenEndpoint, clientId,
                        clientKey);
    ...
    public void testADLSConnection(HttpServiceRequest request,
                                   HttpServiceResponder responder,
                                   @PathParam("context") String namespace) {
        // Extract the body of the request and transform it to the Connection
        // object
        respond(request, responder, () -> {
            RequestExtractor extractor = new RequestExtractor(request);
            ConnectionMeta connection =
                    extractor.getConnectionMeta(ConnectionType.ADLS);
            ConnectionType connectionType = connection.getType();
            if (connectionType != ConnectionType.ADLS) {
                return new ServiceResponse<Void>(String.format("Invalid " +
                        "connection type %s set, expected " +
    ...
    private String testConnection(ADLStoreClient client) throws IOException {
        try {
            ...
            throw new IOException("Connection Failed, please check given " +
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void listADLSDirectory(HttpServiceRequest request,
                                  HttpServiceResponder responder,
                                  @PathParam("context") String namespace,
        ...
        respond(request, responder, namespace, ns -> {
            try {
                Connection connection =
                        getValidatedConnection(new NamespacedId(ns,
                                connectionId), ConnectionType.ADLS);
                String defaultPath = "/";
                ADLStoreClient adlStoreClient =
                        initializeAndGetADLSClient(connection);
                List<ADLSDirectoryEntryInfo> adlsDirectoryEntryInfos;
                if (path == null || path.equals("")) {
                    adlsDirectoryEntryInfos =
                            initClientReturnResponse(adlStoreClient,
                                    defaultPath);
                } else {
                    adlsDirectoryEntryInfos =
                            initClientReturnResponse(adlStoreClient, path);
                }
                return new ServiceResponse<>(adlsDirectoryEntryInfos);
            } catch (ADLException e) {
                throw new StatusCodeException(e.getMessage(), e,
                        e.httpResponseCode);
            }
        });
    }
    ...
    private List<ADLSDirectoryEntryInfo> initClientReturnResponse(ADLStoreClient client, String path)
        ...
        if (!client.checkExists(path)) {
            throw new NotFoundException(String.format("Given path doesn't " +
                    "exist: %s", path));
        }
        ...
        for (DirectoryEntry entry : list) {
            ...
            if (entry.type.equals(DirectoryEntryType.DIRECTORY)) {
                info = ADLSDirectoryEntryInfo.builder(entry.name,
                                entry.type.toString())
    ...
    public void loadADLSFile(HttpServiceRequest request,
                             HttpServiceResponder responder,
                             @PathParam("context") String namespace,
                             @PathParam("connection-id") String connectionId,
        ...
        respond(request, responder, namespace, ns -> {
            try {
                if (Strings.isNullOrEmpty(connectionId)) {
                    throw new BadRequestException("Required path param " +
                            "'connection-id' is missing in the input");
                }
                if (lines == 0) {
                    throw new BadRequestException("The maximum number of " +
                            "sampled lines cannot be zero");
                }
                String header = request.getHeader(PropertyIds.CONTENT_TYPE);
                NamespacedId namespacedConnId = new NamespacedId(ns,
                        connectionId);
                Connection connection =
                        getValidatedConnection(namespacedConnId,
                                ConnectionType.ADLS);
                FileQueryDetails fileQueryDetails =
                        new FileQueryDetails(header, filePath, lines, sampler,
                                fraction, scope);
                ADLSConnectionSample sample = fetchFileFromClient(connection,
                        fileQueryDetails, namespacedConnId);
     ...
     * @throws IOException
     */
    private ADLSConnectionSample fetchFileFromClient(Connection connection,
                                                     FileQueryDetails fileQueryDetails,
                                                     NamespacedId namespaceID) throws IOException {
        ADLStoreClient client = initializeAndGetADLSClient(connection);
        DirectoryEntry file = getFileFromClient(client,
                fileQueryDetails.getFilePath());
        try (InputStream inputStream = clientInputStream(client,
                fileQueryDetails)) {
            if ("text/plain".equalsIgnoreCase(fileQueryDetails.getHeader())) {
                return loadSamplableFile(namespaceID,
                        fileQueryDetails.getScope(), inputStream, file,
                        fileQueryDetails.getLines(),
                        fileQueryDetails.getFraction(),
                        fileQueryDetails.getSampler());
            } else {
                return loadFile(namespaceID, fileQueryDetails.getScope(),
                        inputStream, file);
            }
        } catch (ADLException e) {
            throw new StatusCodeException(e.getMessage(), e,
    ...
    private InputStream clientInputStream(ADLStoreClient client,
                                          FileQueryDetails fileQueryDetails)
            throws IOException {
        DirectoryEntry file =
                client.getDirectoryEntry(fileQueryDetails.getFilePath());
        return client.getReadStream(file.fullName);
    }

    private DirectoryEntry getFileFromClient(ADLStoreClient client,
    ...
    private ADLSConnectionSample loadSamplableFile(NamespacedId connectionId,
        ...
        try (BoundedLineInputStream blis =
                     BoundedLineInputStream.iterator(inputStream,
            ...
            NamespacedId namespacedWorkspaceId =
                    new NamespacedId(connectionId.getNamespace(), identifier);
            ...
            TransactionRunners.run(getContext(), context -> {
                ...
                // Depending on the type, the sampling of the input is
                // performed.
                ...
                ws.updateWorkspaceData(namespacedWorkspaceId,
                        DataType.RECORDS, data);
            });

            // Preparing return response to include mandatory fields : id and
            // name.
            return new ADLSConnectionSample(namespacedWorkspaceId.getId(),
                    name, ConnectionType.ADLS.getType(),
                    samplingMethod.getMethod(), connectionId.getId());
        }
    }

    private ADLSConnectionSample loadFile(NamespacedId connectionId,
                                          String scope, InputStream inputStream,
                                          DirectoryEntry fileEntry) throws IOException {
        if (fileEntry.length > FILE_SIZE) {
            throw new BadRequestException("Files greater than 10MB are not " +
                    "supported.");
        }
        ...
        TransactionRunners.run(getContext(), context -> {
            WorkspaceDataset ws = WorkspaceDataset.get(context);
            ws.writeWorkspaceMeta(namespacedWorkspaceId, workspaceMeta);
            ws.updateWorkspaceData(namespacedWorkspaceId, getDataType(name),
                    bytes);
        });

        return new ADLSConnectionSample(namespacedWorkspaceId.getId(), name,
                ConnectionType.ADLS.getType(),
    ...
    public void specification(HttpServiceRequest request,
        ...
        respond(request, responder, namespace,
                ns -> TransactionRunners.run(getContext(), context -> {
                    ConnectionStore store = ConnectionStore.get(context);
                    WorkspaceDataset ws = WorkspaceDataset.get(context);
                    Format format = Format.TEXT;
                    NamespacedId namespacedWorkspaceId = new NamespacedId(ns,
                            workspaceId);
                    String refName;
                    if (workspaceId != null) {
                        Map<String, String> config =
                                ws.getWorkspace(namespacedWorkspaceId).getProperties();
                        String formatStr =
                                config.getOrDefault(PropertyIds.FORMAT,
                                        Format.TEXT.name());
                        refName = config.getOrDefault(PropertyIds.NAME,
                                Format.TEXT.name());
                    ...
                    Connection conn = getValidatedConnection(store,
                            new NamespacedId(ns, connectionId),
                            ConnectionType.ADLS);
                    ADLSConfiguration adlsConfiguration =
                            new ADLSConfiguration(conn.getProperties());
                    Map<String, String> properties = new HashMap<>();
                    properties.put("format", format.name().toLowerCase());
                    String pathURI =
                            "adl://" + adlsConfiguration.getAccountFQDN() + path;
                    properties.put("path", pathURI);
                    properties.put("referenceName",
                            ReferenceNames.cleanseReferenceName(refName));
                    properties.put("credentials",
                            adlsConfiguration.getClientKey());
                    properties.put("clientId",
                            adlsConfiguration.getADLSClientID());
                    properties.put("refreshTokenURL",
                            adlsConfiguration.getEndpointURL());
                    properties.put("copyHeader",
                            String.valueOf(shouldCopyHeader(ws,
                                    namespacedWorkspaceId)));
                    properties.put("schema", format.getSchema().toString());

                    PluginSpec pluginSpec = new PluginSpec("AzureDataLakeStore",
                            "source", properties);
