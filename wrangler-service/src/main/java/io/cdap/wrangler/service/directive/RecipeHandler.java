...
public class RecipeHandler extends AbstractWranglerHandler {
    ...
    private static final Pattern RECIPE_NAME_PATTERN = Pattern.compile("[a-zA" +
            "-Z0-9 ]*");
    ...
    public void createRecipe(HttpServiceRequest request,
        ...
        respond(responder, namespace, ns -> {
            ...
            recipeStore.createRecipe(recipeId,
                    RecipeRow.builder(recipe).build());
            metrics.gauge(NUMBER_RECIPE_SAVED_METRIC,
                    recipe.getDirectives().size());
    ...
    public void getRecipeById(HttpServiceRequest request,
                              HttpServiceResponder responder,
                              @PathParam("context") String namespace,
    ...
    public void getRecipeByName(HttpServiceRequest request,
                                HttpServiceResponder responder,
                                @PathParam("context") String namespace,
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    @Path("v2/contexts/{context}/recipes")
    public void listRecipes(HttpServiceRequest request,
                            HttpServiceResponder responder,
                            @PathParam("context") String namespace,
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    @Path("v2/contexts/{context}/recipes/id/{recipe-id}")
    public void deleteRecipe(HttpServiceRequest request,
                             HttpServiceResponder responder,
                             @PathParam("context") String namespace,
                             @PathParam("recipe-id") String recipeId) {
        respond(responder, namespace, ns -> {
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    @Path("v2/contexts/{context}/recipes/id/{recipe-id}")
    public void updateRecipe(HttpServiceRequest request,
                             HttpServiceResponder responder,
                             @PathParam("context") String namespace,
        ...
        respond(responder, namespace, ns -> {
            ...
            Recipe recipe = buildRecipeFromRequest(request, recipeId);
            recipeStore.updateRecipe(recipeId,
                    RecipeRow.builder(recipe).build());
            responder.sendStatus(HttpURLConnection.HTTP_OK);
        });
    }
    ...
    private void validateRecipeRequest(Recipe recipe) {
        ...
        if (recipe.getRecipeName() == null) {
            throw new BadRequestException("No name was specified for the " +
                    "Recipe");
        }
        if (recipe.getDescription() == null) {
            throw new BadRequestException("No description was specified for " +
                    "the Recipe");
        }
        if (recipe.getDirectives() == null || recipe.getDirectives().size() == 0) {
            throw new BadRequestException("No directives were specified for " +
                    "the Recipe");
        }
        if (!RECIPE_NAME_PATTERN.matcher(recipe.getRecipeName()).matches()) {
            throw new BadRequestException("Recipe name should contain only " +
                    "alphanumeric characters or spaces");
        }
    }

    private Recipe buildRecipeFromRequest(HttpServiceRequest request,
                                          RecipeId recipeId) {
