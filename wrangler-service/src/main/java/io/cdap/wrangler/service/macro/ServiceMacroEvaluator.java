...
public class ServiceMacroEvaluator implements MacroEvaluator {
    ...
    public ServiceMacroEvaluator(String namespace,
    ...
    public String lookup(String property) throws InvalidMacroException {
        ...
        if (val == null) {
            throw new InvalidMacroException(String.format("Argument '%s' is " +
    ...
    public String evaluate(String macroFunction, String... arguments) throws InvalidMacroException {
        if (!SECURE_FUNCTION.equals(macroFunction)) {
            throw new InvalidMacroException(String.format("%s is not a " +
                    "supported macro function.", macroFunction));
        }
        if (arguments.length != 1) {
            throw new InvalidMacroException("Secure macro function only " +
                    "supports 1 argument.");
        }
        try {
            SecureStoreData secureStoreData = context.get(namespace,
                    arguments[0]);
