...
public class AbstractWranglerHandler extends AbstractSystemHttpServiceHandler {
    private static final Logger LOG =
            LoggerFactory.getLogger(AbstractWranglerHandler.class);
    ...
    /**
     * Return whether the header needs to be copied when creating the
     * pipeline source for the specified workspace.
     * This just amounts to checking whether parse-as-csv with the first line
     * as a header is used as a directive.
     */
    protected boolean shouldCopyHeader(WorkspaceDataset ws,
                                       @Nullable NamespacedId workspaceId) throws IOException {
        if (workspaceId == null) {
            return false;
        }
        try {
            ...
            // yes this is really hacky, but there doesn't seem to be a good
            // way to get the actual directive classes
            return directives.stream()
                    .map(String::trim)
                    .anyMatch(directive -> directive.startsWith("parse-as-csv"
                    ) && directive.endsWith("true"));
    ...
    protected Connection getValidatedConnection(NamespacedId connectionId,
    ...
    /**
     * Validates that the specified connection exists and is of the expected
     * type.
     * Returns null if the connection does not exist or is invalid. Callers
     * should return immediately if
     * a null is returned, as a response has already been sent. This method
     * should only be called from endpoints
     ...
     */
    @Nullable
    protected Connection getValidatedConnection(ConnectionStore store,
        ...
        if (expectedType != connection.getType()) {
            throw new BadRequestException(String.format("Expected connection " +
                            "type '%s' but found '%s'.",
    ...
    /**
     * Utility method for executing an endpoint with common error handling
     * built in.
     * A response will always be sent after this method is called so the http
     * responder should not be used after this.
     * The endpoint logic should also not use the responder in any way.
     * <p>
     * If the callable throws a {@link StatusCodeException}, the exception's
     * status code and message will be used
     ...
     */
    protected <T> void respond(HttpServiceRequest request,
        ...
        try {
            ...
            responder.sendJson(e.getCode(),
                    new ServiceResponse<>(e.getMessage()));
        } catch (ErrorRecordsException e) {
            responder.sendJson(HttpURLConnection.HTTP_BAD_REQUEST,
                    new ServiceResponse<>(e.getErrorRecords(), false,
                            e.getMessage()));
        } catch (JsonSyntaxException e) {
            responder.sendJson(HttpURLConnection.HTTP_BAD_REQUEST,
                    new ServiceResponse<Void>(e.getMessage()));
        } catch (Throwable t) {
            LOG.warn("Error processing {} {}, resulting in a 500 response.",
                    request.getMethod(), request.getRequestURI(), t);
            responder.sendJson(HttpURLConnection.HTTP_INTERNAL_ERROR,
                    new ServiceResponse<Void>(t.getMessage()));
        }
    }

    /**
     * Utility method for executing an endpoint with common error handling
     * and namespace checks built in.
     * <p>
     * If the callable throws a {@link StatusCodeException}, the exception's
     * status code and message will be used
     * to create the response.
    ...
    protected void respond(HttpServiceResponder responder, String namespace,
        ...
        // system namespace does not officially exist, so don't check
        // existence for system namespace.
        ...
        if (Contexts.SYSTEM.equals(namespace)) {
            ...
            try {
                namespaceSummary =
                        getContext().getAdmin().getNamespaceSummary(namespace);
                if (namespaceSummary == null) {
                    responder.sendJson(HttpURLConnection.HTTP_NOT_FOUND,
                            new io.cdap.wrangler.proto.workspace.v2.ServiceResponse<>(
                                    String.format("Namespace '%s' does not " +
                                            "exist", namespace)));
                    return;
                }
            } catch (IOException e) {
        ...
        try {
            ...
            responder.sendJson(HttpURLConnection.HTTP_BAD_REQUEST,
                    new RecipeExceptionResponse<>(e.getMessage(),
                            e.getRowIndex(), e.getDirectiveIndex()));
        } catch (StatusCodeException e) {
            responder.sendJson(e.getCode(),
                    new io.cdap.wrangler.proto.workspace.v2.ServiceResponse<>(e.getMessage()));
        } catch (ErrorRecordsException e) {
    ...
    private int getErrorCode(String exceptionName) {
        boolean isBadRequest = Stream.of(BadRequestException.class,
                        ErrorRecordsException.class, JsonSyntaxException.class)
                .anyMatch(e -> e.getName().equals(exceptionName));
        return isBadRequest ? HttpURLConnection.HTTP_BAD_REQUEST :
     ...
     */
    protected <T> void respond(HttpServiceRequest request,
                               HttpServiceResponder responder, String namespace,
        ...
        if (Contexts.SYSTEM.equals(namespace)) {
            ...
            try {
                ...
                if (namespaceSummary == null) {
                    responder.sendJson(HttpURLConnection.HTTP_NOT_FOUND,
                            new ServiceResponse<Void>(String.format(
        ...
        try {
            T results =
                    callable.respond(new Namespace(namespaceSummary.getName()
                            , namespaceSummary.getGeneration()));
            responder.sendJson(results);
        } catch (StatusCodeException e) {
            responder.sendJson(e.getCode(),
                    new ServiceResponse<>(e.getMessage()));
