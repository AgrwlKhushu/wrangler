...
public class S3Handler extends AbstractWranglerHandler {
    ...
    public static String getPath(Workspace workspace) {
        return String.format(PATH_FORMAT,
                workspace.getProperties().get(BUCKET_NAME),
                workspace.getProperties().get(KEY));
    }
    ...
    public static S3ObjectInfo fromObject(S3ObjectSummary summary,
    ...
    public void testS3Connection(HttpServiceRequest request,
        ...
        respond(request, responder, () -> {
            ...
            ConnectionMeta connection =
                    extractor.getConnectionMeta(ConnectionType.S3);
            // creating a client doesn't test the connection, we will do list
            // buckets so the connection is tested.
    ...
    private AmazonS3 initializeAndGetS3Client(ConnectionMeta connection,
                                              String namespace,
                                              SystemHttpServiceContext context) {
        Map<String, String> evaluateMacros = evaluateMacros(connection,
                context, namespace);
        ...
        Region region =
                Region.getRegion(Regions.fromName(s3Configuration.getRegion()));
        s3.setRegion(region);
        return s3;
    }
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void getS3BucketInfo(HttpServiceRequest request,
                                HttpServiceResponder responder,
                                @PathParam("context") String namespace,
            ...
            try {
                Connection connection =
                        getValidatedConnection(new NamespacedId(ns,
                                connectionId), ConnectionType.S3);
                String bucketName = "";
                String prefix = null;
                ...
                AmazonS3 s3 = initializeAndGetS3Client(connection, namespace,
                        getContext());
                if (bucketName.isEmpty() && prefix == null) {
                    List<Bucket> buckets = s3.listBuckets();
                    List<S3ObjectInfo> bucketInfo =
     ...
     */
    @POST
    @Path("contexts/{context}/connections/{connection-id}/s3/buckets/{bucket" +
            "-name}/read")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
        ...
        respond(request, responder, namespace, ns -> {
            try {
                if (Strings.isNullOrEmpty(key)) {
                    throw new BadRequestException("Required query param 'key'" +
                ...
                if (object == null) {
                    throw new BadRequestException(
                            String.format("S3 Object with key %s and " +
                                            "bucket-name %s is not found", key,
                                    bucketName));
                }
                ...
                try (InputStream inputStream = object.getObjectContent()) {
                    ...
                    if (header != null && header.equalsIgnoreCase("text/plain"
                        ...
                        sample = loadSamplableFile(namespacedConnId, scope,
                                inputStream, object, lines, fraction, sampler);
                    } else {
                        sample = loadFile(namespacedConnId, scope,
                                inputStream, object);
                    }
        ...
        respond(request, responder, namespace,
                ns -> TransactionRunners.run(getContext(), context -> {
                    ...
                    S3Configuration s3Configuration =
                            new S3Configuration(conn.getProperties());
                    Map<String, String> properties = new HashMap<>();
                    properties.put("format", format.name().toLowerCase());
                    properties.put("accessID",
                            s3Configuration.getAWSAccessKeyId());
                    properties.put("accessKey",
                            s3Configuration.getAWSSecretKey());
                    properties.put("path", String.format("s3n://%s/%s",
                            bucketName,
                            key));
                    ...
                    PluginSpec pluginSpec = new PluginSpec("S3", "source",
                            properties);
    ...
    private S3ConnectionSample loadSamplableFile(NamespacedId connectionId,
                                                 String scope,
                                                 InputStream inputStream,
                                                 S3Object s3Object, int lines
            , double fraction,
        ...
        try (BoundedLineInputStream blis =
            ...
            String sampleId = TransactionRunners.run(getContext(), context -> {
                ...
                NamespacedId workspaceId =
                        ws.createWorkspace(connectionId.getNamespace(),
                                workspaceMeta);
            ...
            // name.
            return new S3ConnectionSample(sampleId, name,
                    ConnectionType.S3.getType(),
    ...
    private S3ConnectionSample loadFile(NamespacedId connectionId, String scope,
                                        InputStream inputStream,
                                        S3Object s3Object) throws IOException {
        ...
        byte[] bytes =
                new byte[(int) s3Object.getObjectMetadata().getContentLength() + 1];
