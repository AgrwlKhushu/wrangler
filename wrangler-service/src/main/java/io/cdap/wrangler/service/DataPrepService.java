...
public class DataPrepService extends AbstractSystemService {
    private static final Logger LOG =
            LoggerFactory.getLogger(DataPrepService.class);
    ...
    public void initialize(SystemServiceContext context) {
        ...
        UpgradeState connState =
                upgradeStore.getEntityUpgradeState(UpgradeEntityType.CONNECTION);
        UpgradeState wsState =
        ...
        long upgradeBeforeTsSecs =
                TimeUnit.MILLISECONDS.toSeconds(upgradeBefore);
        if (!isConnDone) {
            try {
                ConnectionUpgrader connectionUpgrader =
                        new ConnectionUpgrader(upgradeStore, context,
                                upgradeBeforeTsSecs);
                connectionUpgrader.upgradeConnections();
            } catch (Exception e) {
                // check if there is any error upgrading the connections, if
                // true, no point to continue upgrading the workspace
                // as most connections won't be able to get the spec.
                // also we don't want the service fail to start due to
                // upgrade failure
