...
/**
 * Specification util specifically needed for upgrade. Can be used to
 * generate the spec that is known by the
 * connector/source or generate path. This class is mostly hardcoding and
 * should be removed once we remove the
 * deprecated handlers.
 */
public class SpecificationUpgradeUtils {
    ...
    /**
     * Method to retrieve the spec that is known to connector/source, this is
     * needed because the connection type handlers
     * sometimes use a different config name for property, need to have this
     * method to generate the connector properties
     ...
     * @return a connector path, null if the connection type cannot be
     * upgraded to a new connection
     */
    @Nullable
    public static String getPath(ConnectionType connectionType,
                                 Workspace workspace) {
        if (!ConnectionType.CONN_UPGRADABLE_TYPES.contains(connectionType)) {
            return null;
        }
