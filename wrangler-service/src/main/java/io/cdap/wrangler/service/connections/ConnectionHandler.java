...
/**
 * This service exposes REST APIs for managing the lifecycle of a connection
...
public class ConnectionHandler extends AbstractWranglerHandler {
    ...
    public void configure() {
        ...
        properties.put(CONNECTION_TYPE_CONFIG,
                GSON.toJson(connectionTypeConfig));
    ...
    public void initialize(SystemHttpServiceContext context) throws Exception {
        ...
        String connectionTypeConfigString =
                context.getSpecification().getProperty(CONNECTION_TYPE_CONFIG);
        connectionTypeConfig = GSON.fromJson(connectionTypeConfigString,
                ConnectionTypeConfig.class);
    ...
    private void validateAndCreateDefaultConnections() throws IOException {
        List<Connection> defaultConnections =
                connectionTypeConfig.getConnections();
        ...
        for (Connection defaultConnection : defaultConnections) {
            ...
            if (defaultConnection.getType() == null) {
                LOG.warn("Skipping the default connection {}, type is missing" +
                        " or un-recognized", defaultConnection.getName());
                continue;
            }
            ...
            if (namespaces.containsKey(connNamespace)) {
                ...
                NamespaceSummary summary =
                        getContext().getAdmin().getNamespaceSummary(connNamespace);
                if (summary == null) {
                    LOG.warn("Skipping connection '{}' since namespace '{}' " +
                                    "does not exist",
                            defaultConnection.getName(), connNamespace);
                    continue;
                }
                namespace = new Namespace(summary.getName(),
                        summary.getGeneration());
            ...
            try {
                TransactionRunners.run(getContext(), context -> {
                    ...
                    if (!store.connectionExists(namespace,
                            defaultConnection.getName())) {
                        store.create(namespace, defaultConnection, true);
                    }
                });
            } catch (Exception e) {
                // we don't want to disrupt data-prep if we are not able to
    ...
    public void create(HttpServiceRequest request,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            // Create an instance of the connection, if the connection id
            // already exists,
    ...
    @Nullable
    private String getDefaultConnection(List<Connection> connections,
    ...
    public void delete(HttpServiceRequest request,
        ...
        respond(request, responder, namespace, ns -> {
            List<Connection> preConfiguredConnections =
                    connectionTypeConfig.getConnections();
            // pre-configured connections provided as part of config only
            // contains name and type of connection
            // since connection id is derived from connection name, we get
            // the connection-id for the
            // pre-configured connection name and check with the input
            // connection id.
            if (preConfiguredConnections.stream().anyMatch(c -> id.equals(ConnectionStore.getConnectionId(c.getName())))) {
                throw new UnauthorizedException(String.format("Cannot delete " +
                        "admin controlled connection %s", id));
            }
            TransactionRunners.run(getContext(), context -> {
                ConnectionStore store = ConnectionStore.get(context);
    ...
    /**
     * Given a connection id, returns the complete information about the
     * connection.
     *
     * @param request   Handler for incoming request.
     * @param responder Responder for data going out.
     * @param id        for which all the information is returned from the
     *                  connection store.
     */
    @GET
    @Path("contexts/{context}/connections/{id}")
    ...
    public void get(HttpServiceRequest request, HttpServiceResponder responder,
        ...
        respond(request, responder, namespace,
                ns -> getConnection(new NamespacedId(ns, id)));
    }
     ...
     * @param request   Handler for incoming request.
     * @param responder Responder for data going out.
     * @param id        of the connection for which all properties need to be
     *                  returned.
     */
    @GET
    ...
    public void properties(HttpServiceRequest request,
                           ...
                           @PathParam("id") String id) {
        respond(request, responder, namespace,
                ns -> getConnection(new NamespacedId(ns, id)).getProperties());
    }
     ...
     * @param request   Handler for incoming request.
     * @param responder Responder for data going out.
     * @param id        of the connection to be who's property needs to be
     *                  updated.
     */
    @PUT
    @Path("contexts/{context}/connections/{id}/properties")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void updateProp(HttpServiceRequest request,
                           HttpServiceResponder responder,
                           @PathParam("context") String namespace,
                           @PathParam("id") String id,
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void clone(HttpServiceRequest request,
                      HttpServiceResponder responder,
                      @PathParam("context") String namespace,
                      @PathParam("id") String id) {
        respond(request, responder, namespace, ns -> {
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void getConnectionTypes(HttpServiceRequest request,
                                   HttpServiceResponder responder) {
