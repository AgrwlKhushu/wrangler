...
public class DataModelHandler extends AbstractWranglerHandler {
    ...
    /**
     * Uploads a one or more data model schemas into the internal data model
     * glossary.
     * The full name and revision of each data model schema form a unique
     * identify.
     * <p>
     * Returns:
     * 200 - if data model successfully added.
     * 400 - if the request is missing a url parameter in the body.
     * 400 - if the server was unable to load the data model at the specified
     * url.
     *
    ...
    public void upload(HttpServiceRequest request,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            DataModelUrlRequest dataModelUrl = handler.getContent("UTF-8",
                    DataModelUrlRequest.class);
    ...
    /**
     * Returns a list of all data models available. By default excludes the
    ...
    @Path("contexts/{context}/datamodels/schemas")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void listSchemas(HttpServiceRequest request,
                            HttpServiceResponder responder,
                            @PathParam("context") String namespace) {
        respond(request, responder, namespace,
                ns -> TransactionRunners.run(getContext(), context -> {
                    if (DataModelGlossary.getGlossary() == null) {
                        throw new NotFoundException("There is no data model " +
                                "initialized.");
                    }
                    Namespace defaultNamespace =
                            new Namespace(Contexts.SYSTEM, 0L);
                    List<DataModelSchemaEntry> results = new ArrayList<>();
                    for (Schema schema :
                            DataModelGlossary.getGlossary().getAll()) {
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void listSchema(HttpServiceRequest request,
                           HttpServiceResponder responder,
                           @PathParam("context") String namespace,
        ...
        respond(request, responder, namespace,
                ns -> TransactionRunners.run(getContext(), context -> {
                    ...
                    Schema schema = DataModelGlossary.getGlossary().get(id,
                            revision);
                    if (schema == null) {
                        throw new NotFoundException(String.format("unable to " +
                                "find " +
                                "schema %s revision %d", id, revision));
                    }
