...
public final class Explorer {
    ...
    public Map<String, Object> browse(String path, boolean hidden) throws ExplorerException {
        try {
            ...
            for (Location location : locations) {
                // If hidden is true, then hide all the files that start with
                // . (dot)
                if (hidden && location.getName().startsWith(".")) {
                    continue;
                }
                Map<String, Object> object = locationInfo(location);
                // If it's a directory, inspect the contents further
                // attempting to detect the type
    ...
    /**
     * This methods provides an efficiently way to read a file from the file
     * system specified by
     * the <code>Location.</code>. It uses a bounded line stream that limits
     * the lines being read
     * from the {@link java.io.BufferedInputStream}.
     *
     * @param path     Specifies the path to file to be read. Assumes the
     ...
     */
    public BoundedLineInputStream read(String path, Charset encoding,
                                       int lines) throws ExplorerException,
            IOException {
        Location file = getLocation(path);
        if (file.isDirectory()) {
            throw new ExplorerException(
                    String.format("Path '%s' specified is a directory and not" +
                            " a file.", file.toURI().getPath())
            );
        }
        return BoundedLineInputStream.iterator(file.getInputStream(),
                encoding, lines);
    }

    public BoundedLineInputStream read(String path, String encoding,
                                       int lines) throws ExplorerException,
            IOException {
     ...
     * @return bytes read.
     */
    public byte[] read(String path, int size) throws ExplorerException,
            IOException {
        Location file = getLocation(path);
        if (file.isDirectory()) {
            throw new ExplorerException(
            ...
            if (!path.isDirectory()) {
                ...
                if (listing.size() > 0) {
                    for (Location location : path.list()) {
                        String type = guessLocationType(location,
                                lookahead - 1);
                        types.add(type);
                    }
    ...
    public Location getLocation(String path) throws ExplorerException {
        ...
        try {
            ...
            Location location = baseLocation.getLocationFactory().create(
                    new URI(uri.getScheme(), uri.getUserInfo(), uri.getHost()
                            , uri.getPort(),
                            path, null, null));
            return location;
        } catch (Exception e) {
