...
/**
 * A {@link FilesystemExplorer} is a HTTP Service handler for exploring the
 * filesystem.
 * It provides capabilities for listing file(s) and directories. It also
 * provides metadata.
 */
@Deprecated
public class FilesystemExplorer extends AbstractWranglerHandler {
    ...
    public void list(HttpServiceRequest request, HttpServiceResponder responder,
        ...
        respond(request, responder, namespace, ns -> explorer.browse(path,
                hidden));
    }
     ...
     * @param responder HTTP response handler.
     * @param path      to the location in the filesystem.
     * @param lines     number of lines to extracted from file if it's a
     *                  text/plain.
    ...
    public void read(HttpServiceRequest request,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            if (header == null) {
                throw new BadRequestException("Content-Type header not " +
                        "specified.");
            }

            FileConnectionSample sample;
            if (header.equalsIgnoreCase("text/plain") || header.contains(
                    "text/")) {
                sample = loadSampleableFile(ns, scope, path, lines, fraction,
                        sampler);
            } else if (header.equalsIgnoreCase("application/xml")) {
                // using BLOB to read xml file as it needs to read the entire
                // content
                sample = loadFile(ns, scope, path, DataType.RECORDS,
                        Format.BLOB);
            } else if (header.equalsIgnoreCase("application/json")) {
                // using TEXT format here since wrangler uses its own
                // directive to parse json
                ...
                throw new BadRequestException("Currently doesn't support " +
                        "wrangling of this type of file.");
            }
    ...
    public void specification(HttpServiceRequest request,
                              HttpServiceResponder responder,
                              @PathParam("context") String namespace,
                              @QueryParam("path") String path, @QueryParam(
                    "wid") String workspaceId) {
        respond(request, responder, namespace, ns -> {
            NamespacedId namespacedId = new NamespacedId(ns, workspaceId);

            PluginSpec pluginSpec = TransactionRunners.run(getContext(),
                    context -> {
                        WorkspaceDataset ws = WorkspaceDataset.get(context);
                        Format format = Format.TEXT;
                        if (workspaceId != null) {
                            Map<String, String> config =
                                    ws.getWorkspace(namespacedId).getProperties();
                            String formatStr =
                        ...
                        properties.put("referenceName",
                                ReferenceNames.cleanseReferenceName(location.getName()));
    ...
    private FileConnectionSample loadFile(Namespace namespace, String scope,
                                          String path,
                                          DataType type, Format format) throws ExplorerException, IOException {
        Location location = explorer.getLocation(path);
        if (!location.exists()) {
            throw new BadRequestException(String.format("%s (No such file)",
                    path));
        }

        if (location.length() > FILE_SIZE) {
            throw new BadRequestException("Files larger than 10MB are " +
        ...
        String sampleId = TransactionRunners.run(getContext(), context -> {
            WorkspaceDataset ws = WorkspaceDataset.get(context);
            NamespacedId workspaceId = ws.createWorkspace(namespace,
                    workspaceMeta);
            ...
            if (type == DataType.RECORDS) {
                ...
                rows.add(new Row(COLUMN_NAME, new String(bytes,
                        Charsets.UTF_8)));
        ...
        return new FileConnectionSample(sampleId, name,
                ConnectionType.FILE.getType(),
    ...
    private FileConnectionSample loadSampleableFile(Namespace namespace,
        ...
        String sampleId = TransactionRunners.run(getContext(), context -> {
            ...
            List<Row> rows = new ArrayList<>();
            BoundedLineInputStream blis =
                    BoundedLineInputStream.iterator(location.getInputStream()
