...
public class SpannerHandler extends AbstractWranglerHandler {
    ...
    // Spanner queries for listing tables and listing schema of table are
    // documented at
    // https://cloud.google.com/spanner/docs/information-schema
    private static final Statement LIST_TABLES_STATEMENT =
            Statement.of("SELECT t.table_name FROM " +
                    "information_schema.tables AS t WHERE t.table_catalog = " +
                    "'' and t.table_schema = ''");

    private static final Statement.Builder SCHEMA_STATEMENT_BUILDER =
            Statement.newBuilder(
                    String.format("SELECT t.column_name, t.spanner_type, t" +
                            ".is_nullable FROM information_schema.columns AS " +
                            "t WHERE " +
    ...
    public static String getPath(Workspace workspace) {
        ...
        return String.format(PATH_FORMAT, conf.getInstance(),
    ...
    public void testSpannerConnection(HttpServiceRequest request,
        ...
        respond(request, responder, () -> {
            ...
            ConnectionMeta connection =
                    extractor.getConnectionMeta(ConnectionType.SPANNER);
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void getSpannerInstances(HttpServiceRequest request,
                                    HttpServiceResponder responder,
                                    @PathParam("context") String namespace,
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void getSpannerDatabases(HttpServiceRequest request,
                                    HttpServiceResponder responder,
                                    @PathParam("context") String namespace,
                                    @PathParam("connection-id") String connectionId,
        ...
        respond(request, responder, namespace, ns -> {
            Connection connection =
                    getValidatedConnection(new NamespacedId(ns, connectionId)
                            , ConnectionType.SPANNER);
            List<SpannerDatabase> databases = getDatabases(connection,
                    instanceId);
    ...
    @GET
    @Path("contexts/{context}/connections/{connection-id}/spanner/instances" +
            "/{instance-id}/databases/{database-id}/tables")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void getSpannerTables(HttpServiceRequest request,
                                 HttpServiceResponder responder,
                                 @PathParam("context") String namespace,
                                 @PathParam("connection-id") String connectionId,
                                 @PathParam("instance-id") String instanceId,
        ...
        respond(request, responder, namespace, ns -> {
            Connection connection =
                    getValidatedConnection(new NamespacedId(ns, connectionId)
                            , ConnectionType.SPANNER);
            List<SpannerTable> tables = getTables(connection, instanceId,
                    databaseId);
    ...
    public void readTable(HttpServiceRequest request,
        ...
        respond(request, responder, namespace, ns -> {
            Connection connection =
                    getValidatedConnection(new NamespacedId(ns, connectionId)
                            , ConnectionType.SPANNER);
            Schema schema = getTableSchema(connection, instanceId, databaseId
                    , tableId);
            List<Row> data = getTableData(connection, instanceId, databaseId,
                    tableId, schema, Long.parseLong(limit));
            ...
            String externalDsName =
                    new StringJoiner(".").add(instanceId).add(databaseId).add(tableId).toString();

            SpannerSpecification specification =
                    new SpannerSpecification(externalDsName, path, projectId,
                            instanceId, databaseId, tableId, schema);

            Map<String, String> workspaceProperties = new HashMap<>();
            workspaceProperties.put(PropertyIds.NAME, tableId);
            workspaceProperties.put(PropertyIds.CONNECTION_TYPE,
                    ConnectionType.SPANNER.getType());
            workspaceProperties.put(PropertyIds.CONNECTION_ID, connectionId);
            workspaceProperties.put(PropertyIds.PLUGIN_SPECIFICATION,
                    GSON.toJson(specification));
            ...
            String sampleId = TransactionRunners.run(getContext(), context -> {
                ...
                ws.updateWorkspaceData(workspaceId, DataType.RECORDS,
                        dataBytes);
    ...
    public void specification(HttpServiceRequest request,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            properties.put("referenceName",
                    ReferenceNames.cleanseReferenceName(conf.getReferenceName()));
            ...
            PluginSpec pluginSpec = new PluginSpec("Spanner", "source",
                    properties);
    ...
    private Schema getTableSchema(Connection connection,
        ...
        try {
            ...
            Statement getTableSchemaStatement =
                    SCHEMA_STATEMENT_BUILDER.bind(TABLE_NAME).to(tableId).build();
            try (ResultSet resultSet =
                         spanner.getDatabaseClient(DatabaseId.of(projectId,
                ...
                while (resultSet.next()) {
                    ...
                    Schema typeSchema =
                            parseSchemaFromSpannerTypeString(spannerType);
                    Schema fieldSchema = isNullable ?
                            Schema.nullableOf(typeSchema) : typeSchema;
    ...
    private Schema parseSchemaFromSpannerTypeString(String spannerType) throws UnsupportedTypeException {
        if (spannerType.startsWith("STRING")) {
            // STRING and BYTES also have size at the end in the format,
            // example : STRING(1024)
            ...
            switch (Type.Code.valueOf(spannerType)) {
                ...
                default:
                    throw new UnsupportedTypeException(String.format("Type : " +
                            "%s is unsupported currently", spannerType));
            }
        }
    }
    ...
    private List<Row> getTableData(Connection connection, String instanceId,
                                   ...
                                   Schema schema, long limit) throws Exception {
        Spanner spanner = GCPUtils.getSpannerService(connection);
        try {
            String projectId = spanner.getOptions().getProjectId();
            ...
            try (ResultSet resultSet =
                         spanner.getDatabaseClient(DatabaseId.of(projectId,
                                         instanceId, databaseId)).singleUse()
                                 .read(tableId, KeySet.all(), columnNames,
                                         Options.limit(limit))) {
                while (resultSet.next()) {
    ...
    private Row convertResultSetToRow(ResultSet resultSet) {
        List<Type.StructField> structFields =
                resultSet.getType().getStructFields();
        Row row = new Row();
        for (Type.StructField field : structFields) {
            ...
            switch (columnType.getCode()) {
                ...
                case BYTES:
                    ...
                    break;
                case DATE:
                    // spanner DATE is a date without time zone. so create
                    // LocalDate from spanner DATE
                    Date spannerDate = resultSet.getDate(fieldName);
                    LocalDate date = LocalDate.of(spannerDate.getYear(),
                ...
                case TIMESTAMP:
                    ...
                    // Spanner TIMESTAMP supports nano second level
                    // precision, however, cdap schema only supports
                    // microsecond level precision.
                    Instant instant =
                            Instant.ofEpochSecond(spannerTs.getSeconds()).plusNanos(spannerTs.getNanos());
                    row.add(fieldName, ZonedDateTime.ofInstant(instant,
    ...
    private List<SpannerDatabase> getDatabases(ConnectionMeta connection,
    ...
    private List<SpannerTable> getTables(Connection connection,
                                         String instanceId,
        ...
        try {
            ...
            try (ResultSet resultSet =
                ...
                while (resultSet.next()) {
                    tables.add(new SpannerTable(resultSet.getString(
                            "table_name")));
                }
                return tables;
            }
