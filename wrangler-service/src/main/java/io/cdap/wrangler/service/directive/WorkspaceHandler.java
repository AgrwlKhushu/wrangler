...
public class WorkspaceHandler extends AbstractDirectiveHandler {
    ...
    private static final Pattern PRAGMA_PATTERN = Pattern.compile("^\\s" +
            "*#pragma\\s+load-directives\\s+");
    ...
    public void createWorkspace(HttpServiceRequest request,
                                HttpServiceResponder responder,
                                @PathParam("context") String namespace) {
        respond(responder, namespace, ns -> {
            if (ns.getName().equalsIgnoreCase(NamespaceId.SYSTEM.getNamespace())) {
                throw new BadRequestException("Creating workspace in system " +
            ...
            if (creationRequest.getConnection() == null) {
                throw new BadRequestException("Connection name has to be " +
                        "provided to create a workspace");
            }

            SampleRequest sampleRequest = creationRequest.getSampleRequest();
            if (sampleRequest == null) {
                throw new BadRequestException("Sample request has to be " +
                        "provided to create a workspace");
            }

            SampleResponse sampleResponse =
                    discoverer.retrieveSample(namespace,
                            creationRequest.getConnection(),
                            sampleRequest);
            ...
            SampleSpec spec = new SampleSpec(
                    creationRequest.getConnection(),
                    creationRequest.getConnectionType(),
                    ...
                    detail.getRelatedPlugins().stream().map(plugin -> {
                        ...
                        Plugin pluginSpec = new Plugin(
                                ...
                                new Artifact(artifact.getName(),
                                        artifact.getVersion(),
                                        artifact.getScope()));
                        return new StageSpec(plugin.getSchema(), pluginSpec);
                    }).collect(Collectors.toSet()),
                    detail.getSupportedSampleTypes(), sampleRequest);

            WorkspaceId wsId = new WorkspaceId(ns);
            long now = System.currentTimeMillis();
            Workspace workspace =
                    Workspace.builder(generateWorkspaceName(wsId,
                                            creationRequest.getSampleRequest().getPath()),
                                    wsId.getWorkspaceId())
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    @Path("v2/contexts/{context}/workspaces")
    public void listWorkspaces(HttpServiceRequest request,
                               HttpServiceResponder responder,
                               @PathParam("context") String namespace) {
        respond(responder, namespace, ns -> {
            if (ns.getName().equalsIgnoreCase(NamespaceId.SYSTEM.getNamespace())) {
                throw new BadRequestException("Listing workspaces in system " +
                        "namespace is currently not supported");
            }
    ...
    public void getWorkspace(HttpServiceRequest request,
                             HttpServiceResponder responder,
                             @PathParam("context") String namespace,
                             @PathParam("id") String workspaceId) {
        respond(responder, namespace, ns -> {
            if (ns.getName().equalsIgnoreCase(NamespaceId.SYSTEM.getNamespace())) {
                throw new BadRequestException("Getting workspace in system " +
                        "namespace is currently not supported");
            }
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    @Path("v2/contexts/{context}/workspaces/{id}")
    public void updateWorkspace(HttpServiceRequest request,
                                HttpServiceResponder responder,
                                @PathParam("context") String namespace,
                                @PathParam("id") String workspaceId) {
        respond(responder, namespace, ns -> {
            if (ns.getName().equalsIgnoreCase(NamespaceId.SYSTEM.getNamespace())) {
                throw new BadRequestException("Updating workspace in system " +
                        "namespace is currently not supported");
            }
            ...
            Workspace newWorkspace =
                    Workspace.builder(wsStore.getWorkspace(wsId))
    ...
    /**
     * Resample the workspace using a new sample request. Keeps all
     * previously-applied directives.
     */
    @POST
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    @Path("v2/contexts/{context}/workspaces/{id}/resample")
    public void resampleWorkspace(HttpServiceRequest request,
                                  HttpServiceResponder responder,
                                  @PathParam("context") String namespace,
                                  @PathParam("id") String workspaceId) {
        respond(responder, namespace, ns -> {
            if (ns.getName().equalsIgnoreCase(NamespaceId.SYSTEM.getNamespace())) {
                throw new BadRequestException("Resampling workspace in system" +
                        " namespace is currently not supported");
            }
            ...
            String connectionName = currentWorkspace.getSampleSpec() == null
                    ? null :
                    currentWorkspace.getSampleSpec().getConnectionName();
            if (connectionName == null) {
                throw new BadRequestException("Connection name has to exist " +
                        "to resample a workspace");
            }

            String sampleRequestString =
                    StandardCharsets.UTF_8.decode(request.getContent()).toString();
            // sampleRequestString looks like {"sampleRequest": {...}}, so to
            // parse it into SampleRequest.class
            ...
            SampleRequest sampleRequest = GSON.fromJson(sampleRequestJson,
                    SampleRequest.class);
            if (sampleRequest == null) {
                throw new BadRequestException("Sample request has to be " +
                        "provided to resample a workspace");
            }

            SampleResponse sampleResponse =
                    discoverer.retrieveSample(namespace, connectionName,
                            sampleRequest);
            List<Row> rows = getSample(sampleResponse);

            SampleSpec oldSpec = currentWorkspace.getSampleSpec();
            SampleSpec newSpec = new SampleSpec(oldSpec.getConnectionName(),
                    oldSpec.getConnectionType(), oldSpec.getPath(),
                    oldSpec.getRelatedPlugins(),
                    oldSpec.getSupportedSampleTypes(), sampleRequest);
            ...
            wsStore.saveWorkspace(wsId, new WorkspaceDetail(newWorkspace,
                    rows));
            responder.sendStatus(HttpURLConnection.HTTP_OK);
        });
    }
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    @Path("v2/contexts/{context}/workspaces/{id}")
    public void deleteWorkspace(HttpServiceRequest request,
                                HttpServiceResponder responder,
                                @PathParam("context") String namespace,
                                @PathParam("id") String workspaceId) {
        respond(responder, namespace, ns -> {
            if (ns.getName().equalsIgnoreCase(NamespaceId.SYSTEM.getNamespace())) {
                throw new BadRequestException("Deleting workspace in system " +
                        "namespace is currently not supported");
            }
    ...
    public void upload(HttpServiceRequest request,
        ...
        respond(responder, namespace, ns -> {
            if (ns.getName().equalsIgnoreCase(NamespaceId.SYSTEM.getNamespace())) {
                throw new BadRequestException("Uploading data in system " +
                        "namespace is currently not supported");
            }
            ...
            // For back-ward compatibility, we check if there is delimiter
            // specified
            ...
            if (content == null) {
                throw new BadRequestException(
                        "Body not present, please post the file containing " +
                                "the records to create a workspace.");
            }
            ...
            Metrics child =
                    metrics.child(ImmutableMap.of(Constants.Metrics.Tag.APP_ENTITY_TYPE,
    ...
    public void execute(HttpServiceRequest request,
                        ...
                        @PathParam("id") String workspaceId) {
        respond(responder, namespace, ns -> {
            validateNamespace(ns, "Executing directives in system namespace " +
                    "is currently not supported");

            DirectiveExecutionResponse response = execute(ns, request,
                    new WorkspaceId(ns, workspaceId),
                    null);
    ...
    public void getDirectives(HttpServiceRequest request,
                              HttpServiceResponder responder,
                              @PathParam("context") String namespace) {
        respond(responder, namespace, ns -> {
            ...
            for (DirectiveInfo directive : composite.list(namespace)) {
                DirectiveUsage directiveUsage =
                        new DirectiveUsage(directive.name(), directive.usage(),
    ...
    public void specification(HttpServiceRequest request,
                              HttpServiceResponder responder,
                              @PathParam("context") String namespace,
                              @PathParam("id") String workspaceId) {
        respond(responder, namespace, ns -> {
            if (ns.getName().equalsIgnoreCase(NamespaceId.SYSTEM.getNamespace())) {
                throw new BadRequestException("Getting specification in " +
                        "system namespace is currently not supported");
            }
            ...
            List<String> directives =
                    new ArrayList<>(detail.getWorkspace().getDirectives());
            UserDirectivesCollector userDirectivesCollector =
                    new UserDirectivesCollector();
            List<Row> result = executeDirectives(ns.getName(), directives,
                    detail, userDirectivesCollector);
            userDirectivesCollector.addLoadDirectivesPragma(directives);

            Schema outputSchema;
            if (schemaManagementEnabled) {
                outputSchema =
                        TRANSIENT_STORE.get(TransientStoreKeys.OUTPUT_SCHEMA) != null ?
                ...
                outputSchema = result.isEmpty() ? null :
                        schemaConvertor.toSchema("record",
                                RowHelper.createMergedRow(result));
            }

            // check if the rows are empty before going to create a record
            // schema, it will result in a 400 if empty fields
            // are passed to a record type schema
            Map<String, String> properties = ImmutableMap.of("directives",
                    String.join("\n", directives),
                    "field", "*",
                    "precondition", "false",
            ...
            responder.sendString(GSON.toJson(new WorkspaceSpec(
                    srcSpecs, new StageSpec(outputSchema, new Plugin(
                    "Wrangler", "transform", properties,
                    wrangler == null ? null :
                            new Artifact(wrangler.getName(),
                                    wrangler.getVersion(),
    ...
    public void applyRecipe(HttpServiceRequest request,
                            HttpServiceResponder responder,
                            @PathParam("context") String namespace,
                            @PathParam("id") String workspaceId,
        ...
        respond(responder, namespace, ns -> {
            ...
            RecipeId recipeId =
                    RecipeId.builder(ns).setRecipeId(recipeIdString).build();
    ...
    private DirectiveExecutionResponse execute(NamespaceSummary ns,
        ...
        List<String> directives =
                new ArrayList<>(executionRequest.getDirectives());
     ...
     */
    private Set<StageSpec> getSourceSpecs(WorkspaceDetail detail,
                                          List<String> directives) {
        SampleSpec sampleSpec = detail.getWorkspace().getSampleSpec();
        Set<StageSpec> srcSpecs = sampleSpec == null ?
                Collections.emptySet() : sampleSpec.getRelatedPlugins();

        // really hacky way for the parse-as-csv directive, should get
        // removed once we have support to provide the
        // format properties when doing sampling
        boolean shouldCopyHeader =
                directives.stream()
                        .map(String::trim)
                        .anyMatch(directive -> directive.startsWith("parse-as" +
                                "-csv") && directive.endsWith("true"));
        if (shouldCopyHeader && !srcSpecs.isEmpty()) {
            srcSpecs = srcSpecs.stream().map(stageSpec -> {
                ...
                Map<String, String> srcProperties =
                        new HashMap<>(plugin.getProperties());
                srcProperties.put("copyHeader", "true");
                return new StageSpec(stageSpec.getSchema(),
                        new Plugin(plugin.getName(), plugin.getType(),
     ...
     * 1. If the path is not null or empty, the name will be last portion of
     * path starting from "/".
     * If "/" does not exist, the name will be the path itself.
     * 2. If path is null or empty or equal to "/", the name will be the uuid
     * for the workspace
     */
    private String generateWorkspaceName(WorkspaceId id,
     ...
     *
     * @param namespace      the namespace to operate on for finding user
     *                       defined directives
     * @param directives     the list of directives to apply. The list
     *                       provided must be a mutable list for the addition of
    ...
    private <E extends Exception> List<Row> executeDirectives(String namespace,
        ...
        // Remove all the #pragma from the existing directives. New ones will
        // be generated.
        ...
        if (schemaManagementEnabled) {
            ...
            Schema inputSchema = spec != null ?
                    spec.getRelatedPlugins().iterator().next().getSchema() :
                    Schema.recordOf("inputSchema", Schema.Field.of("body",
    ...
    /**
     * Executes the given list of directives on the given workspace locally
     * in the same JVM.
     *
     * @param namespace      the namespace to operate on for finding user
     *                       defined directives
     ...
     * @return the resulting rows after applying the directives
     */
    private <E extends Exception> List<Row> executeLocally(String namespace,
                                                           List<String> directives,
        ...
        // load the udd
        composite.reload(namespace);
        return executeDirectives(namespace, directives,
                new ArrayList<>(detail.getSample()),
                grammarVisitor);
    }

    /**
     * Executes the given list of directives on the given workspace remotely
     * using the task worker framework.
     *
     * @param namespace      the namespace to operate on for finding user
     *                       defined directives
     * @param directives     the list of directives to apply. The list
     ...
     * @return the resulting rows after applying the directives
     */
    private <E extends Exception> List<Row> executeRemotely(String namespace,
                                                            List<String> directives,
        ...
        RemoteDirectiveRequest directiveRequest =
                new RemoteDirectiveRequest(recipe, systemDirectives,
                        namespace, detail.getSampleAsBytes(),
                        TRANSIENT_STORE.get(INPUT_SCHEMA));
        RunnableTaskRequest runnableTaskRequest =
                RunnableTaskRequest.getBuilder(RemoteExecutionTask.class.getName())
        ...
        if (Feature.WRANGLER_KRYO_SERIALIZATION.isEnabled(getContext())) {
            ...
            response =
                    new ObjectSerDe<RemoteDirectiveResponse>().toObject(bytes);
        }
        if (response.getOutputSchema() != null) {
            TRANSIENT_STORE.set(TransientVariableScope.GLOBAL, OUTPUT_SCHEMA,
