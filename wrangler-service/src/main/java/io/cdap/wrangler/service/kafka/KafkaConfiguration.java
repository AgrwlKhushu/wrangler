...
public final class KafkaConfiguration {
    ...
    public KafkaConfiguration(ConnectionMeta conn) {
        ...
        if (conn.getType() != ConnectionType.KAFKA) {
            throw new IllegalArgumentException(
                    String.format("Connection '%s' is not a Kafka " +
                            "configuration.", conn.getName())
            );
        }

        Map<String, String> properties = conn.getProperties();
        if (properties == null || properties.size() == 0) {
            throw new IllegalArgumentException("Kafka properties are not " +
                    "defined. Check connection setting.");
        }
        ...
        String requestTimeoutMs =
                properties.get(ConsumerConfig.REQUEST_TIMEOUT_MS_CONFIG);
        // default the request timeout to 15 seconds, to avoid hanging for
        // minutes
    ...
    private String deserialize(String type) {
        ...
        switch (type) {
            ...
            default:
                throw new IllegalArgumentException(
                        String.format("Deserializer '%s' type not supported."
                                , type)
                );
        }
    }
