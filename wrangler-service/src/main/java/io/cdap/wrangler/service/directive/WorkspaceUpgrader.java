...
/**
 * Upgrader for workspaces. This upgrade should be run after the
 * {@link ConnectionUpgrader}
 */
public class WorkspaceUpgrader {
    private static final Logger LOG =
            LoggerFactory.getLogger(WorkspaceUpgrader.class);
    ...
    public WorkspaceUpgrader(UpgradeStore upgradeStore,
                             SystemServiceContext context,
                             long upgradeBeforeTsSecs,
    ...
    public void upgradeWorkspaces() throws Exception {
        ...
        for (NamespaceSummary ns : namespaces) {
            UpgradeState state = upgradeStore.getEntityUpgradeState(ns,
                    UpgradeEntityType.WORKSPACE);
            if (state == null || state.getVersion() == 0L) {
                upgradeWorkspacesInConnections(ns);
            }
        }
        upgradeStore.setEntityUpgradeState(UpgradeEntityType.WORKSPACE,
                WS_COMPLETE_STATE);
    }
    ...
    private void upgradeWorkspacesInConnections(NamespaceSummary namespace) {
        ...
        for (Workspace workspace : workspaces) {
            ...
            try {
                ...
                // this should not happen, but guard here to avoid failing
                // the entire upgrade process
                LOG.warn("Could not decode the sample data for workspace {}. " +
                                "This workspace will not be upgraded",
                        workspace.getName(), e);
            }
            List<String> directives = workspace.getRequest() == null ?
                    Collections.emptyList() :
                    workspace.getRequest().getRecipe().getDirectives();
            WorkspaceId workspaceId = new WorkspaceId(namespace,
                    workspace.getNamespacedId().getId());

            long now = System.currentTimeMillis();
            io.cdap.wrangler.proto.workspace.v2.Workspace.Builder ws =
                    io.cdap.wrangler.proto.workspace.v2.Workspace
                            .builder(workspace.getName(),
                                    workspace.getNamespacedId().getId()).setDirectives(directives)
            ...
            if (!ConnectionType.CONN_UPGRADABLE_TYPES.contains(connectionType)) {
                wsStore.saveWorkspace(workspaceId,
                        new WorkspaceDetail(ws.build(), sample));
                continue;
            }

            String connectorName = connectionType.getConnectorName();
            String path = SpecificationUpgradeUtils.getPath(connectionType,
                    workspace);

            String connection =
                    workspace.getProperties().get(PropertyIds.CONNECTION_ID);
            try {
                ConnectorDetail detail =
                        discoverer.getSpecification(namespace.getName(),
                                connection,
                                new SpecGenerationRequest(path,
                                        Collections.emptyMap()));

                // special handle of the database type, the current db
                // connector supports browsing from schema/table depending
                // on the database type, here it is safe to just use the old
                // way to generate spec since:
                // 1. the old db connection might not contain database in
                // connection string
                // 2. it does not support schema, so db type which supports
                // schema can easily get messed up due to the path
                //    not set up correctly. For example, for mysql, the
                //    getspec call will return the source schema since it
                //    mysql itself does not support schema. But SqlServer
                //    will not return source schema because it supports
                //    schema and the original workspace does not have this
                //    information. To make sure, the path does not mess up,
                //    the above path for db connection will always be "/".
                // 3. Using the old properties will ensure the old behavior
                // get reserved for the db connectors
                ...
                if (connectionType.equals(ConnectionType.DATABASE)) {
                    ...
                    try {
                        dbSchema = sample.isEmpty() ? null :
                                schemaConvertor.toSchema("record",
                                        RowHelper.createMergedRow(sample));
                    } catch (RecordConvertorException e) {
                        LOG.warn("Unable to get the source schema for " +
                                        "workspace {}, the generated spec " +
                                        "will not contain schema.",
                                workspace.getName());
                    }
                    ...
                    dbProperties = DatabaseHandler.getSpecification(conn,
                            workspace.getName());
                }
                ...
                for (PluginDetail plugin : detail.getRelatedPlugins()) {
                    Schema schema =
                            connectionType.equals(ConnectionType.DATABASE) ?
                                    dbSchema : plugin.getSchema();
                    Map<String, String> properties =
                            connectionType.equals(ConnectionType.DATABASE) ?
                                    dbProperties : plugin.getProperties();
                ...
                SampleSpec spec = new SampleSpec(connection, connectorName,
                        path, relatedPlugins);
                wsStore.saveWorkspace(workspaceId,
                        new WorkspaceDetail(ws.setSampleSpec(spec).build(),
                                sample));
            } catch (NotFoundException e) {
                LOG.warn("Connection {} related to workspace {} does not " +
                        "exist. " +
                        "The workspace will be upgraded without that " +
                        "information", connection, workspace.getName());
                wsStore.saveWorkspace(workspaceId,
                        new WorkspaceDetail(ws.build(), sample));
            } catch (Exception e) {
                LOG.warn("Unable to get the spec from connection {} for " +
                                "workspace {}. The workspace will not be " +
                                "upgraded.",
                        connection, workspace.getName());
            }
        }

        upgradeStore.setEntityUpgradeState(namespace,
                UpgradeEntityType.WORKSPACE, WS_COMPLETE_STATE);
    }
}
