...
public final class KafkaHandler extends AbstractWranglerHandler {
    ...
    public void test(HttpServiceRequest request, HttpServiceResponder responder,
        ...
        respond(request, responder, () -> {
            ...
            ConnectionMeta connection =
                    extractor.getConnectionMeta(ConnectionType.KAFKA);
            ...
            return new ServiceResponse<Void>(String.format("Successfully " +
                    "connected to Kafka at %s", config.getConnection()));
        });
    }
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void list(HttpServiceRequest request,
        ...
        respond(request, responder, namespace,
                ns -> TransactionRunners.run(getContext(), context -> {
                    ...
                    Connection connection = getValidatedConnection(store,
                            new NamespacedId(ns, id), ConnectionType.KAFKA);

                    KafkaConfiguration config =
                            new KafkaConfiguration(connection);
                    KafkaConsumer<String, String> consumer =
                            new KafkaConsumer<>(config.get());
                    ...
                    properties.put(PropertyIds.CONNECTION_TYPE,
                            connection.getType().getType());
                    properties.put(PropertyIds.KEY_DESERIALIZER,
                            config.getKeyDeserializer());
                    properties.put(PropertyIds.VALUE_DESERIALIZER,
                            config.getValueDeserializer());
                    properties.put(PropertyIds.SAMPLER_TYPE,
                            SamplingMethod.FIRST.getMethod());
                    ...
                    try {
                        ...
                        ConnectionSample sample =
                                new ConnectionSample(workspaceId.getId(), topic,
                                        ConnectionType.KAFKA.getType(),
    ...
    public void specification(HttpServiceRequest request,
                              ...
                              @PathParam("topic") String topic) {
        respond(request, responder, namespace, ns -> {
            Connection conn = getValidatedConnection(new NamespacedId(ns, id)
            ...
            properties.put("referenceName",
                    ReferenceNames.cleanseReferenceName(topic));
            properties.put("brokers", connProperties.get(PropertyIds.BROKER));
            properties.put("kafkaBrokers",
                    connProperties.get(PropertyIds.BROKER));
            properties.put("keyField",
                    connProperties.get(PropertyIds.KEY_DESERIALIZER));
            properties.put("format", "text");

            PluginSpec pluginSpec = new PluginSpec("Kafka", "source",
                    properties);
