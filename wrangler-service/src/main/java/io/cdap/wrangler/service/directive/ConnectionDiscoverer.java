...
public class ConnectionDiscoverer {
    private static final Gson GSON =
            new GsonBuilder()
                    ...
                    .registerTypeAdapter(SampleResponse.class,
                            new SampleResponseCodec()).create();
    ...
    private static final long RETRY_MAX_DELAY_MILLIS =
    ...
    public void addConnection(String namespace, String connectionName,
        ...
        String url = String.format("v1/contexts/%s/connections/%s", namespace
                , connectionName);
    ...
    public ConnectorDetail getSpecification(String namespace,
                                            String connectionName,
                                            SpecGenerationRequest request) throws IOException, InterruptedException {
        String url = String.format("v1/contexts/%s/connections/%s" +
                "/specification", namespace, connectionName);
    ...
    public SampleResponse retrieveSample(String namespace,
                                         String connectionName,
                                         SampleRequest sampleRequest) throws IOException, InterruptedException {
        String url = String.format("v1/contexts/%s/connections/%s/sample",
    ...
    private <T> T execute(String namespace, String connectionName, String url,
        ...
        while (stopWatch.elapsed(TimeUnit.MILLISECONDS) < TIMEOUT_MILLIS) {
            try {
                ...
                throw new IOException(
                        String.format("Failed to retrieve sample for " +
                                        "connection '%s' in namespace '%s'. " +
                                        "Error: %s",
        ...
        if (latest != null) {
            throw new IOException(String.format("Timed out when trying to " +
                            "retrieve sample for " +
                            "connection '%s' in namespace '%s'.",
                    connectionName,
                    namespace), latest);
        ...
        // should not get here, the call should either already fail with a
        // non retryable failure or timed out because of
        // retryable failure
        throw new IllegalStateException(String.format("Unable to retrieve " +
                        "sample for " +
                        "connection '%s' in namespace '%s'.", connectionName,
                namespace));
    }

    private HttpURLConnection retrieveConnectionUrl(String url) throws IOException {
        // encode the url since connection name can contain space, and other
        // special characters
        // use guava url escaper since URLEncoder will encode space to plus,
        // which is wrong for the connection name
        ...
        HttpURLConnection urlConn = serviceDiscoverer.openConnection(
                ...
                Constants.STUDIO_SERVICE_NAME, url);
     ...
     */
    private <T> T retrieveResult(HttpURLConnection urlConn,
                                 @Nullable Class<T> type) throws IOException {
        ...
        if (responseCode != HttpURLConnection.HTTP_OK) {
            switch (responseCode) {
                ...
                case HttpURLConnection.HTTP_GATEWAY_TIMEOUT:
                    throw new RetryableException("Connection service is not " +
                            "available with status " + responseCode);
            ...
            throw new IOException("Failed to call connection service with " +
                    "status " + responseCode + ": " +
                    getError(urlConn));
        }
    ...
    /**
     * Returns the full content of the error stream for the given
