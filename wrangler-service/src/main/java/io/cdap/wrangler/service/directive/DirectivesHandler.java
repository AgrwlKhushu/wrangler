...
/**
 * Service for managing workspaces and also application of directives on to
 * the workspace.
 */
@Deprecated
public class DirectivesHandler extends AbstractDirectiveHandler {
    private static final Logger LOG =
            LoggerFactory.getLogger(DirectivesHandler.class);
    private static final Gson GSON =
            new GsonBuilder().registerTypeAdapter(Schema.class,
                    new SchemaTypeAdapter()).create();

    private static final String DATA_MODEL_PROPERTY = "dataModel";
    private static final String DATA_MODEL_REVISION_PROPERTY =
            "dataModelRevision";
            ...
            case RECORDS: {
                if (workspace.getData() != null) {
                    try (ObjectInputStream ois =
                                 new ObjectInputStream(new ByteArrayInputStream(workspace.getData()))) {
    ...
    public void destroy() {
        ...
        try {
            ...
            // If something bad happens here, you might see a a lot of open
            // file handles.
            LOG.warn("Unable to close the directive registry. You might see " +
                    "increasing number of open file handle.", e);
        }
    }
    ...
    public void healthCheck(HttpServiceRequest request,
    ...
    public void create(HttpServiceRequest request,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            return new ServiceResponse<Void>(String.format("Successfully " +
                    "created workspace '%s'", id));
        });
    }
    ...
    public void list(HttpServiceRequest request, HttpServiceResponder responder,
                     @PathParam("context") String namespace, @QueryParam(
                    "scope") @DefaultValue("default") String scope) {
        respond(request, responder, namespace, ns -> {
            List<WorkspaceIdentifier> workspaces =
                    TransactionRunners.run(getContext(), context -> {
                        WorkspaceDataset ws = WorkspaceDataset.get(context);
    ...
    @Path("contexts/{context}/workspaces/{id}")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void delete(HttpServiceRequest request,
                       HttpServiceResponder responder,
                       @PathParam("context") String namespace, @PathParam("id"
            ) String id) {
        respond(request, responder, namespace, ns -> {
            TransactionRunners.run(getContext(), context -> {
                WorkspaceDataset ws = WorkspaceDataset.get(context);
    ...
    @Path("contexts/{context}/workspaces/")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void deleteGroup(HttpServiceRequest request,
                            HttpServiceResponder responder,
                            @PathParam("context") String namespace,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            return new ServiceResponse<Void>(String.format("Successfully " +
                    "deleted workspaces within group '%s'", group));
        });
    }
    ...
    public void get(HttpServiceRequest request, HttpServiceResponder responder,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            req.add("properties", merge(req.getAsJsonObject("properties"),
                    prop));
    ...
    /**
     * Upload data to the workspace, the workspace is created automatically
     * on fly.
     *
     * @param request   Handler for incoming request.
     * @param responder Responder for data going out.
    ...
    @Path("contexts/{context}/workspaces")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void upload(HttpServiceRequest request,
                       HttpServiceResponder responder,
                       @PathParam("context") String namespace) {
        respond(request, responder, namespace, ns -> {
            String name = request.getHeader(PropertyIds.FILE_NAME);
            if (name == null) {
                throw new BadRequestException("Name must be provided in the " +
                        "'file' header");
            }
            NamespacedId id = new NamespacedId(ns,
                    ServiceUtils.generateMD5(name));

            return TransactionRunners.run(getContext(), context -> {
                // if workspace doesn't exist, then we create the workspace
                // before
                WorkspaceDataset ws = WorkspaceDataset.get(context);
                // adding data to the workspace.
                if (!ws.hasWorkspace(id)) {
                    ws.writeWorkspaceMeta(id,
                            WorkspaceMeta.builder(name).build());
                }

                RequestExtractor handler = new RequestExtractor(request);

                // For back-ward compatibility, we check if there is
                // delimiter specified
                ...
                String charset =
                        handler.getHeader(RequestExtractor.CHARSET_HEADER,
                                "UTF-8");

                // Get content type - application/data-prep,
                // application/octet-stream or text/plain.
                String contentType =
                        handler.getHeader(RequestExtractor.CONTENT_TYPE_HEADER, "application/data-prep");

                // Extract content.
                byte[] content = handler.getContent();
                if (content == null) {
                    throw new BadRequestException("Body not present, please " +
                            "post the file containing the "
                ...
                if (type == null) {
                    throw new BadRequestException("Invalid content type. Must" +
                            " be 'text/plain', 'application/octet-stream' " +
                ...
                switch (type) {
                    case TEXT:
                        ...
                        ws.updateWorkspaceData(id, DataType.TEXT,
                                Bytes.toBytes(body));
                        break;
                ...
                properties.put(PropertyIds.CONNECTION_TYPE,
                        ConnectionType.UPLOAD.getType());
                ws.updateWorkspaceProperties(id, properties);

                WorkspaceInfo workspaceInfo = new WorkspaceInfo(id.getId(),
                        name, delimiter, charset, contentType,
                        ConnectionType.UPLOAD.getType(),
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void uploadData(HttpServiceRequest request,
                           HttpServiceResponder responder,
                           @PathParam("context") String namespace,
                           @PathParam("id") String id) {
        respond(request, responder, namespace, ns -> {
            ...
            // Extract charset, if not specified, default it to UTF-8.
            String charset =
                    handler.getHeader(RequestExtractor.CHARSET_HEADER, "UTF-8");

            // Get content type - application/data-prep,
            // application/octet-stream or text/plain.
            String contentType =
                    handler.getHeader(RequestExtractor.CONTENT_TYPE_HEADER,
                            "application/data-prep");

            // Extract content.
            byte[] content = handler.getContent();
            if (content == null) {
                throw new BadRequestException("Body not present, please post " +
                        "the file containing the records to be wrangled.");
            }

            // Depending on content type, load data.
            DataType type = DataType.fromString(contentType);
            if (type == null) {
                throw new BadRequestException("Invalid content type. Must be " +
            ...
            return TransactionRunners.run(getContext(), context -> {
                ...
                switch (type) {
                    case TEXT:
                        ...
                        String body =
                                Charset.forName(charset).decode(ByteBuffer.wrap(content)).toString();
                        ws.updateWorkspaceData(namespaceId, DataType.TEXT,
                                Bytes.toBytes(body));
                        break;
                    ...
                    case RECORDS:
                        ...
                        ObjectSerDe<List<Row>> serDe = new ObjectSerDe<>();
                        byte[] bytes = serDe.toByteArray(rows);
                        ws.updateWorkspaceData(namespaceId, DataType.RECORDS,
                                bytes);
                        break;

                    case BINARY:
                        ws.updateWorkspaceData(namespaceId, DataType.BINARY,
                                content);
                        break;
                }
                ...
                return new ServiceResponse<Void>(String.format("Successfully " +
                        "uploaded data to workspace '%s'", id));
            });
        });
    }
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void execute(HttpServiceRequest request,
                        HttpServiceResponder responder,
                        @PathParam("context") String namespace, @PathParam(
                    "id") String id) {
        respond(request, responder, namespace, ns -> {
            composite.reload(namespace);
            try {
                RequestExtractor handler = new RequestExtractor(request);
                Request directiveRequest = handler.getContent("UTF-8",
                        Request.class);
                if (directiveRequest == null) {
                    throw new BadRequestException("Request body is empty.");
                }
                List<String> directives =
                        new ArrayList<>(directiveRequest.getRecipe().getDirectives());

                int limit = directiveRequest.getSampling().getLimit();
                NamespacedId namespacedId = new NamespacedId(ns, id);
                UserDirectivesCollector userDirectivesCollector =
                        new UserDirectivesCollector();
                List<Row> rows = executeDirectives(namespacedId, directives,
                        records -> {
                            if (records == null) {
                ...
                io.cdap.wrangler.proto.workspace.v2.DirectiveExecutionResponse response =
                        generateExecutionResponse(rows,
                ...
                return new DirectiveExecutionResponse(response.getValues(),
                        response.getHeaders(),
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void summary(HttpServiceRequest request,
                        HttpServiceResponder responder,
                        @PathParam("context") String namespace, @PathParam(
                    "id") String id) {
        respond(request, responder, namespace, ns -> {
            try {
                ...
                int limit = directiveRequest.getSampling().getLimit();
                List<String> directives =
                        new LinkedList<>(directiveRequest.getRecipe().getDirectives());
                List<Row> rows = executeDirectives(new NamespacedId(ns, id),
                        directives, records -> {
                            if (records == null) {
                                return Collections.emptyList();
                            }
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void schema(HttpServiceRequest request,
                       HttpServiceResponder responder,
                       @PathParam("context") String namespace, @PathParam("id"
            ) String id) {
        respond(request, responder, namespace, ns -> {
            ...
            List<String> directives =
                    new LinkedList<>(user.getRecipe().getDirectives());
            List<Row> rows = executeDirectives(new NamespacedId(ns, id),
                    directives, records -> {
                        if (records == null) {
                            return Collections.emptyList();
            ...
            Schema schema = schemaConvertor.toSchema("record",
                    RowHelper.createMergedRow(rows));
            if (schema.getType() != Schema.Type.RECORD) {
                schema = Schema.recordOf("array", Schema.Field.of("value",
                        schema));
            }
    ...
    /**
     * Adds a data model to workspace. There is a one-to-one mapping between
    ...
    public void addDataModel(HttpServiceRequest request,
                             HttpServiceResponder responder,
                             @PathParam("context") String namespace,
                             @PathParam("id") String id) {
        respond(request, responder, namespace, ns -> {
            ...
            properties.put(DATA_MODEL_REVISION_PROPERTY,
                    Long.toString(dataModelInfo.getRevision()));
    ...
    @Path("contexts/{context}/workspaces/{id}/datamodels")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void removeDataModel(HttpServiceRequest request,
                                HttpServiceResponder responder,
                                @PathParam("context") String namespace,
                                @PathParam("id") String id) {
        respond(request, responder, namespace, ns -> {
            ...
            return new ServiceResponse<Void>("successfully removed the data " +
                    "model from the workspace");
        });
    }

    /**
     * Adds a model to the workspace. There is a one-to-one mapping between
     * workspace and model. In addition, the model
     * must be a member of the data model associated with the workspace. If
    ...
    public void addModels(HttpServiceRequest request,
                          HttpServiceResponder responder,
                          @PathParam("context") String namespace, @PathParam(
                    "id") String id) {
        respond(request, responder, namespace, ns -> {
            ...
            try {
                revision =
                        Long.parseLong(properties.get(DATA_MODEL_REVISION_PROPERTY));
            } catch (NumberFormatException e) {
                revision = DataModelInfo.INVALID_REVISION;
            }
            DataModelInfo dataModelInfo =
                    new DataModelInfo(properties.get(DATA_MODEL_PROPERTY),
                            revision);
            if (dataModelInfo.getId() == null || dataModelInfo.getRevision().equals(DataModelInfo.INVALID_REVISION)) {
                throw new BadRequestException("data model or data model " +
                        "revision properties has not been set.");
            }
            ...
            org.apache.avro.Schema schema = DataModelGlossary.getGlossary()
                    .get(dataModelInfo.getId(), dataModelInfo.getRevision());
            List<org.apache.avro.Schema.Field> fieldMatch =
                    schema.getFields().stream()
                            .filter(field -> field.name().equals(model.getId()))
                            .collect(Collectors.toList());
            if (fieldMatch.isEmpty()) {
                throw new NotFoundException(
                        String.format("Unable to find model %s in data model " +
                                        "%s revision %d.", model.getId(),
                                dataModelInfo.getId(),
                                dataModelInfo.getRevision()));
            }
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void removeModels(HttpServiceRequest request,
                             HttpServiceResponder responder,
                             @PathParam("context") String namespace,
                             @PathParam("id") String id,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            if (properties.containsKey(DATA_MODEL_MODEL_PROPERTY)) {
                if (!modelId.equals(properties.get(DATA_MODEL_MODEL_PROPERTY))) {
                    throw new NotFoundException(String.format("model %s is " +
                            "not a property of the workspace.", modelId));
                }
    ...
    @Path("info")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void capabilities(HttpServiceRequest request,
                             HttpServiceResponder responder) {
        respond(request, responder,
                () -> new ServiceResponse<>(ProjectInfo.getProperties()));
    }

    /**
     * This REST API returns an array of all the directives, their usage and
     * description.
     * <p>
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void usage(HttpServiceRequest request,
                      HttpServiceResponder responder,
                      @PathParam("context") String namespace) {
        respond(request, responder, namespace, ns -> {
            // CDAP-15397 - reload must be called before it can be safely used
            composite.reload(namespace);
            DirectiveConfig config = TransactionRunners.run(getContext(),
                    context -> {
            ...
            for (DirectiveInfo directive : composite.list(namespace)) {
                DirectiveUsage directiveUsage =
                        new DirectiveUsage(directive.name(),
                                directive.usage(), directive.description(),
                ...
                if (aliases.containsKey(directive.name())) {
                    ...
                    for (String alias : list) {
                        directiveUsage = new DirectiveUsage(alias,
                                directive.usage(), directive.description(),
     ...
     * of type <code>Directive.Type</code> (directive). Artifact will be
     * reported
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void artifacts(HttpServiceRequest request,
                          HttpServiceResponder responder,
                          @PathParam("context") String namespace) {
        respond(request, responder, namespace, ns -> {
            List<DirectiveArtifact> values = new ArrayList<>();
            List<ArtifactInfo> artifacts =
                    getContext().listArtifacts(namespace);
     ...
     * if it atleast has one plugin that is of type "directive".
     * <p>
     * TODO: (CDAP-14694) Doesn't look like this should exist. Why wasn't the
     * CDAP endpoint used?
     */
    @GET
    @Path("contexts/{context}/directives")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void directives(HttpServiceRequest request,
                           HttpServiceResponder responder,
                           @PathParam("context") String namespace) {
        respond(request, responder, namespace, ns -> {
            ...
            for (ArtifactInfo artifact : artifacts) {
                ...
                DirectiveArtifact directiveArtifact =
                        new DirectiveArtifact(artifact.getName(),
                                artifact.getVersion(),
                                artifact.getScope().name());
                for (PluginClass plugin : plugins) {
                    if (Directive.TYPE.equalsIgnoreCase(plugin.getType())) {
                        values.add(new DirectiveDescriptor(plugin,
                                directiveArtifact));
                    }
                }
            }
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void directivesReload(HttpServiceRequest request,
                                 HttpServiceResponder responder,
                                 @PathParam("context") String namespace) {
        respond(request, responder, namespace, ns -> {
            composite.reload(namespace);
            return new ServiceResponse<Void>("Successfully reloaded all user " +
                    "defined directives.");
        });
    }
    ...
    @Path("charsets")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void charsets(HttpServiceRequest request,
                         HttpServiceResponder responder) {
        respond(request, responder,
                () -> new ServiceResponse<>(Charset.availableCharsets().keySet()));
    }
    ...
    @Path("config")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void uploadConfig(HttpServiceRequest request,
                             HttpServiceResponder responder) {
        respond(request, responder, () -> {
            // Read the request body
            RequestExtractor handler = new RequestExtractor(request);
            DirectiveConfig config = handler.getContent("UTF-8",
                    DirectiveConfig.class);
            if (config == null) {
                throw new BadRequestException("Config is empty. Please check " +
                        "if the request is sent as HTTP POST body.");
            }
            TransactionRunners.run(getContext(), context -> {
                ConfigStore configStore = ConfigStore.get(context);
                configStore.updateConfig(config);
            });
            return new ServiceResponse<Void>("Successfully updated " +
                    "configuration.");
        });
    }
    ...
    @Path("config")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void getConfig(HttpServiceRequest request,
                          HttpServiceResponder responder) {
        respond(request, responder, () -> TransactionRunners.run(getContext()
                , context -> {
                    ConfigStore configStore = ConfigStore.get(context);
     ...
     */
    private List<Row> executeDirectives(NamespacedId id,
                                        List<String> directives,
     ...
     * @return records generated from the directives.
     */
    private <E extends Exception> List<Row> executeDirectives(NamespacedId id
            , List<String> directives,
                                                              Function<List<Row>, List<Row>> sample,
        ...
        return TransactionRunners.run(getContext(), ctx -> {
            ...
            return executeDirectives(id.getNamespace().getName(), directives,
                    sample.apply(rows),
                    grammarVisitor);
        });
    }
