...
public class SchemaRegistryHandler extends AbstractWranglerHandler {
    ...
    /**
     * Creates an entry for Schema with id, name, description and type of
     * schema.
     * if the 'id' already exists, then it overwrites the data with new
     * information.
    ...
    public void create(HttpServiceRequest request,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            SchemaDescriptorType descriptorType = GSON.fromJson(type,
                    SchemaDescriptorType.class);
            if (descriptorType == null) {
                throw new BadRequestException(String.format("Schema type '%s'" +
                        " is invalid.", type));
            }
            if (description == null || description.isEmpty()) {
                throw new BadRequestException("Schema description must be " +
                        "specified.");
            }
            SchemaDescriptor descriptor =
                    new SchemaDescriptor(new NamespacedId(ns, id),
            ...
            return new ServiceResponse<Void>(String.format("Successfully " +
                            "created schema entry with id '%s', name '%s'",
                    id, name));
        });
    }
     ...
     * This API will automatically increment the schema version. Upon adding
     * the schema to associated id in
     * schema registry, it returns the version to which the uploaded schema
     * was added to.
     * <p>
     ...
     * }
     * <p>
     * On any issues, returns error with proper error message and Internal
     * Server error (500).
     *
     * @param request   HTTP request handler.
     * @param responder HTTP response handler.
    ...
    public void upload(HttpServiceRequest request,
        ...
        respond(request, responder, namespace, ns -> {
            ...
            if (bytes == null) {
                throw new BadRequestException("No schema was provided in the " +
                        "request body");
            }
            ...
            return new ServiceResponse<>(new SchemaEntryVersion(namespacedId,
                    version));
        });
    }
    ...
    /**
     * Returns information of schema, including schema requested along with
     * versions available and other metadata.
     *
     * @param request   HTTP request handler.
     * @param responder HTTP response handler.
     * @param id        of the schema.
    ...
    @Path("contexts/{context}/schemas/{id}/versions/{version}")
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void get(HttpServiceRequest request,
                    HttpServiceResponder responder,
                    @PathParam("context") String namespace,
                    @PathParam("id") String id,
                    @PathParam("version") long version) {
    ...
    @TransactionPolicy(value = TransactionControl.EXPLICIT)
    public void versions(HttpServiceRequest request,
                         HttpServiceResponder responder,
