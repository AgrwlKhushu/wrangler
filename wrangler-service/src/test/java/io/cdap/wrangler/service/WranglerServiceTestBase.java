...
/**
 * Base class which exposes utility functions for interacting with
 * {@link DirectivesHandler}.
 */
@Ignore
public class WranglerServiceTestBase extends TestBase {

    protected void createAndUploadWorkspace(URL baseURL, String workspace,
                                            List<String> lines) throws Exception {
        HttpResponse response =
                HttpRequests.execute(HttpRequest.put(new URL(baseURL,
                        "workspaces/" + workspace)).build());
...
//        .build());
    ...
    protected ExecuteResponse execute(URL baseURL, String workspace,
                                      String[] directives) throws Exception {
        ...
        URL url = new URL(baseURL,
                "workspaces/" + workspace + "/execute" + createQueryParams(queryParams));
        HttpResponse response =
                HttpRequests.execute(HttpRequest.get(url).build());
        Assert.assertEquals(200, response.getResponseCode());
        return new Gson().fromJson(response.getResponseBodyAsString(),
    ...
    protected String createQueryParams(List<Map.Entry<String, String>> queryParams) {
        ...
        String params =
                "?" + queryParams.get(0).getKey() + "=" + queryParams.get(0).getValue();
