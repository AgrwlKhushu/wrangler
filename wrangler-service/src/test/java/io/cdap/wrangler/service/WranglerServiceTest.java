...
public class WranglerServiceTest extends WranglerServiceTestBase {
    ...
    public void test() throws Exception {
        ...
        Schema expectedSchema =
                Schema.recordOf("avroSchema",
                        Schema.Field.of("fname",
                                Schema.nullableOf(Schema.of(Schema.Type.STRING))),
                        Schema.Field.of("lname",
                                Schema.nullableOf(Schema.of(Schema.Type.STRING))));
    ...
    public Schema schema(URL baseURL, String workspace,
                         List<String> directives) throws Exception {
        List<Map.Entry<String, String>> queryParams = new ArrayList<>();
        for (String directive : directives) {
            queryParams.add(new AbstractMap.SimpleEntry<>("directive",
                    URLEncoder.encode(directive, "UTF-8")));
        }

        URL url = new URL(baseURL,
                "workspaces/" + workspace + "/schema" + createQueryParams(queryParams));
        HttpResponse response =
                HttpRequests.execute(HttpRequest.get(url).build());
        Assert.assertEquals(200, response.getResponseCode());

        // we have to do this, simply because of how the service REST API
        // returns only the Fields of the Schema
        return GSON.fromJson("{ \"name\": \"avroSchema\", \"type\": " +
                "\"record\", \"fields\":"
