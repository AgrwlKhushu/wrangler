...
public class TableLookupTest extends WranglerServiceTestBase {
    ...
    public void test() throws Exception {
        ...
        lookupTable.get().put(Bytes.toBytes("bob"), Bytes.toBytes("age"),
                Bytes.toBytes("21"));
        lookupTable.get().put(Bytes.toBytes("bob"), Bytes.toBytes("city"),
                Bytes.toBytes("Los Angeles, CA"));
        lookupTable.get().put(Bytes.toBytes("joe"), Bytes.toBytes("age"),
                Bytes.toBytes("34"));
        lookupTable.get().put(Bytes.toBytes("joe"), Bytes.toBytes("city"),
                Bytes.toBytes("Palo Alto, CA"));
        lookupTable.flush();


        ApplicationManager wrangerApp = deployApplication(DataPrep.class);
        ServiceManager serviceManager = wrangerApp.getServiceManager("service"
        ).start();
        // should throw exception, instead of returning null
        URL baseURL = serviceManager.getServiceURL();

        List<String> uploadContents = ImmutableList.of("bob,anderson", "joe," +
                "mchall");
        ...
        ExecuteResponse executeResponse = execute(baseURL, "test_ws",
                directives);
        Assert.assertEquals(uploadContents.size(),
                executeResponse.value.size());
        ...
        Assert.assertEquals("Los Angeles, CA",
                executeResponse.value.get(0).get("fname_city"));
        ...
        Assert.assertEquals("Palo Alto, CA",
                executeResponse.value.get(1).get("fname_city"));
    }
}
