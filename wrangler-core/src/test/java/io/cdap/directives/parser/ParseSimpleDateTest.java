...
public class ParseSimpleDateTest {
    ...
    public void testPatterns() throws Exception {
        ...
        List<String> columns = IntStream.range(0, numValues).mapToObj(i ->
                "col" + i).collect(Collectors.toList());
        String[] directives = Streams
                .zip(
                        columns.stream(),
                        patterns.stream(),
                        (column, pattern) -> String.format("%s :%s \"%s\"",
                                ParseSimpleDate.NAME, column, pattern)
                ).toArray(String[]::new);
        Row row = new Row();
        ...
        List<Row> resultRows = TestingRig.execute(directives,
        ...
        for (int i = 0; i < numValues; i++) {
            ...
            Assert.assertEquals(zonedDateTime,
                    resultRows.get(0).getValue(columns.get(i)));
        }
    }
    ...
    public void testUTCTimeZoneUsedByDefault() throws Exception {
        ...
        String column = "col";
        List<String> dates = Arrays.asList("1850-09-09", "1998-09-04", "2018" +
                "-05-14");
        String[] directives = new String[]{String.format("%s :%s \"%s\"",
                ParseSimpleDate.NAME, column, pattern)};
        List<Row> rows =
                dates.stream().map(date -> new Row(column, date)).collect(Collectors.toList());
        ...
        for (int i = 0; i < dates.size(); i++) {
            ...
            Assert.assertEquals(zonedDateTime,
                    resultRows.get(i).getValue(column));
        }
    }
    ...
    public void testGregorianCutoff() throws Exception {
        ...
        // For good measure :)
        Assert.assertEquals("0001-01-01T00:00Z[UTC]",
                resultRows.get(0).getValue(column).toString());
    }
    ...
    public void testParseExceptionCollectsAsErrorRecord() throws Exception {
        ...
        Pair<List<Row>, List<Row>> result =
                TestingRig.executeWithErrors(directives,
                        Collections.singletonList(row));
