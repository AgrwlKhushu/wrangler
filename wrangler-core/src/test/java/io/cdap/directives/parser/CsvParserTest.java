...
public class CsvParserTest {
    ...
    public void testTrailingCommas() throws Exception {
        ...
        List<Row> rows = Arrays.asList(
                new Row("body", "school_id, student_id, last_name, " +
                        "first_name,,,"),
        ...
        for (int i = 0; i < rows.size(); ++i) {
            ...
            for (int j = 0; j < 4; j++) {
                Assert.assertEquals(expected.get(i).getValue(j),
                        rows.get(i).getValue(j));
            }
        }
    }
    ...
    public void testExtraCommasAndLeadingZeros() throws Exception {
        String[] directives = new String[]{
                ...
                "set columns school_id, student_id, last_name, first_name, " +
                        "body_5",
                "set-column :last_name exp:{ this.width() == 5 ? (last_name +" +
                        " ',' + first_name) : last_name}",
                "set-column :first_name exp:{ this.width() == 5 ? body_5 : " +
                        "first_name}",
                "drop body_5"
        };
