...
public class ParseDateTimeTest {
    ...
    public void testDateTimeFormats() throws Exception {
        ...
        String[] colNames = new String[]{"col1", "col2", "col3", "col4",
                "col5"};
        String[] dateTimes = new String[]{"03/30/2010 01:05", "2020-01-28T04" +
                ":50:12", "2011-12-03T10:15:30+01:00",
                "2011-12-03T10:15:30+01:00[Europe/Paris]", "19901212 10:12 AM"};
        String[] directives = new String[testPatterns.length];
        Row row = new Row();
        for (int i = 0; i < testPatterns.length; i++) {
            directives[i] = String
                    .format("%s :%s \"%s\"", ParseDateTime.NAME, colNames[i],
                            testPatterns[i]);
        ...
        for (Row resultRow : rows) {
            for (int i = 0; i < testPatterns.length; i++) {
                DateTimeFormatter dateTimeFormatter =
                        DateTimeFormatter.ofPattern(testPatterns[i]);
                Assert.assertEquals(LocalDateTime.parse(dateTimes[i],
                                dateTimeFormatter),
    ...
    public void testDateTimeMultipleRows() throws Exception {
        ...
        String[] directives = new String[]{
                String.format("%s :%s '%s'", ParseDateTime.NAME, colName,
                        pattern)
        };
        Row row1 = new Row();
