...
public class ParseAvroFileTest {
    ...
    public void testParseAsAvroFile() throws Exception {
        InputStream stream =
                ParseAvroFileTest.class.getClassLoader().getResourceAsStream(
                        "cdap-log.avro");
        ...
        Assert.assertEquals(1495172588118L, results.get(0).getValue(
                "timestamp"));
        Assert.assertEquals(1495194308245L, results.get(1688).getValue(
                "timestamp"));
    }
    ...
    public void testIncorrectType() throws Exception {
        ...
        List<Row> rows = new ArrayList<>();
        rows.add(new Row("body",
                new String("failure").getBytes(Charsets.UTF_8)));
