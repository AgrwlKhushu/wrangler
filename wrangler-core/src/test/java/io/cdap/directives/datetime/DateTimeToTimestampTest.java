...
public class DateTimeToTimestampTest {
    ...
    public void testZones() throws Exception {
        String[] testZones = new String[]{"UTC", "GMT", "Australia/Sydney",
                "America/Los_Angeles"};
        String[] colNames = new String[]{"col1", "col2", "col3", "col4"};
        LocalDateTime localDateTime = LocalDateTime.of(2000, 8, 22, 20, 36,
                45, 1234);
        String[] directives = new String[testZones.length];
        Row row = new Row();
        for (int i = 0; i < testZones.length; i++) {
            directives[i] = String.format("%s :%s \"%s\"",
                    DateTimeToTimeStamp.NAME, colNames[i], testZones[i]);
        ...
        for (Row resultRow : rows) {
            for (int i = 0; i < testZones.length; i++) {
                Assert.assertEquals(ZonedDateTime.of(localDateTime,
                                ZoneId.of(testZones[i])),
    ...
    public void testDefaultZone() throws Exception {
        String colName = "col1";
        String[] directives = new String[]{String.format("%s :%s",
                DateTimeToTimeStamp.NAME, colName)};
        Row row1 = new Row();
        LocalDateTime now = LocalDateTime.now();
        row1.add(colName, now);
        List<Row> result = TestingRig.execute(directives,
    ...
    public void testInvalidZone() throws Exception {
        ...
        String[] directives = new String[]{String.format("%s :%s '%s'",
                DateTimeToTimeStamp.NAME, colName, zone)};
        Row row1 = new Row();
    ...
    public void testInvalidObject() throws Exception {
        ...
        final List<Row> results = TestingRig.execute(directives,
                Collections.singletonList(row1));
