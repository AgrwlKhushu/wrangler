...
public class FormatDateTimeTest {
    ...
    public void testDateTimeFormats() throws Exception {
        String[] testPatterns = new String[]{"MM/dd/yyyy HH:mm", "yyyy-MM-dd" +
                "'T'HH:mm:ss", "yyyy-MM-dd'T'HH:mm:ss[xxx]",
                "yyyyMMdd h:mm a"};
        String[] colNames = new String[]{"col1", "col2", "col3", "col4",
                "col5"};
        LocalDateTime localDateTime = LocalDateTime.of(2000, 8, 22, 20, 36,
                45, 1234);
        String[] dateTimes = new String[]{"08/22/2000 20:36", "2000-08-22T20" +
                ":36:45", "2000-08-22T20:36:45",
                "20000822 8:36 PM"};
        String[] directives = new String[testPatterns.length];
        Row row = new Row();
        for (int i = 0; i < testPatterns.length; i++) {
            directives[i] = String.format("%s :%s \"%s\"",
                    FormatDateTime.NAME, colNames[i], testPatterns[i]);
        ...
        for (Row resultRow : rows) {
            for (int i = 0; i < testPatterns.length; i++) {
                Assert.assertEquals(dateTimes[i],
    ...
    public void testInvalidFormat() throws Exception {
        String pattern = "abcd";
        String colName = "col1";
        String[] directives = new String[]{String.format("format-datetime :%s" +
                " '%s'", colName, pattern)};
        Row row1 = new Row();
