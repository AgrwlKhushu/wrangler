...
public class ValidateStandardTest {
    ...
    private static Map<String, Standard> getSpecsInArchive()
        ...
        CodeSource src =
                ValidateStandard.class.getProtectionDomain().getCodeSource();
        if (src != null) {
            File schemasRoot =
                    Paths.get(src.getLocation().getPath(),
                            ValidateStandard.SCHEMAS_RESOURCE_PATH).toFile();

            if (!schemasRoot.isDirectory()) {
                throw new IOException(
                        String.format("Schemas root %s was not a directory",
                                schemasRoot.getPath()));
            }
    ...
    private static String calcHash(InputStream is) throws IOException,
    ...
    private static InputStream readResource(String name) throws IOException {
        ...
        if (resourceStream == null) {
            throw new IOException(String.format("Can't read/find resource %s"
                    , name));
        }
    ...
    public void testValidation() throws Exception {
        JsonObject badJson =
                new Gson()
                        .fromJson("{\"resourceType\": \"Patient\", " +
                                "\"active\": \"meow\"}", JsonObject.class);
        JsonObject goodJson =
                new Gson()
                        .fromJson(
                                "{\"resourceType\": \"Patient\", \"active\": " +
                                        "true, \"gender\": \"female\"}",
                                JsonObject.class);
    ...
    /**
     * This test verifies that the manifest in the resources matches up with
     * both the actual schemas in the resources as
    ...
    public void verifyManifest() throws Exception {
        ...
        assertEquals(
                "Manifest contains different number of specs than there are " +
                        "in the artifact",
                declaredSpecs.size(),
                actualSpecs.size());
        ...
        for (String spec : declaredSpecs.keySet()) {
            assertTrue(
                    String.format("Manifest had spec %s but the artifact did " +
                            "not", spec),
            ...
            assertTrue(
                    String.format(
                            "Standard %s does not have a handler/factory " +
                                    "registered in %s",
