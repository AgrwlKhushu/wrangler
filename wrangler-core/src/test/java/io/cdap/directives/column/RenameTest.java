...
public class RenameTest {
    ...
    public void testRenameOfExistingColumns() throws Exception {
        ...
        List<Row> rows = Collections.singletonList(
                new Row("C1", "A").add("C2", "B").add("C3", "C").add("C4", "D"
                ).add("C5", "E")
        );
    ...
    public void testRenameNonExistingCols() throws Exception {
        ...
        // here create new row and compare individual row since row is not
        // immutable during execution
        Assert.assertEquals(4, result.size());
        Assert.assertEquals(new Row("body", "A").add("body_1", "B").add(
                "body_2", "C"), result.get(0));
        Assert.assertEquals(new Row("body", "D").add("body_1", "E").add(
                "body_2", "F").add("BODY_3", "G"), result.get(1));
        Assert.assertEquals(new Row("body", "H").add("body_1", "I").add(
                "body_2", "J"), result.get(2));
        Assert.assertEquals(new Row("body", "K").add("body_1", "L").add(
                "body_2", "M").add("BODY_3", "N"), result.get(3));
    }
