...
public class WriteAsJsonMapTest {
    ...
    public void testWriteToJson() throws Exception {
        ...
        Map<String, String> map =
                GSON.fromJson((String) rows.get(0).getValue("test"),
                        stringStringMapType);
        Assert.assertEquals(ImmutableMap.of("string", "this is string", "int"
                , "1"), map);

        JsonObject jsonObject =
                new JsonParser().parse((String) rows.get(1).getValue("test")).getAsJsonObject();
