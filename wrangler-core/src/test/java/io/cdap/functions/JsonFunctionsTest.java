...
public class JsonFunctionsTest {
    ...
    public void testJsonSelect() throws Exception {
        ...
        Assert.assertEquals(new JsonParser().parse(JSON_SELECTION_EG1),
                rows.get(0).getValue("mayo"));
    ...
    public void testBasicJson() throws Exception {
        String[] directives = new String[]{
                "set-column baddata if(json:IsValid(malformed)) { json:Parse" +
                        "(malformed) } else { 'Invalid Json'}",
        ...
        List<Row> rows = Arrays.asList(
                new Row("malformed", JsonTestData.MALFORMED_BASIC_JSON).add(
                        "basic", JsonTestData.BASIC)
        );
        rows = TestingRig.execute(directives, rows);
        Assert.assertTrue(rows.size() == 1);
