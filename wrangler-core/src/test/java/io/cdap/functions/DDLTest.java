...
public class DDLTest {
    ...
    public void testGetRecursiveRecord() {
        ...
        Schema inner1 = Schema.recordOf("inner1",
                Schema.Field.of("rec2",
                        Schema.arrayOf(Schema.nullableOf(inner2))),
        ...
        Schema nestedSchema = Schema.recordOf(
                ...
                Schema.Field.of("m",
                        Schema.mapOf(Schema.nullableOf(Schema.of(Schema.Type.STRING)),
        ...
        Assert.assertEquals(Schema.of(Schema.Type.BOOLEAN),
                DDL.select(nestedSchema, "z"));
        Assert.assertTrue((boolean) DDL.select(nestedRecord, "z"));

        Assert.assertEquals(Schema.arrayOf(Schema.of(Schema.Type.STRING)),
                DDL.select(nestedSchema, "a"));

        Assert.assertEquals(Schema.mapOf(Schema.nullableOf(Schema.of(Schema.Type.STRING)),
                        Schema.nullableOf(inner2)),
                DDL.select(nestedSchema, "m"));
        Assert.assertEquals(nestedRecord.<Map<String, StructuredRecord>>get(
                "m"), DDL.select(nestedRecord, "m"));
        Assert.assertEquals(Schema.nullableOf(inner2),
                DDL.select(nestedSchema, "m[rec2]"));

        Assert.assertEquals(Schema.of(Schema.Type.STRING),
                DDL.select(nestedSchema, "a[0]"));
        ...
        Assert.assertEquals(Schema.of(Schema.Type.STRING),
                DDL.select(nestedSchema, "rec1.s"));
        Assert.assertEquals("str1", DDL.select(nestedRecord, "rec1.s"));

        Assert.assertEquals(Schema.of(Schema.Type.LONG),
                DDL.select(nestedSchema, "rec1.l"));
        Assert.assertEquals(3L,
                DDL.<Long>select(nestedRecord, "rec1.l").longValue());

        Assert.assertEquals(Schema.arrayOf(Schema.nullableOf(inner2)),
                DDL.select(nestedSchema, "rec1.rec2"));

        Assert.assertEquals(Schema.mapOf(Schema.of(Schema.Type.STRING),
                        Schema.of(Schema.Type.INT)),
                DDL.select(nestedSchema, "rec1.m"));
        Assert.assertEquals(Schema.of(Schema.Type.INT),
                DDL.select(nestedSchema, "rec1.m[a]"));
        Assert.assertEquals(1,
                DDL.<Integer>select(nestedRecord, "rec1.m[a]").intValue());
        Assert.assertEquals(2,
                DDL.<Integer>select(nestedRecord, "rec1.m[b]").intValue());

        Assert.assertEquals(Schema.nullableOf(inner2),
                DDL.select(nestedSchema, "rec1.rec2[0]"));
        ...
        Assert.assertEquals(Schema.of(Schema.Type.STRING),
                DDL.select(nestedSchema, "rec1.rec2[1].x"));
        Assert.assertEquals(Schema.of(Schema.Type.STRING),
                DDL.select(nestedSchema, "m[rec2].x"));
        Assert.assertEquals("str2", DDL.select(nestedRecord, "rec1.rec2[1].x"));
        Assert.assertEquals("str2", DDL.select(nestedRecord, "m[rec2].x"));

        Assert.assertEquals(Schema.of(Schema.Type.INT),
                DDL.select(nestedSchema, "rec1.rec2[1].y"));
        Assert.assertEquals(Schema.of(Schema.Type.INT),
                DDL.select(nestedSchema, "m[rec2].y"));
        Assert.assertEquals(5, DDL.<Integer>select(nestedRecord, "rec1" +
                ".rec2[1].y").intValue());
        Assert.assertEquals(5,
                DDL.<Integer>select(nestedRecord, "m[rec2].y").intValue());

        Assert.assertEquals(Schema.arrayOf(Schema.of(Schema.Type.INT)),
                DDL.select(nestedSchema, "rec1.rec2[1].z"));
        Assert.assertEquals(Schema.arrayOf(Schema.of(Schema.Type.INT)),
                DDL.select(nestedSchema, "m[rec2].z"));

        Assert.assertEquals(Schema.of(Schema.Type.INT),
                DDL.select(nestedSchema, "rec1.rec2[1].z[0]"));
        Assert.assertEquals(Schema.of(Schema.Type.INT),
                DDL.select(nestedSchema, "m[rec2].z[0]"));
        Assert.assertEquals(0, DDL.<Integer>select(nestedRecord, "rec1" +
                ".rec2[1].z[0]").intValue());
        Assert.assertEquals(1, DDL.<Integer>select(nestedRecord, "rec1" +
                ".rec2[1].z[1]").intValue());
        Assert.assertEquals(2, DDL.<Integer>select(nestedRecord, "rec1" +
                ".rec2[1].z[2]").intValue());
        Assert.assertEquals(3, DDL.<Integer>select(nestedRecord, "rec1" +
                ".rec2[1].z[3]").intValue());
        Assert.assertEquals(0, DDL.<Integer>select(nestedRecord, "m[rec2]" +
                ".z[0]").intValue());
        Assert.assertEquals(1, DDL.<Integer>select(nestedRecord, "m[rec2]" +
                ".z[1]").intValue());
        Assert.assertEquals(2, DDL.<Integer>select(nestedRecord, "m[rec2]" +
                ".z[2]").intValue());
        Assert.assertEquals(3, DDL.<Integer>select(nestedRecord, "m[rec2]" +
                ".z[3]").intValue());
        ...
        Schema inner1New = Schema.recordOf("inner1",
                Schema.Field.of("rec2",
                        Schema.arrayOf(Schema.nullableOf(inner2New))),
                Schema.Field.of("s", Schema.of(Schema.Type.STRING)));
