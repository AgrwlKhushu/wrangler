...
public class ExpressionTest {
    ...
    public void testRowNoOfColumns() throws Exception {
        ...
        List<Row> rows = Arrays.asList(new Row("body", "1098,Root,Joltie," +
                "01/26/1956,root@jolite.io,32,11.79," +
    ...
    public void testApplyExpr() throws Exception {
        String[] directives = new String[]{
                "parse-as-csv __col ,",
                "drop __col",
                "set columns id,first,last,dob,email,age,hrlywage,address," +
                ...
                "set column hrlywage var x; x = math:ceil(toFloat(hrlywage));" +
                        " x + 1",
        };

        // Run through the wrangling steps.
        List<Row> rows = Arrays.asList(new Row("__col", "1098,Root,Joltie," +
                "01/26/1956,root@jolite.io,32,11.79," +
                "150 Mars Ave,Palo Alto,CA,USA,32826"));
    ...
    public void testNegativeConditionApply() throws Exception {
        String[] directives = new String[]{
                ...
                "set columns id,first,last,dob,email,age,hrlywage,address," +
                        "city,state,country,zip",
                "set column email string:reverse(email1)" // email1 not
                // defined in the record.
        };

        List<Row> rows = Arrays.asList(new Row("__col", "1098,Root,Joltie," +
                "01/26/1956,root@jolite.io," +
    ...
    public void testDateFunctions() throws Exception {
        ...
        ZonedDateTime date = ZonedDateTime.of(2017, 2, 2, 21, 6, 44, 0,
                ZoneId.ofOffset("UTC", ZoneOffset.UTC));
        ZonedDateTime other = ZonedDateTime.of(2017, 2, 3, 21, 6, 44, 0,
                ZoneId.ofOffset("UTC", ZoneOffset.UTC));
        Assert.assertEquals(date, rows.get(0).getValue("date"));
        Assert.assertEquals(other, rows.get(0).getValue("other"));
        Assert.assertEquals(1486069604000L, rows.get(0).getValue(
                "unixtimestamp"));
    ...
    public void testInvalidDateFunction() throws Exception {
        ...
        //2017-02-02T21:06:44Z
        List<Row> rows = Arrays.asList(
                new Row("date", null).add("seconds", 86401).add("other",
                        "2017-02-03T21:06:44Z")
        );
    ...
    public void testGeoFence() throws Exception {
        ...
        String geoJsonFence = "{\"type\":\"FeatureCollection\"," +
                "\"features\":[{\"type\":\"Feature\",\"properties\":{}," +
                "\"geometry\":{\"type\":\"Polygon\",\"coordinates\":[[[-122" +
                ".05870628356934,37.37943348292772]," +
                "[-122.05724716186525,37.374727268782294],[-122" +
                ".04634666442871,37.37493189292912]," +
                "[-122.04608917236328,37.38175237839049],[-122" +
                ".05870628356934,37.37943348292772]]]}}]}";
