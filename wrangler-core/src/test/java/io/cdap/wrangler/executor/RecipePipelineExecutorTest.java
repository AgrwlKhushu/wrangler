...
public class RecipePipelineExecutorTest {
    ...
    public void testPipeline() throws Exception {
        ...
        StructuredRecord record =
                (StructuredRecord) pipeline.execute(Collections.singletonList(row), schema).get(0);
    ...
    public void testPipelineWithMoreSimpleTypes() throws Exception {
        ...
        RecipePipeline pipeline = TestingRig.execute(commands);
        Row row = new Row("__col", "Larry,Perez,lperezqt@umn.edu,1481666448," +
                "186.66");
        StructuredRecord record =
                (StructuredRecord) pipeline.execute(Collections.singletonList(row), schema).get(0);
        ...
        Assert.assertEquals(1481666448L,
                record.<Long>get("timestamp").longValue());
