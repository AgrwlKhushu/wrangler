...
public class ProtobufDecoderUsingDescriptorTest {
    ...
    public void testBasicConversion() throws Exception {
        ...
        try {
            ...
            Decoder<Row> decoder = new ProtobufDecoderUsingDescriptor(bytes,
                    "AddressBook");
            Assert.assertNotNull(decoder);
            List<Row> rows = decoder.decode(addressBook);
            Assert.assertNotNull(rows);
            Assert.assertEquals("Joltie Root", rows.get(0).getValue(
                    "people_name"));
            Assert.assertEquals(1234, rows.get(0).getValue("people_id"));
            Assert.assertEquals("joltie.root@example.com",
                    rows.get(0).getValue("people_email"));
            Assert.assertEquals("555-4321", rows.get(0).getValue(
                    "people_phones_number"));
            Assert.assertEquals("HOME", rows.get(0).getValue(
                    "people_phones_type"));
        } finally {
            if (is != null) {
                is.close();
