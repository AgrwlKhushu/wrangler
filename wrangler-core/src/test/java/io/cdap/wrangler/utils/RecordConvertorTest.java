...
public class RecordConvertorTest {
    ...
    public void testComplexNestedStructureConversion() throws Exception {
        ...
        List<StructuredRecord> outputs = convertor.toStructureRecord(rows,
                schema);

        Assert.assertEquals(1, outputs.size());
        Assert.assertEquals(6,
                ((List) outputs.get(0).get("body_numbers")).size());
    }
    ...
    public void testNullableEmptyArray() throws Exception {
        Schema schema = Schema.recordOf("test", Schema.Field.of("test_id",
                        Schema.of(Schema.Type.STRING)),
                Schema.Field.of("values",
                        Schema.nullableOf(Schema.arrayOf(Schema.nullableOf(Schema.of(Schema.Type.INT))))));

        String[] directives = new String[]{};

        List<Row> rows = Arrays.asList(
                new Row().add("test_id", "a").add("values",
                        ImmutableList.of(1)),
                new Row().add("test_id", "b").add("values", null),
                new Row().add("test_id", "c"),
                new Row().add("test_id", "d").add("values", ImmutableList.of()),
                new Row().add("test_id", "e").add("values",
    ...
    public void testTypeConversions() throws Exception {
        ...
        List<Row> rows = Collections.singletonList(
                new Row("body", "{\"DeviceID\":\"xyz-abc\",\"SeqNo\":1000," +
                        "\"TimeStamp\":123456,\"LastContact\":123456," +
                        "\"IMEI\":\"345rft567hy65\"," +
                        "\"MSISDN\":\"+19999999999\"," +
                        "\"AuthToken\":\"erdfgg34gtded\",\"Position\":" +
                        "{\"Lat\":\"20.22\",\"Lon\":\"-130.45\"," +
                        "\"Accuracy\":16,\"Compass\":108.22," +
                        "\"TimeStamp\":123456}," +
                        "\"Battery\":50,\"Alert\":{\"Id\":26," +
                        "\"Type\":\"SOS\",\"TimeStamp\":123456}," +
                        "\"Steps\":100,\"Calories\":15}")
                        .add("i2l", 2)
                        .add("sh2l", (short) 1)
                        .add("s2l", "2")
                        .add("i2f", 1)
        ...
        Assert.assertEquals(123456L, results.get(0).<Long>get("body_TimeStamp"
        ).longValue());
    ...
    public void testRowWithLogicalType() throws Exception {
        ...
        testRow.add("timestamp", ZonedDateTime.of(2018, 11, 11, 11, 11, 11, 0
                , ZoneId.of("UTC")));
        ...
        StructuredRecord expected = StructuredRecord.builder(schema)
                ...
                .setTime("time", null)
                .setTimestamp("timestamp", ZonedDateTime.of(2018, 11, 11, 11,
                        11, 11, 0, ZoneId.of("UTC"))).build();
    ...
    public void testNestedRecordConversion() throws Exception {
        ...
        List<StructuredRecord> results = pipeline.execute(records,
                expectedSchema);
    ...
    public void testDateTimeConversion() throws RecordConvertorException {
        ...
        Schema schema = Schema.recordOf("test",
                Schema.Field.of(fieldName,
                        Schema.nullableOf(Schema.of(LogicalType.DATETIME))));
        Row row = new Row();
        row.add(fieldName, value);
        StructuredRecord structuredRecord =
                new RecordConvertor().decodeRecord(row, schema);
