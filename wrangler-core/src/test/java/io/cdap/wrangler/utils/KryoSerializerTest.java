...
public class KryoSerializerTest {
    ...
    public void testJsonTypes() throws Exception {
        ...
        for (String test : TESTS) {
            ...
            byte[] serializedRows =
                    new KryoSerializer().fromRemoteDirectiveResponse(
                            new RemoteDirectiveResponse(expectedRows, null));
            List<Row> gotRows =
                    new KryoSerializer().toRemoteDirectiveResponse(serializedRows).getRows();
    ...
    public void testWithSchema() throws Exception {
        ...
        SchemaConverter converter = new SchemaConverter();
        Schema expectedSchema = converter.toSchema("myrecord",
                expectedRows.get(0));

        byte[] serializedRows =
                new KryoSerializer().fromRemoteDirectiveResponse(
                        new RemoteDirectiveResponse(expectedRows,
                                expectedSchema));
        RemoteDirectiveResponse response =
                new KryoSerializer().toRemoteDirectiveResponse(
                        serializedRows);

        Assert.assertArrayEquals(expectedRows.toArray(),
                response.getRows().toArray());
