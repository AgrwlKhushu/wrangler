    ...
    public void testStructuredRecordToRow() {
        ...
        StructuredRecord memberRecord =
                StructuredRecord.builder(memberSchema)
                        .set("f1", 1).set("f2", "aaa").set("f3", 1L).set("f4"
                                , 0d)
                        .set("f5",
                                ByteBuffer.wrap("test".getBytes(Charsets.UTF_8)))
                        .set("f6", true).set("f7", 0f).setDate("f8",
                                LocalDate.now()).setTimestamp("f9",
                                ZonedDateTime.now())
                        .setTimestamp("f10", ZonedDateTime.now()).setTime(
                                "f11", LocalTime.now()).setTime("f12",
                                LocalTime.now())
                        .set("f13",
                                ByteBuffer.wrap(new BigDecimal(new BigInteger("111"), 2).unscaledValue().toByteArray()))
                        .setDateTime("f14", LocalDateTime.now()).set("f16",
                                Arrays.asList("A", "B", "C"))
                        .build();
        ...
        StructuredRecord record = StructuredRecord.builder(schema)
                ...
                .set("f13", ByteBuffer.wrap(new BigDecimal(new BigInteger(
                        "111"), 2).unscaledValue().toByteArray()))
        ...
        Assert.assertArrayEquals("test".getBytes(Charsets.UTF_8),
                (byte[]) row.getValue("f5"));
        // set it to byte buffer to compare all values
        row.addOrSet("f5", ByteBuffer.wrap((byte[]) row.getValue("f5")));
        ((Row) row.getValue("f15"))
                .addOrSet("f5", ByteBuffer.wrap((byte[]) ((Row) row.getValue(
                        "f15")).getValue("f5")));
