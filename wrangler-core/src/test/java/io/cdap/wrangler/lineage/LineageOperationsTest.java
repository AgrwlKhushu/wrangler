...
public class LineageOperationsTest {
    ...
    private static Schema outputSchema = Schema.recordOf("output",
            Arrays.asList(
    ...
    public void testDropLineage() throws Exception {
        List<FieldOperation> lineage4 = lineage(inputSchema, outputSchema,
                ImmutableList.of("drop a,b,c,d;",
                        "set-column :o exp:{h}"));
    ...
    public void testCombination() throws Exception {
        ...
        List<String> recipes = ImmutableList.of(
                // parse body to body, body_1, and body_2, drop body and
                // rename other two columns, body_1 and body_2 are actual
        ...
        List<FieldOperation> operations = lineage(inputSchema, outputSchema,
                recipes);
        ...
        // 2 identity transforms, order of identity transforms might not
        // follow the input schema
        Assert.assertEquals(11, operations.size());

        // won't compare directly for the FieldTransformOperation since the
        // output fields are converted from Set, so
        // order might not preserve
        compareFieldOperation(operations, 0, "Parsed column 'body' as CSV " +
                        "with delimiter ','",
        ...
        compareFieldOperation(operations, 3, "Renamed column 'body_2' to " +
                        "'name'",
        ...
        compareFieldOperation(operations, 5, "Mapped result of expression 'a " +
                        "+ b + c ' to column 'sum'",
                ImmutableSet.of("a", "b", "c"), Collections.singleton("sum"));
        compareFieldOperation(operations, 6, "Mapped result of expression 'a " +
                        "+ b + c ' to column 'sum'",
                Collections.singleton("a"), Collections.singleton("a"));
        compareFieldOperation(operations, 7, "Mapped result of expression 'a " +
                        "+ b + c ' to column 'sum'",
                Collections.singleton("b"), Collections.singleton("b"));
        compareFieldOperation(operations, 8, "Mapped result of expression 'a " +
                        "+ b + c ' to column 'sum'",
                Collections.singleton("c"), Collections.singleton("c"));

        // rest two are identity transform, but the order we cannot
        // gaurantee, just make sure input and output fields
        // are identical
        FieldTransformOperation actual =
                (FieldTransformOperation) operations.get(9);
    ...
    private void compareFieldOperation(List<FieldOperation> operations,
                                       int index, String description,
                                       Set<String> expectedInput,
                                       Set<String> expectedOutput) {
        FieldTransformOperation actual =
                (FieldTransformOperation) operations.get(index);
        Assert.assertEquals("operation_" + index, actual.getName());
        Assert.assertEquals(description, actual.getDescription());
        Assert.assertEquals(expectedInput,
                new HashSet<>(actual.getInputFields()));
        Assert.assertEquals(expectedOutput,
                new HashSet<>(actual.getOutputFields()));
    }

    private List<FieldOperation> lineage(Schema inputSchema,
                                         Schema outputSchema,
    ...
    private RecipeParser getRecipeParser(List<String> directives) throws DirectiveParseException {
        CompositeDirectiveRegistry registry =
                new CompositeDirectiveRegistry(SystemDirectiveRegistry.INSTANCE);
        return new GrammarBasedParser("default",
