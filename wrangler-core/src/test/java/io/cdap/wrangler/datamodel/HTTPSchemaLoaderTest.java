...
public class HTTPSchemaLoaderTest {
    private static final String DATA_MODEL_NAME = "google.com.datamodels" +
            ".TEST_DATA_MODEL";
    ...
    public void testLoad_dataModels_successfulLoad() throws Exception {
        String base = String.format("http://localhost:%d/test-bucket",
    ...
    public void testLoad_dataModels_manifestNotFound() throws Exception {
        String base = String.format("http://localhost:%d/errors",
                httpService.getBindAddress().getPort());
        HTTPSchemaLoader client = new HTTPSchemaLoader(base,
    ...
    public void testLoad_dataModels_emptyManifest() throws Exception {
        String base = String.format("http://localhost:%d/errors",
                httpService.getBindAddress().getPort());
        HTTPSchemaLoader client = new HTTPSchemaLoader(base, "empty_manifest" +
                ".json");
        SetValuedMap<String, Schema> glossary = client.load();
    ...
    public void testLoad_dataModels_emptyStandards() throws Exception {
        String base = String.format("http://localhost:%d/errors",
                httpService.getBindAddress().getPort());
        HTTPSchemaLoader client = new HTTPSchemaLoader(base,
                "manifest_standard_mismatch.json");
        SetValuedMap<String, Schema> glossary = client.load();
        Assert.assertTrue(glossary.isEmpty());
    }
    ...
    public void testLoad_dataModels_parserError() throws Exception {
        String base = String.format("http://localhost:%d/errors",
                httpService.getBindAddress().getPort());
        HTTPSchemaLoader client = new HTTPSchemaLoader(base,
                "manifest_invalid_data_model.json");
        SetValuedMap<String, Schema> glossary = client.load();
        Assert.assertTrue(glossary.isEmpty());
    }
    ...
    public static class ServiceHandler implements HttpHandler {
        private static final String MANIFEST_JSON = ""
                ...
                + "      \"format\": \"avsc\",\n"
                + "      \"hash\": " +
                "\"d8c119c43873ba2571c72ecccde8bb8c7b237746b227daf3ba78caf85feaca57\"\n"
                + "    }\n"
                + "  }\n"
                + "}";
        ...
        public void errors(HttpRequest request, HttpResponder responder,
                           @PathParam("resource") String resource) {
