...
public class ELTest {
    ...
    public void testArrays() throws Exception {
        EL el = EL.compile("runtime['map'] > token['ABC.EDFG']['input'] " +
                "&& math:max(toDouble(runtime['map']), toDouble(token['ABC" +
                ".EDFG']['input'])) > 9");
    ...
    public void testDecimalTransform() throws Exception {
        ...
        for (String directive : directives) {
            ...
            switch (directive) {
                ...
                case "scale":
                    ...
                    break;
                case "unscaled":
                    Assert.assertEquals(bd.unscaledValue(),
                            execute.getObject());
                    break;
                case "precision":
                ...
                case "negate":
                    ...
                    break;
                case "strip_zero":
                    Assert.assertEquals(bd.stripTrailingZeros(),
                            execute.getObject());
                    break;
                case "sign":
        ...
        for (String directive : directives2) {
            ...
            ELResult execute = el.execute(new ELContext().add("a", bd).add("b"
                    , n));
            ...
            switch (directive) {
                case "pow":
                    ...
                    break;
                case "decimal_left":
                    Assert.assertEquals(new BigDecimal("1234567.89123456789")
                            , execute.getObject());
                    break;
                case "decimal_right":
                    Assert.assertEquals(new BigDecimal("12345678912.3456789")
                            , execute.getObject());
                    break;
                case "add":
                    Assert.assertEquals(bd.add(BigDecimal.valueOf(n)),
                            execute.getObject());
                    break;
                case "subtract":
                    Assert.assertEquals(bd.subtract(BigDecimal.valueOf(n)),
                            execute.getObject());
                    break;
                case "multiply":
                    Assert.assertEquals(bd.multiply(BigDecimal.valueOf(n)),
                            execute.getObject());
                    break;
                case "divideq":
                    Assert.assertEquals(bd.divide(BigDecimal.valueOf(n),
                            BigDecimal.ROUND_HALF_EVEN), execute.getObject());
                    break;
                case "divider":
                    Assert.assertEquals(bd.remainder(BigDecimal.valueOf(n)),
                            execute.getObject());
                    break;
            }
        }
    ...
    public void testArithmeticOperations() throws Exception {
        ...
        ELResult execute = el.execute(new ELContext().add("a", bd1).add("b",
                bd2));
