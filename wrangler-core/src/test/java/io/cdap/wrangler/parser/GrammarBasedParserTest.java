...
public class GrammarBasedParserTest {
    ...
    public void testLoadableDirectives() throws Exception {
        String[] recipe = new String[]{
                ...
                "#pragma load-directives test-change,text-exchange, test1," +
                        "test2,test3,test4;"
        };

        Compiler compiler = new RecipeCompiler();
        CompileStatus status =
                compiler.compile(new MigrateToV2(recipe).migrate());
        Assert.assertEquals(7,
                status.getSymbols().getLoadableDirectives().size());
    }
    ...
    public void testByteSizeAndTimeDurationParsing() throws Exception {
        ...
        Assert.assertEquals(10 * 1024, byteSize1.getBytes()); // 10 KB =
        // 10240 bytes
        Assert.assertEquals(1 * 1024 * 1024, byteSize2.getBytes()); // 1 MB =
        // 1048576 bytes
        Assert.assertEquals(1.5 * 1024 * 1024 * 1024, byteSize3.getBytes(),
                0.001); // 1.5 GB = 1610612736 bytes

        // TimeDuration parsing assertions
        Assert.assertEquals(5, time1.getMilliseconds()); // 5 ms
        Assert.assertEquals(2100, time2.getMilliseconds()); // 2.1 seconds =
        // 2100 ms
        Assert.assertEquals(180000, time3.getMilliseconds()); // 3 min =
        // 180000 ms
    }
