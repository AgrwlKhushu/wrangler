...
public class AggregateStatsTest {
    ...
    public void testAggregateStatsCalculation() throws Exception {
        List<Row> rows = Arrays.asList(
                new Row().add("data_transfer_size", "10kb").add(
                        "response_time", "2s"),
                new Row().add("data_transfer_size", "1.5MB").add(
                        "response_time", "500ms")
        );
        ...
        // Calculation
        long totalBytes = 10 * 1024 + (long) (1.5 * 1024 * 1024); // 10kb + 1
        // .5MB
        ...
        Assert.assertEquals(expectedTotalSizeMB,
                ((Number) output.getValue("total_size_mb")).doubleValue(),
                0.001);
        Assert.assertEquals(expectedTotalTimeSeconds,
                ((Number) output.getValue("total_time_sec")).doubleValue(),
                0.001);
    }
}
