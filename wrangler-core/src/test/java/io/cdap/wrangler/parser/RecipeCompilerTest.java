...
public class RecipeCompilerTest {
    ...
    /**
     * Test that a complex multi-line recipe compiles successfully and
        ...
        try {
            CompileStatus status = compiler.compile(
                    "parse-as-csv :body ' ' true;\n" +
                            ...
                            "parse-as-simple-date :col 'yyyy-mm-dd' :col " +
                            "'test' :col2,:col4,:col9 10 exp:{test < 10};\n"
            );
    ...
    /**
     * Test recipe compilation with embedded macros to verify macro skipping
     * works.
     */
    @Test
    ...
    public void testMissingPragmaHash() throws Exception {
        String[] recipe = {"pragma load-directives test1,test2,test3,test4," +
                "test5;"};
        TestingRig.compileFailure(recipe);
    }
    ...
    public void testWithIfStatement() throws Exception {
        String[] recipe = {
                ...
                "${macro_1}",
                "if ((test > 10) && (window < 20)) { parse-as-csv :body ',' " +
                        "true; if (window > 10) { send-to-error exp:{test > " +
                        "10}; } }"
        };
        TestingRig.compileSuccess(recipe);
    }
    ...
    public void testError() throws Exception {
        ...
        CompileStatus compile = TestingRig.compile(recipe);
        Assert.assertTrue(compile.isSuccess()); // Note: If you expect
        // failure, use compileFailure instead.
    }

    /**
     * Validate that the pragma #load-directives is correctly parsed and
    ...
    public void testRecipePragmaWithCompiler() throws Exception {
        ...
        CompileStatus compile = TestingRig.compile(recipe);
        Set<String> loadableDirectives =
                compile.getSymbols().getLoadableDirectives();
