...
/**
 * A directive for mapping a column to a field within a data model. Mapping
 * entails
 * changing the type of the column to match a target type and changing the
 * name of
...
@Description("Maps a column to target data model field so that matches the " +
...
public class DataModelMapColumn implements Directive, Lineage {
    ...
    private static Map<String, AvroSchemaGlossary> glossaryCache =
            new HashMap<>();
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (!glossaryCache.containsKey(dataModelUrl)) {
            AvroSchemaGlossary glossary =
                    new AvroSchemaGlossary(new HTTPSchemaLoader(dataModelUrl,
                            "manifest.json"));
            if (!glossary.configure()) {
                throw new DirectiveParseException(NAME, String.format("Unable" +
                        " to load data models from %s.", dataModelUrl));
            }
        ...
        long revision =
                ((Numeric) args.value(DATA_MODEL_REVISION)).value().longValue();
        Schema dataModel = glossaryCache.get(dataModelUrl).get(dataModelName,
                revision);
        if (dataModel == null) {
            throw new DirectiveParseException(NAME, String
                    .format("Unable to find data model %s revision %d.",
        ...
        if (modelField == null) {
            throw new DirectiveParseException(NAME, String
                    .format("Model %s does not exist in data model %s.",
                            modelName, dataModelName));
        }
        Schema subSchema = modelField.schema();
        if (subSchema == null) {
            throw new DirectiveParseException(NAME, String.format("Model %s " +
                    "has no schema.", modelField.name()));
        }
        ...
        if (model == null) {
            throw new DirectiveParseException(NAME, String.format("Model %s " +
                    "has no schema.", subSchema.getName()));
        }
        ...
        if (targetField == null) {
            throw new DirectiveParseException(NAME, String
                    .format("Field %s does not exist in model %s.",
                            targetName, model.getName()));
        }
        ...
        if (type == null) {
            throw new DirectiveParseException(NAME,
                    String
                            .format(" Field %s of model %s in data model %s " +
                                            "is missing type information.",
                                    targetField.name(), modelName,
                                    dataModelName));
        }
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context) throws DirectiveExecutionException {
        for (Row row : rows) {
            ColumnConverter.convertType(NAME, row, column,
                    targetFieldTypeName, null, null, RoundingMode.UNNECESSARY);
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Mapped column '%s' to column name '%s' and type " +
                        "'%s'", column, targetFieldName, targetFieldTypeName)
