...
@Description("Formats a column using a date-time format. Use 'parse-as-date` " +
        "beforehand.")
public class FormatDate implements Directive, Lineage {
        ...
        for (Row row : rows) {
            ...
            if (object != null) {
                ...
                if (object instanceof LocalDate) {
                    zonedDateTime =
                            ((LocalDate) object).atStartOfDay(ZoneId.ofOffset("UTC", ZoneOffset.UTC));
                } else if (object instanceof ZonedDateTime) {
                    zonedDateTime = (ZonedDateTime) object;
                } else {
                    throw new DirectiveExecutionException(
                            NAME, String.format("Column '%s' has invalid type" +
                                    " '%s'. Apply 'parse-as-date' directive " +
                                    "first.",
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Formatted date in column '%s' using format '%s'",
                        column, format)
