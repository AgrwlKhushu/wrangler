...
@Description("Calculates the difference in milliseconds between two Date " +
        "objects." +
        "Positive if <column2> earlier. Must use 'parse-as-date' or " +
        "'parse-as-simple-date' first.")
public class DiffDate implements Directive, Lineage {
    public static final String NAME = "diff-date";
    // Timestamp now UTC
    private final ZonedDateTime date = ZonedDateTime.now(ZoneId.ofOffset("UTC"
            , ZoneOffset.UTC));
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context) throws DirectiveExecutionException {
        for (Row row : rows) {
            ...
            if (date1 != null && date2 != null) {
                row.addOrSet(destCol,
                        date1.toInstant().toEpochMilli() - date2.toInstant().toEpochMilli());
            } else {
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Calculated difference between dates in column '%s'" +
                                " and '%s' and store in '%s'"
