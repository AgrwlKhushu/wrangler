...
/**
 * Directive for converting a datetime column to timestamp with the specified
 * zone
 */
...
public class DateTimeToTimeStamp implements Directive, Lineage {
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context) throws ErrorRowException {
        for (Row row : rows) {
            ...
            ZonedDateTime zonedDateTime =
                    ZonedDateTime.of((LocalDateTime) value, zoneId);
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Datetime column '%s' converted to timestamp with " +
                        "zone '%s'", column, zone)
