...
/**
 * A directive for splitting a col into two additional columns based on a
 * start and end.
 */
@Plugin(type = Directive.TYPE)
@Name(IndexSplit.NAME)
@Categories(categories = {"transform"})
@Description("[DEPRECATED] Use the 'split-to-columns' or " +
        "'parse-as-fixed-length' directives instead.")
@Deprecated
        ...
        for (Row row : rows) {
            ...
            if (idx != -1) {
                ...
                throw new DirectiveExecutionException(
                        col + " is not of type string in the row. Please " +
                                "check the wrangle configuration."
                );
            }
            results.add(row);
        }
