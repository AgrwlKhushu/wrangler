 ...
 *
 * <p>
 * Substitution masking is generally used for masking credit card or SSN
 * numbers.
 ...
 *    <li>Use x/X to mask the digit at that position (converted to lowercase
 *    x in the result)</li>
...
public class MaskNumber implements Directive, Lineage {
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Masked numbers in the column '%s' using masking " +
                        "pattern '%s'", column, mask)
    ...
    private String maskNumber(String number, String mask) {
        ...
        for (int i = 0; i < mask.length(); i++) {
            ...
            if (c == '#') {
                // if we have print numbers and the mask index has exceed, we
                // continue further.
