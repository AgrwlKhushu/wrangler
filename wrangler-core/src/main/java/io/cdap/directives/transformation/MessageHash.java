...
@Description("Creates a message digest for the column using algorithm, " +
...
public class MessageHash implements Directive, Lineage {
    ...
    private static final String HASH_ALGORITHM_METRIC_NAME = "wrangler" +
            ".hash-algo.count";
    private static final int HASH_ALGORITHM_COUNT = 1;
    private static final String HASH_ALGORITHM_ENTITY_NAME = "directive-hash" +
            "-algo";
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (!MessageHash.isValid(algorithm.value())) {
            throw new DirectiveParseException(
                    NAME, String.format("Algorithm '%s' specified at line %d " +
                    "is not supported.", algorithm, args.line()));
        }
        try {
            this.digest = MessageDigest.getInstance(algorithm.value());
        } catch (NoSuchAlgorithmException e) {
            throw new DirectiveParseException(
                    NAME, String.format("Unable to find algorithm '%s' " +
                    "specified at line %d.", algorithm, args.line()));
        }
            ...
            if (idx != -1) {
                ...
                if (object == null) {
                    throw new DirectiveExecutionException(
                            NAME, String.format("Column '%s' has null value. " +
                                    "It should be a non-null 'String' or " +
                                    "'byte array'.",
                            column));
                }
                ...
                if (object instanceof String) {
                    message =
                            ((String) object).getBytes(StandardCharsets.UTF_8);
                } else if (object instanceof byte[]) {
                    message = ((byte[]) object);
                } else {
                    throw new DirectiveExecutionException(
                            NAME, String.format("Column '%s' has invalid type" +
                                    " '%s'. It should be of type 'String' or " +
                                    "'byte array'.",
                ...
                if (encode) {
                    ...
                    String hasedHex = String.format("%064x",
                            new java.math.BigInteger(1, hashed));
                ...
                throw new DirectiveExecutionException(NAME, String.format(
                        "Column '%s' does not exist.", column));
            }
        }
        return rows;
    }
    ...
    public List<EntityCountMetric> getCountMetrics() {
        return ImmutableList.of(
                new EntityCountMetric(HASH_ALGORITHM_METRIC_NAME,
                        HASH_ALGORITHM_ENTITY_NAME,
