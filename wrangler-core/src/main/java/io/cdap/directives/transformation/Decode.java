...
/**
 * A directive that decodes a column that was encoded as base-32, base-64, or
 * hex.
 */
...
public class Decode implements Directive, Lineage {
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (!type.equals("BASE64") && !type.equals("BASE32") && !type.equals(
                "HEX")) {
            throw new DirectiveParseException(
                    NAME, String.format("Decoding type '%s' is not supported." +
                    " Supported types are base64, base32 & hex.", type));
        }
        ...
        for (Row row : rows) {
            ...
            if (object instanceof String) {
                ...
                throw new DirectiveExecutionException(
                        NAME, String.format("Column '%s' has invalid type " +
                                "'%s'. It should be a non-null 'String' or " +
                                "'byte array'.",
            ...
            if (method == Method.BASE32) {
                ...
                try {
                    ...
                    throw new DirectiveExecutionException(
                            NAME, String.format("Failed to decode hex value. " +
                            "%s", e.getMessage()), e);
                }
            } else {
                throw new DirectiveExecutionException(
                        NAME, String.format("Specified decoding type '%s' is " +
                        "not supported. Supported types are base64, " +
                        "base32 & hex.", method.toString()));
            }

            String obj = new String(out, StandardCharsets.UTF_8);
            row.addOrSet(String.format("%s_decode_%s", column,
                    method.toString().toLowerCase(Locale.ENGLISH)), obj);
        }
        return rows;
    }
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Decoded column '%s' using method '%s'", column,
                        method.getType())
