 ...
 * <p>
 * The expressions are specified in JEXL format (http://commons.apache
 * .org/proper/commons-jexl/reference/syntax.html)
 * Executor is response for executing only one expression for each
 * {@link Row} record that is
 * being passed. The result of expression either adds a new column or
 * replaces the value of
 * the existing column.
 *
 * <p>
 * Executor step = new ColumnExpression(lineno, directive, column, "if (age >
 * 24 ) { 'adult' } else { 'teen' }");
 * </p>
 */
...
public class ColumnExpression implements Directive, Lineage {
    ...
    public Mutation lineage() {
        Mutation.Builder builder = Mutation.builder()
                .readable("Mapped result of expression '%s' to column '%s'",
                        expression, column);
    ...
    public List<EntityCountMetric> getCountMetrics() {
        EntityCountMetric jexlCategoryMetric =
                getJexlCategoryMetric(el.getScriptParsedText());
        return (jexlCategoryMetric == null) ? null :
                ImmutableList.of(jexlCategoryMetric);
    }
}
