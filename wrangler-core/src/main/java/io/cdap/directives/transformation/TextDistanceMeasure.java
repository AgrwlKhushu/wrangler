...
/**
 * A directive for implementing the directive for measuring the difference
 * between two sequence of characters.
 */
@Plugin(type = Directive.TYPE)
@Name(TextDistanceMeasure.NAME)
@Categories(categories = {"transform"})
@Description("Calculates a text distance measure between two columns " +
        "containing string.")
public class TextDistanceMeasure implements Directive, Lineage {
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context) throws DirectiveExecutionException {
        for (Row row : rows) {
            ...
            row.add(destination, distance.distance((String) object1,
                    (String) object2));
        }
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Compared text in columns '%s' and '%s' and saved " +
                                "it in column '%s'",
