...
@Description("Finds and replaces text in column values using a sed-format " +
        "expression.")
public class FindAndReplace implements Directive, Lineage {
                ...
                if (idx != -1) {
                    ...
                    try {
                        if (v instanceof String) {
                            String value = (String) v; // Safely converts to
                            // String.
                            Unix4jCommandBuilder builder =
                                    Unix4j.echo(value).sed(pattern);
                            if (builder.toExitValue() == 0) {
                                row.setValue(idx, builder.toStringResult());
                            }
                        }
                    } catch (Exception e) {
                        // If there is any issue, we pass it on without any
                        // transformation.
                    }
                }
                results.add(row);
    ...
    public Mutation lineage() {
        Mutation.Builder builder = Mutation.builder()
                .readable("Found and replaced '%s' using expression '%s'",
                        columns, pattern);
