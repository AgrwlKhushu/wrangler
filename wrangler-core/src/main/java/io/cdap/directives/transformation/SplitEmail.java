...
public class SplitEmail implements Directive, Lineage {
        ...
        for (Row row : rows) {
            ...
            if (idx != -1) {
                ...
                if (object instanceof String) {
                    ...
                    int nameIdx = emailAddress.lastIndexOf("<"); // Joltie,
                    // Root <joltie.root@yahoo.com>
                    if (nameIdx == -1) {
                        Pair<String, String> components =
                                extractDomainAndAccount(emailAddress);
                        ...
                        if (endIdx == -1) {
                            ...
                            emailAddress = emailAddress.substring(nameIdx + 1
                                    , endIdx);
                            Pair<String, String> components =
                                    extractDomainAndAccount(emailAddress);
                            row.add(generatedAccountCol, components.getFirst());
                    ...
                    throw new DirectiveExecutionException(
                            NAME, String.format("Column '%s' has invalid type" +
                                    " '%s'. " +
                                    "It should be of type 'String'.", column,
    ...
    public Mutation lineage() {
        return Mutation.builder()
                ...
                .relation(column, Many.of(column, generatedAccountCol,
                        generatedDomainCol))
                .build();
    }
    ...
    private Pair<String, String> extractDomainAndAccount(String emailId) {
        ...
        if (lastidx == -1) {
            ...
            return new Pair<>(emailId.substring(0, lastidx),
                    emailId.substring(lastidx + 1));
        }
    }
}
