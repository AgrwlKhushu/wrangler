...
/**
 * A Wrangle step for filtering rows that match the pattern specified on the
 * column.
 */
...
public class RecordRegexFilter implements Directive, Lineage {
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (matchType.equalsIgnoreCase("if-matched")) {
            ...
            throw new DirectiveParseException(
                    NAME, "Match type specified is not 'if-matched' or " +
                    "'if-not-matched'");
        }
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Filtered column '%s' based on whether the " +
                                "expression '%s' %s ",
    ...
    private boolean matchPattern(String value) {
        boolean matches = pattern.matcher(value).matches(); // pattern
