...
/**
 * A CSV Parser Stage for parsing the {@link Row} provided based on
 * configuration.
 */
...
public class CsvParser implements Directive, Lineage {
    ...
    public UsageDefinition define() {
        UsageDefinition.Builder builder = UsageDefinition.builder("parse-as" +
                "-csv");
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (args.contains("delimiter")) {
            ...
            if (delimiterArg.value().startsWith("\\")) {
                String unescapedStr =
                        StringEscapeUtils.unescapeJava(delimiterArg.value());
                if (unescapedStr == null) {
                    throw new DirectiveParseException(
                            NAME, String.format("Invalid delimiter for CSV " +
                            "Parser '%s'", delimiterArg.value()));
                }
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context)
        ...
        for (Row row : rows) {
            ...
            try {
                ...
                for (CSVRecord csvRecord : csvRecords) {
                    if (!checkedHeader && hasHeader && isHeader(csvRecord)) {
                        for (int i = 0; i < csvRecord.size(); i++) {
                            headers.add(csvRecord.get(i).trim().replaceAll(
                                    "\\s+", "_"));
                        }
                        if (rows.size() > 0) {
                ...
                // When there is error parsing data, the data is written to
                // error.
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Parsed column '%s' as CSV with delimiter '%s'",
                        columnArg.value(), delimiterArg.value())
