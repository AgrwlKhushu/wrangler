...
/**
 * This class is a JSON Parser directive with optional argument specifying
 * the depth
...
public class JsParser implements Directive, Lineage {
     ...
     */
    public static void jsonFlatten(JsonObject root, String field, int depth,
                                   int maxDepth, Row row) {
        if (depth > maxDepth) {
            row.addOrSet(String.format("%s", field), root);
            return;
        }

        Iterator<Map.Entry<String, JsonElement>> elements =
                root.entrySet().iterator();
        while (elements.hasNext()) {
            ...
            if (element instanceof JsonObject) {
                jsonFlatten(element.getAsJsonObject(),
                        String.format("%s_%s", field, key), depth + 1,
                        maxDepth, row);
            } else {
                ...
                try {
                    ...
                    if (value instanceof String) {
                        ...
                        throw new DirectiveExecutionException(
                                NAME, String.format("Column '%s' is of " +
                                        "invalid type '%s'. It should be of " +
                                        "type " +
                                        "'String'" +
                                        " or 'JsonObject' or 'JsonArray'.",
                                column,
                    ...
                    if (element != null) {
                        if (element instanceof JsonObject) {
                            jsonFlatten(element.getAsJsonObject(), column, 1,
                                    depth, row);
                            results.add(row);
                            ...
                            row.add(column,
                                    getValue(element.getAsJsonPrimitive()));
                        }
                    }
