...
public class ParseTimestamp implements Directive, Lineage {
    ...
    private static final Set<TimeUnit> SUPPORTED_TIME_UNITS =
            EnumSet.of(TimeUnit.SECONDS, TimeUnit.MILLISECONDS,
    ...
    private static TimeUnit getTimeUnit(String unitValue) throws DirectiveParseException {
        ...
        try {
            ...
            throw new DirectiveParseException(
                    NAME, String.format("Time unit '%s' is not a supported " +
                            "time unit. Supported time units are %s",
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context)
        ...
        for (Row row : rows) {
            ...
            if (idx != -1) {
                ...
                // If the data in the cell is null or is already of date
                // format, then
                ...
                ZonedDateTime zonedDateTime = getZonedDateTime(longValue,
                        timeUnit, ZoneId.ofOffset("UTC", ZoneOffset.UTC));
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Parsed column '%s' as a timestamp using time unit " +
                        "as '%s'", column, timeUnit)
                .relation(column, column)
                .build();
    }

    private long getLongValue(Object object) throws ErrorRowException {
        String errorMsg = String.format("Invalid type '%s' of column '%s'. " +
                        "Must be of type 'Long' or 'String'.",
        ...
        try {
            ...
            // Exception while casting the object, do not handle it here, so
            // that ErrorRowException is thrown.
            errorMsg = String.format("Invalid value for column '%s'. Must be " +
                    "of type 'Long' or 'String' " +
    ...
    private ZonedDateTime getZonedDateTime(long ts, TimeUnit unit,
        ...
        // create an Instant with time in seconds and fraction which will be
        // stored as nano seconds.
        Instant instant = Instant.ofEpochSecond(tsInSeconds,
                unit.toNanos(fraction));
