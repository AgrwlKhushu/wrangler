...
public class ParseExcel implements Directive, Lineage {
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (args.contains("first-row-as-header")) {
            this.firstRowAsHeader = ((Boolean) args.value("first-row-as" +
                    "-header").value());
        }
    }
                ...
                if (idx != -1) {
                    ...
                    if (object instanceof byte[]) {
                        ...
                        throw new DirectiveExecutionException(
                                NAME, String.format("Column '%s' should be of" +
                                " type 'byte array' or 'ByteBuffer'.", column));
                    }
                    ...
                    if (bytes != null) {
                        ...
                        if (Types.isInteger(sheet)) {
                            excelsheet =
                                    book.getSheetAt(Integer.parseInt(sheet));
                        } else {
                        ...
                        if (excelsheet == null) {
                            throw new DirectiveExecutionException(
                                    NAME, String.format("Failed to extract " +
                                            "sheet '%s' from the excel. " +
                                            "Sheet '%s' does not exist.", sheet,
                                    sheet));
                        }

                        Map<Integer, String> columnNames = new TreeMap<>();
                        Iterator<org.apache.poi.ss.usermodel.Row> it =
                                excelsheet.iterator();
                        int rows = 0;
                        while (it.hasNext()) {
                            ...
                            while (cellIterator.hasNext()) {
                                ...
                                String name =
                                        columnName(cell.getAddress().getColumn());
                                if (firstRowAsHeader && rows > 0) {
                                    String value =
                                            columnNames.get(cell.getAddress().getColumn());
                                    if (value != null) {
                                        name = value;
                                    }
                                }
                            ...
                            for (Pair<String, Object> field :
                                    record.getFields()) {
                                String colName = field.getFirst();
                                // if new row does not contain this column
                                // and this column is not the blob column
                                // that contains
                                // the excel data.
                        ...
                        for (int i = rows - 1; i >= 0; --i) {
                            results.get(rows - i - 1).addOrSetAtIndex(1, "bkd"
                                    , i); // fwd - 0, bkd - 1.
                        }
                    }
                }
