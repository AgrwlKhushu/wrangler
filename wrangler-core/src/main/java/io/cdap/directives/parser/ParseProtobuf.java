...
public class ParseProtobuf implements Directive, Lineage {
    ...
    private static final Logger LOG =
            LoggerFactory.getLogger(ParseProtobuf.class);
    ...
    public List<Row> execute(List<Row> rows, final ExecutorContext context)
        ...
        if (!decoderInitialized) {
            // Retryer callable, that allows this step attempt to connect to
            // schema registry service
            // before giving up.
            Callable<Decoder<Row>> decoderCallable =
                    new Callable<Decoder<Row>>() {
                        @Override
                        public Decoder<Row> call() throws Exception {
                            client = SchemaRegistryClient.getInstance(context);
                            byte[] bytes;
                            if (version != -1) {
                                bytes = client.getSchema(context.getNamespace(),
                                        schemaId, version);
                            } else {
                                bytes = client.getSchema(context.getNamespace(),
                                        schemaId);
                            ...
                            return new ProtobufDecoderUsingDescriptor(bytes,
                                    recordName);
                        }
                    };

            // Retryer that retries when there is connection issue or any
            // request / response
            // issue. It would exponentially back-off till wait time of 10
            // seconds is reached
            // for 5 attempts.
            Retryer<Decoder<Row>> retryer =
                    RetryerBuilder.<Decoder<Row>>newBuilder()
                            .retryIfExceptionOfType(IOException.class)
                            .retryIfExceptionOfType(RestClientException.class)
                            .withWaitStrategy(WaitStrategies.exponentialWait(10,
                                    TimeUnit.SECONDS))
            ...
            try {
                ...
                if (decoder == null) {
                    throw new DirectiveExecutionException(NAME, "Unsupported " +
                            "protobuf decoder type.");
                }
                decoderInitialized = true;
            } catch (ExecutionException | RetryException e) {
                throw new DirectiveExecutionException(
                        NAME, String.format("Unable to retrieve protobuf " +
                                "descriptor from schema registry. %s",
                        e.getMessage()), e);
            }
        }
                ...
                if (idx != -1) {
                    ...
                    if (object instanceof byte[]) {
                        ...
                        throw new ErrorRowException(NAME,
                                "Column " + column + " should be of type " +
                                        "'byte array'", 1);
                    }
                }
            }
        } catch (DecoderException e) {
            throw new ErrorRowException(NAME, "Issue decoding Protobuf record" +
