...
/**
 * A Fixed length Parser Stage for parsing the {@link Row} provided based on
 * configuration.
 */
@Plugin(type = Directive.TYPE)
@Name("parse-as-fixed-length")
@Categories(categories = {"parser"})
@Description("Parses fixed-length records using the specified widths and " +
        "padding-character.")
public final class FixedLengthParser implements Directive, Lineage {
            ...
            if (idx != -1) {
                ...
                if (object == null) {
                    throw new DirectiveExecutionException(
                            ...
                            "It should be a non-null 'String'.", col));
                }

                if (object instanceof String) {
                    String data = (String) object;
                    int length = data.length();
                    // If the recordLength length doesn't match the string
                    // length.
                    if (length < recordLength) {
                        throw new ErrorRowException(
                                NAME, String.format("Column '%s' contains a " +
                                        "value with fewer characters than the" +
                                        " specified length " +
                                        "of row. Expected at least %d " +
                                        "characters but found %s characters.",
                    ...
                    while ((index + recordLength - 1) <= length) {
                        ...
                        for (int width : widths) {
                            String val = data.substring(recPosition - 1,
                                    recPosition + width - 1);
                            if (padding != null) {
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Parsed column '%s' with fixed lengths for columns"
                        , col)
