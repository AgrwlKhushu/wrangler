    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (!"json".equalsIgnoreCase(type) && !"binary".equalsIgnoreCase(type)) {
            throw new DirectiveParseException(
                    NAME, String.format("Invalid encoding type '%s'. The type" +
                    " must be either 'json' or 'binary'.", type));
        }
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context)
        ...
        if (!decoderInitialized) {
            ...
            try {
                ...
                if (decoder == null) {
                    throw new DirectiveExecutionException(NAME, "Avro parsing" +
                            " is supported for 'json' and 'binary' types only" +
                            ".");
                }
                ...
                throw new DirectiveExecutionException(
                        NAME, String.format("Unable to retrieve schema from " +
                        "schema registry. %s", e.getMessage()), e);
            }
        }
                ...
                if (idx != -1) {
                    ...
                    if (object instanceof byte[]) {
                        ...
                        throw new ErrorRowException(
                                NAME, "Column " + column + " should be of " +
                                "type 'String' or 'byte array'.", 1);
                    }
                }
            }
        } catch (DecoderException e) {
            throw new ErrorRowException(NAME, "Issue decoding Avro record. " +
