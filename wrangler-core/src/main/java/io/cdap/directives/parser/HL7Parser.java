...
@Description("Parses <column> for Health Level 7 Version 2 (HL7 V2) messages;" +
        " <depth> indicates at which point " +
...
public class HL7Parser implements Directive, Lineage {
                ...
                if (idx != -1) {
                    ...
                    if (object == null) {
                        throw new DirectiveExecutionException(
                                NAME, String.format("Column '%s' has null " +
                                        "value. It should be a non-null " +
                                        "'String'.",
                                column)
                        );
                    }
                    ...
                    if (object instanceof String) {
                        ...
                        HL7MessageVisitor visitor = new HL7MessageVisitor(row
                                , column + "_hl7", depth);
                        MessageVisitors.visit(message,
                                MessageVisitors.visitPopulatedElements(visitor)).getDelegate();
                    } else {
                        throw new DirectiveExecutionException(
                                NAME, String.format("Column '%s' has invalid " +
                                        "type '%s'. It should be of type " +
                                        "'String'.",
    ...
    private final class HL7MessageVisitor implements MessageVisitor {
        ...
        public boolean end(Composite composite, Location location) {
            segmentObject.add(Integer.toString(location.getField()),
                    compositeObject);
            inComposite = false;
            return true;
        }
        ...
        public boolean visit(Primitive primitive, Location location) {
            if (inComposite) {
                ...
                String fieldComponent = String.format("%d_%d",
                        location.getField(), location.getComponent());
