...
public class ValidateStandard implements Directive {
    ...
    private static final Logger LOG =
            LoggerFactory.getLogger(ValidateStandard.class);
    ...
    static {
        FORMAT_TO_FACTORY.put(JsonConformer.SCHEMA_FORMAT,
                new JsonConformer.Factory());
    ...
    private static Manifest getManifest() throws IOException {
        ...
        if (resourceStream == null) {
            throw new IOException(
                    String.format("Can't read/find resource %s",
                            ValidateStandard.MANIFEST_PATH));
        }

        InputStream manifestStream =
                readResource(ValidateStandard.MANIFEST_PATH);
    ...
    private static InputStream readResource(String name) {
        InputStream resourceStream =
                ValidateStandard.class.getClassLoader().getResourceAsStream(name);
    ...
    public UsageDefinition define() {
        ...
        builder.define(
                ...
                String.format(
                        "[one of: %s]", String.join(", ",
                                standardsManifest.getStandards().keySet())));
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (spec.equals("")) {
            throw new DirectiveParseException("No standard specified to " +
                    "validate against");
        }
        if (standardsManifest == null) {
            throw new DirectiveParseException(
                    "Standards manifest was not loaded. Please check logs for" +
                            " information");
        }
        ...
        schema =
                Paths.get(SCHEMAS_RESOURCE_PATH, String.format("%s.%s", spec,
                                standard.getFormat()))
                        .toString();

        if (!schemaToConformer.containsKey(schema)) {
            if (!FORMAT_TO_FACTORY.containsKey(standard.getFormat())) {
                throw new DirectiveParseException(String.format("No validator" +
                        " for format %s", standard.getFormat()));
            }

            try {
                Conformer<JsonObject> conformer =
                        FORMAT_TO_FACTORY.get(standard.getFormat())
                                .setSchemaStreamSupplier(() -> readResource(schema))
                                .build();
                conformer.initialize();
                schemaToConformer.put(schema, conformer);
            } catch (IOException e) {
                throw new DirectiveParseException(String.format("Unable to " +
                        "read standard schema: %s", e.getMessage()), e);
            }
        }
    }
            ...
            if (!(object instanceof JsonObject)) {
                throw new DirectiveExecutionException(
                        String.format(
                                ...
                                column, JsonObject.class.getName(),
                                object.getClass().getName()));
            }

            Conformer<JsonObject> conformer = schemaToConformer.get(schema);
            if (conformer == null) {
                throw new DirectiveExecutionException("Directive was not " +
                        "initialized for schema " + schema);
            }
