...
public class XmlToJson implements Directive, Lineage {
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (args.contains(ARG_KEEP_STRING) &&
            ...
            this.keepString =
                    Boolean.parseBoolean(args.value(ARG_KEEP_STRING).value().toString());
        }
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context) throws DirectiveExecutionException {
        for (Row row : rows) {
            ...
            if (idx != -1) {
                ...
                if (object == null) {
                    throw new DirectiveExecutionException(NAME, "' : Column " +
                            "'" + col + "' does not exist.");
                }
                ...
                try {
                    if (object instanceof String) {
                        JsonObject element =
                                gson.fromJson(XML.toJSONObject((String) object, this.keepString).toString(),
