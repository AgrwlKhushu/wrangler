...
public class CatalogLookup implements Directive, Lineage {
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (!type.equalsIgnoreCase("ICD-9") && !type.equalsIgnoreCase("ICD-10" +
            ...
            throw new DirectiveParseException(
                    NAME, "Invalid ICD type - should be 9 (ICD-9) or 10 " +
                    "(ICD-10-2016 or ICD-10-2017).");
        } else {
            catalog = new ICDCatalog(type.toLowerCase());
            if (!catalog.configure()) {
                throw new DirectiveParseException(NAME, "Failed to configure " +
                        "ICD StaticCatalog. Check with your administrator");
            }
        }
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Looked up catalog using value in column '%s' and " +
                        "wrote it to column '%s'", column, generatedColumn)
