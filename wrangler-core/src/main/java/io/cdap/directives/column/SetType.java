 ...
 * Accepted types are: int, short, long, double, float, string, boolean and
 * bytes
 * When decimal type is selected, can also specify the scale, precision and
 * rounding mode
 */
@Plugin(type = "directives")
@Name(SetType.NAME)
@Categories(categories = {"column"})
@Description("Converting data type of a column. Optional arguments scale, " +
        "precision and "
...
public final class SetType implements Directive, Lineage {
    ...
    public UsageDefinition define() {
        ...
        builder.define("precision", TokenType.PROPERTIES, "prop:{precision" +
    ...
    public void initialize(Arguments args) throws DirectiveParseException {
        ...
        if (type.equalsIgnoreCase("decimal")) {
            precision = args.contains("precision") ?
                    (Integer) ((HashMap<String, Numeric>) args.
                            value("precision").value()).get("precision").value().intValue() : null;
            if (precision != null && precision < 1) {
                throw new DirectiveParseException("precision cannot be less " +
                        "than 1");
            }
            scale = args.contains("scale") ?
                    ((Numeric) args.value("scale")).value().intValue() : null;
            if (scale == null && precision == null && args.contains("rounding" +
                    "-mode")) {
                throw new DirectiveParseException("'rounding-mode' can only " +
                        "be specified when a 'scale' or 'precision' is set");
            }
            try {
                roundingMode = args.contains("rounding-mode") ?
                        RoundingMode.valueOf(((Text) args.value("rounding" +
                                "-mode")).value()) :
                ...
                throw new DirectiveParseException(String.format(
                        "Specified rounding-mode '%s' is not a valid Java " +
                                "rounding mode",
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context) throws DirectiveExecutionException {
        for (Row row : rows) {
            ColumnConverter.convertType(NAME, row, col, type, scale,
                                ...
                                field -> {
                                    try {
                                        if (field.getName().equals(col)) {
                                            ...
                                            Integer inputSchemaScale =
                                                    scaleAndPrecision.getSecond();
                                            Integer inputSchemaPrecision =
                                                    scaleAndPrecision.getFirst();
                                                ...
                                                if (precision - inputSchemaScale < 1) {
                                                    throw new DirectiveParseException(String.format(
                                                            "Cannot set scale" +
                                                                    ...
                                                                    "precision as '%s' when "
                                                                    + "given " +
                                                                    "precision - scale is less than 1 ", inputSchemaScale,
                                                            precision));
                                                }
                                                outputScale = inputSchemaScale;
                                                ...
                                                if (inputSchemaPrecision - scale < 1) {
                                                    throw new DirectiveParseException(String.format(
                                                            "Cannot set scale" +
                                                                    ...
                                                                    "precision as '%s' when "
                                                                    + "given " +
                                                                    "precision - scale is less than 1 ", scale,
                                                            inputSchemaPrecision));
                                                }
                                                outputScale = scale;
                                                outputPrecision =
                                            ...
                                            return Schema.Field.of(col,
                                                    ColumnConverter.getSchemaForType(type,
                                                            outputScale,
                                                            outputPrecision));
                                        }
                                        return field;
