        ...
        for (Row row : rows) {
            ...
            if (sidx == -1) {
                throw new DirectiveExecutionException(NAME, String.format(
                        "Column '%s' does not exist.", left));
            }

            if (didx == -1) {
                throw new DirectiveExecutionException(NAME, String.format(
                        "Column '%s' does not exist.", right));
            }
    ...
    public Schema getOutputSchema(SchemaResolutionContext context) {
        ...
        return Schema.recordOf(
                ...
                inputSchema.getFields().stream()
                        .map(
                                field -> field.getName().equals(left) ?
                                        Schema.Field.of(right,
                                                field.getSchema()) :
                                        (field.getName().equals(right) ?
                                                Schema.Field.of(left,
                                                        field.getSchema()) :
                                                field)
                        )
