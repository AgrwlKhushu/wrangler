...
public class Copy implements Directive, Lineage {
        ...
        for (Row row : rows) {
            ...
            if (sidx == -1) {
                throw new DirectiveExecutionException(NAME, String.format(
                        "Column '%s' does not exist.", source.value()));
            }
            ...
            if (didx == -1) {
                ...
                // if destination column exists, and force is set to false,
                // then throw exception, else
                // overwrite it.
                if (!force) {
                    throw new DirectiveExecutionException(
                            NAME, String.format("Destination column '%s' " +
                                    "already exists in the row. Use 'force' " +
                                    "option to overwrite the column.",
                            destination.value()));
                }
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Copied value from column '%s' to '%s'",
    ...
    public Schema getOutputSchema(SchemaResolutionContext context) {
        ...
        boolean destinationExists =
                inputSchema.getField(destination.value()) != null;

        for (Schema.Field field : inputSchema.getFields()) {
            if (force && field.getName().equals(destination.value())) {
                outputFields.add(Schema.Field.of(destination.value(),
                        sourceSchema));
            } else {
