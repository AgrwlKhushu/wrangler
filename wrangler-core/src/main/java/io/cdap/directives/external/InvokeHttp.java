...
/**
 * A directives that invokes HTTP endpoint to merge the results back into
 * dataset.
 */
@Plugin(type = Directive.TYPE)
@Name(InvokeHttp.NAME)
@Categories(categories = {"http"})
@Description("Invokes an HTTP endpoint, passing columns as a JSON map " +
        "(potentially slow).")
public class InvokeHttp implements Directive, Lineage {
            ...
            for (String header : parsedHeaders) {
                ...
                if (components.length != 2) {
                    throw new DirectiveParseException(
                            NAME, String.format("Incorrect header '%s' " +
                            "specified. Header should be specified as " +
                            "'key=value' " +
                ...
                if (key.isEmpty()) {
                    throw new DirectiveParseException(
                            NAME, String.format("Key specified for header " +
                            "'%s' cannot be empty.", header));
                }
                if (value.isEmpty()) {
                    throw new DirectiveParseException(
                            NAME, String.format("Value specified for header " +
                            "'%s' cannot be empty.", header));
                }
    ...
    public List<Row> execute(List<Row> rows, ExecutorContext context)
        ...
        for (Row row : rows) {
            ...
            try {
                ...
                // If there are any issues, they will be pushed on the error
                // port.
    ...
    public Mutation lineage() {
        return Mutation.builder()
                .readable("Invoked external service '%s' to enrich row based " +
                        "on columns '%s'", url, columns)
                .all(Many.of(columns), Many.of(columns))
                .build();
    }

    private Map<String, Object> invokeHttp(String url,
