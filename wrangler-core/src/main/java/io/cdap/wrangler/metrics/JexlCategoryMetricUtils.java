...
/**
 * Utility class for the 'JEXL category' metric. This metric counts the
 * number of times a JEXL function category
...
public final class JexlCategoryMetricUtils {
    ...
    public static final String JEXL_CATEGORY_METRIC_NAME = "wrangler" +
            ".jexl-category.count";
    public static final int JEXL_CATEGORY_METRIC_COUNT = 1;
    public static final String JEXL_CATEGORY_ENTITY_TYPE = "jexl-category";
    private static final Map<?, ?> EL_DEFAULT_FUNCTIONS =
            new EL.DefaultFunctions().functions();

    /**
     * This method parses the JEXL function category from the given JEXL
     * script and returns the metric if the category is
     * present in {@link io.cdap.wrangler.expression.EL.DefaultFunctions}
     *
     * @param jexlScript the JEXL script from which the JEXL function
     *                   category will be parsed
     * @return {@link EntityCountMetric} with the metric name and necessary
     * tags representing a JEXL category metric
     */
    @Nullable
    public static EntityCountMetric getJexlCategoryMetric(String jexlScript) {
        ...
        if (EL_DEFAULT_FUNCTIONS.containsKey(category)) {
            return new EntityCountMetric(
                    JEXL_CATEGORY_METRIC_NAME, JEXL_CATEGORY_ENTITY_TYPE,
                    category, JEXL_CATEGORY_METRIC_COUNT);
        }
        return null;
    }
