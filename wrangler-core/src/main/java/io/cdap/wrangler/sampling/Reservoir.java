 ...
 * replacement, and with only one
 * pass through the input iteration whose size is unpredictable. The basic
 * idea behind this sampler
 * implementation is to generate a random number for each input element as
 * its weight, select the
 * top K elements with max weight. As the weights are generated randomly, so
 * are the selected
 * top K elements. In the first phase, we generate random numbers as the
 * weights for each element and
...
public class Reservoir<T> extends Sampler<T> {
    ...
    /**
     * Create a new sampler with reservoir size and a supplied random number
     * generator.
     *
    ...
    public Iterator<T> sample(Iterator<T> input) {
        ...
        // This queue holds fixed number elements with the top K weight for
        // current partition.
        PriorityQueue<IntermediateSample<T>> queue =
        ...
        while (input.hasNext()) {
            ...
            if (index < numSamples) {
                ...
                queue.add(new IntermediateSample<T>(random.nextDouble(),
                        element));
                smallest = queue.peek();
            } else {
                double rand = random.nextDouble();
                // Remove the element with the smallest weight, and append
                // current element into the queue.
