 ...
 */
public final class RecipePipelineExecutor implements RecipePipeline<Row,
        StructuredRecord, ErrorRecord> {

    private static final Logger LOG =
            LoggerFactory.getLogger(RecipePipelineExecutor.class);
    ...
    public RecipePipelineExecutor(RecipeParser recipeParser,
                                  @Nullable ExecutorContext context) {
        this.context = context;
        this.recipeParser = recipeParser;
    }

    /**
     * Invokes each directives destroy method to perform any cleanup required
     * by each individual directive.
     */
    @Override
    public void close() {
        if (directives == null) {
    ...
    public List<StructuredRecord> execute(List<Row> rows, Schema schema) throws RecipeException {
        try {
            ...
            throw new RecipeException("Problem converting into output record." +
    ...
    public List<Row> execute(List<Row> rows) throws RecipeException {
        ...
        // Initialize schema with input schema from TransientStore if running
        // in service env (design-time) / testing env
        boolean schemaManagementEnabled =
                context != null && context.isSchemaManagementEnabled();
        ...
        try {
            ...
            while (i < rows.size()) {
                ...
                directiveIndex = 0;
                try {
                    for (Executor<List<Row>, List<Row>> directive :
                            directives) {
                        try {
                            directiveIndex++;
                            cumulativeRows = directive.execute(cumulativeRows
                                    , context);
                            ...
                            messages.add(String.format("%s (ecode: %d)",
                                    e.getMessage(), e.getCode()));
                            collector
                                    .add(new ErrorRecord(rows.subList(i,
                                            i + 1).get(0), String.join(",",
                                            messages), e.getCode(), true));
        ...
        if (schemaManagementEnabled && inputSchema != null) {
            context.getTransientStore().set(TransientVariableScope.GLOBAL,
                    TransientStoreKeys.OUTPUT_SCHEMA,
    ...
    private Schema getOutputSchema(Schema inputSchema,
        ...
        for (DirectiveOutputSchemaGenerator outputSchemaGenerator :
                outputSchemaGenerators) {
            try {
                schema =
                        outputSchemaGenerator.getDirectiveOutputSchema(new DirectiveSchemaResolutionContext(schema));
            } catch (RecordConvertorException e) {
                throw new RecipeException("Error while generating output " +
                        "schema for a directive: " + e, e);
            }
        }
        return schema;
    }
