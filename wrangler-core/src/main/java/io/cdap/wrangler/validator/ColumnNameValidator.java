 ...
 *   <ul>
 *     <li> Column name is a an identifier that is a sequence of alphaNumeric
 *     and under_score
 *     characters or a sequence.</li>
 *     <li> Column name is a non-reserved word. Check reserved-column-names
 *     .txt for the list of names.</li>
 *     <li> Column name is less than 255 characters.</li>
 *   </ul>
 * </p>
 */
public class ColumnNameValidator implements Validator<String> {
    private static final String RESERVED_WORDS_FILE = "reserved-column-names" +
            ".txt";
    ...
    public void initialize() throws Exception {
        InputStream in =
                ColumnNameValidator.class.getClassLoader().getResourceAsStream(RESERVED_WORDS_FILE);
        if (in == null) {
            throw new Exception("Unable to load '" + RESERVED_WORDS_FILE + "'" +
                    " from the resources");
        }
    ...
    public void validate(String name) throws ValidatorException {
        ...
        if (!name.matches("^[a-zA-Z0-9_]*$")) {
            throw new ValidatorException("Column '" + name + "' contains " +
                    "non-alphanumeric characters");
        }
        // Reserved words not allowed
        if (reservedWords.contains(name)) {
            throw new ValidatorException("Column '" + name + "' is a reserved" +
                    " word.");
        }
        // Column name length.
        if (name.length() > 255) {
            throw new ValidatorException("Column '" + name + "' is greater " +
                    "than 255 characters.");
        }
    }
}
