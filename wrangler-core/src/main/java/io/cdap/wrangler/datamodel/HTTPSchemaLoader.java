...
/**
 * The {@link HTTPSchemaLoader} object loads an AVRO data model schema at a
 * given url.
 */
public final class HTTPSchemaLoader implements AvroSchemaLoader {

    private static final Logger LOG =
            LoggerFactory.getLogger(HTTPSchemaLoader.class);
    ...
    /**
     * Loads the data models located at the url. All available data models
     * should be
     * listed within a manifest.json file found at the root of the url. The
     * manifest.json should have the schema of
     * {@link io.cdap.wrangler.utils.Manifest}.
     * If the loader is unable to download the manifest or referenced data
     * models,
    ...
    private SetValuedMap<String, Schema> downloadSchemas(Manifest manifest) throws IOException {
        ...
        // The AVRO parser treats meta data properties with a name that has
        // an underscore prefix as invalid names while
        // the AVRO 1.9.1 spec states that underscore prefixes are valid.
        // Consequently disabling validation.
        Schema.Parser parser = new Schema.Parser().setValidate(false);
        for (Map.Entry<String, Manifest.Standard> spec :
                manifest.getStandards().entrySet()) {
            if (spec.getValue().getFormat().equals(HTTPSchemaLoader.FORMAT)) {
                String resource = String.format("%s.%s", spec.getKey(),
                        HTTPSchemaLoader.FORMAT);
                Schema schema = downloadResource(baseUrl, resource, is -> {
                    try {
                        return parser.parse(is);
                    } catch (Exception e) {
                        LOG.warn(String.format("Unable to parse resource %s " +
                                "due to error: %s", resource, e.getMessage()));
                    }
                    return null;
                });
                if (schema != null) {
    ...
    private <R> R downloadResource(String baseUrl, String resourceName,
                                   Function<InputStream, R> callable) {
        R resource = null;
        try {
            URL resourceURL = concat(new URI(baseUrl), resourceName).toURL();
            resource = getRequest(resourceURL, callable);
        } catch (IOException e) {
            LOG.warn(String.format("Unable to download resource %s at %s",
                    resourceName, baseUrl));
        } catch (RestClientException r) {
            LOG.warn(String.format("Failed to make request to due to error " +
                    "%s", r.getMessage()));
        } catch (Exception e) {
            LOG.warn(String.format("Unable to generate resource url for " +
                    "%s/%s", baseUrl, resourceName));
            return null;
        }
        return resource;
    }
    ...
    private URI concat(URI uri, String extraPath) {
        ...
        String newPath = String.format("%s%s%s", uri.getPath(), separator,
                extraPath);
