...
public class DataModelGlossary {
    ...
    /**
     * Initializes the data model glossary with the data models located at
     * the url. All available
     * data models should be listed within a manifest.json file found at the
     * root of the url.
     * The manifest.json should have the schema of
     * {@link io.cdap.wrangler.utils.Manifest}. If
     * the loader is unable to download the manifest or referenced data
     * models, the
     ...
     * @return true if successfully downloaded and store data models into
     * glossary, otherwise
     * false for all other errors.
     */
    public static boolean initialize(String dataModelUrl) {
        HTTPSchemaLoader schemaLoader = new HTTPSchemaLoader(dataModelUrl,
                "manifest.json");
        if (glossary == null) {
