...
public final class RecordConvertor implements Serializable {
    ...
    /**
     * Converts a list of {@link Row} into populated list of
     ...
     */
    public List<StructuredRecord> toStructureRecord(List<Row> rows,
                                                    Schema schema) throws RecordConvertorException {
    ...
    public StructuredRecord decodeRecord(@Nullable Row row, Schema schema) throws RecordConvertorException {
        ...
        // TODO: This is a hack to workaround StructuredRecord processing.
        //  NEED TO RETHINK.
        ...
        // We optimize for use case where wrangler list of fields is equal to
        // the output schema one
        // This value would hold first row field index that we did not map to
        // schema yet
        ...
        for (Schema.Field field : fields) {
            ...
            try {
                ...
                throw new RecordConvertorException(
                        String.format("Field '%s' of type '%s' cannot be set " +
                                        "to '%s'. Make sure the value is " +
                                        "being set is inline with the " +
                                        "specified schema.",
                                name,
                                isNullable ?
                                        fSchema.getNonNullable().getDisplayName() : fSchema.getDisplayName(),
    ...
    private Object decode(String name, Object object, Schema schema) throws RecordConvertorException {
        ...
        Schema.LogicalType logicalType = isNullable ?
                ...
                case DATETIME:
                    ...
                    if (object == null) {
                        throw new UnexpectedFormatException(
                                String.format("Datetime field %s should have " +
                                        "a non null value", name));
                    }
                    try {
                        LocalDateTime.parse((String) object);
                    } catch (DateTimeParseException exception) {
                        throw new UnexpectedFormatException(
                                String.format("Datetime field '%s' with value" +
                                                " '%s' is not in ISO-8601 " +
                                                "format.",
        ...
        switch (type) {
            ...
            case MAP:
                ...
                return decodeMap(name, (Map<Object, Object>) object, key,
                        value);
            case UNION:
        ...
        throw new RecordConvertorException(
                String.format("Unable decode object '%s' with schema type " +
    ...
    private StructuredRecord decodeRecord(String name,
        ...
        if (object instanceof StructuredRecord) {
            ...
            List<Object> values = decodeArray(name, object,
        ...
        throw new RecordConvertorException(
                String.format("Unable decode object '%s' with schema type " +
                        "'%s'.", name, schema.getType().toString())
        );
    }

    private StructuredRecord decodeRecord(String name,
                                          JsonObject nativeObject,
                                          Schema schema) throws RecordConvertorException {
        StructuredRecord.Builder builder = StructuredRecord.builder(schema);
    ...
    private StructuredRecord decodeRecord(String name, Map nativeObject,
                                          Schema schema) throws RecordConvertorException {
        StructuredRecord.Builder builder = StructuredRecord.builder(schema);
        for (Schema.Field field : schema.getFields()) {
            String fieldName = field.getName();
    ...
    private Object decodeSimpleTypes(String name, Object object,
                                     Schema schema) throws RecordConvertorException {
        Schema.Type type = schema.getType();
        ...
        switch (type) {
            ...
            case INT:
                if (object instanceof Integer || object instanceof Short) {
                    ...
                    try {
                        ...
                        throw new RecordConvertorException(
                                String.format("Unable to convert '%s' to " +
                                                "integer for field name '%s'"
                                        , value,
                                        name), e);
                    }
                } else {
                    throw new RecordConvertorException(
                            String.format("Schema specifies field '%s' is " +
                                            "integer, but the value is not a " +
                                            "integer " +
                                            "or string. " +
                                            "It is of type '%s'", name,
            ...
            case LONG:
                if (object instanceof Long) {
                    ...
                    try {
                        ...
                        throw new RecordConvertorException(
                                String.format("Unable to convert '%s' to long" +
                                        " for field name '%s'", value, name),
                                e);
                    }
                } else {
                    throw new RecordConvertorException(
                            String.format("Schema specifies field '%s' is " +
                                            "long, but the value is nor a " +
                                            "string or " +
                                            "long. " +
                                            "It is of type '%s'", name,
                                    object.getClass().getSimpleName())
            ...
            case BYTES:
                if (object instanceof byte[]) {
                    ...
                    throw new RecordConvertorException(
                            String.format("Unable to convert '%s' to bytes " +
                                            "for field name '%s'",
                                    object.toString(),
                                    name)
                    );
    ...
    private Map<Object, Object> decodeMap(String name,
        ...
        for (Map.Entry<Object, Object> entry : object.entrySet()) {
            output.put(decode(name, entry.getKey(), key), decode(name,
                    entry.getValue(), value));
        }
        return output;
    }

    private Object decodeUnion(String name, Object object,
                               List<Schema> schemas) throws RecordConvertorException {
    ...
    private List<Object> decodeArray(String name, Object object,
                                     Schema schema) throws RecordConvertorException {
    ...
    private List<Object> decodeArray(String name, JsonArray list,
                                     Schema schema) throws RecordConvertorException {
