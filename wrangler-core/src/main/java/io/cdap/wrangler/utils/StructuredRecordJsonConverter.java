...
/**
 * Utility class for converting {@link StructuredRecord} to and from json as
 * the json
...
public final class StructuredRecordJsonConverter {
    ...
    private static final Map<Class<?>, Schema.Type> TYPE_TO_SCHEMA =
            new IdentityHashMap<>(
    ...
    private static Object readJson(JsonReader reader, Schema schema) throws IOException {
        switch (schema.getType()) {
            ...
            case ENUM:
                // Currently there is no standard container to represent enum
                // type
                return reader.nextString();
            case ARRAY:
    ...
    private static List<Object> readArray(JsonReader reader,
                                          Schema elementSchema) throws IOException {
    ...
    private static StructuredRecord readRecord(JsonReader reader,
    ...
    private static void writeJson(JsonWriter writer, Schema schema,
    ...
    private static void writeUnion(JsonWriter writer, Schema schema,
                                   Object value) throws IOException {
    ...
    private static void writeBytes(JsonWriter writer, ByteBuffer buffer) throws IOException {
        if (buffer.hasArray()) {
            writeBytes(writer, buffer.array(),
    ...
    private static void writeBytes(JsonWriter writer, byte[] bytes, int off,
    ...
    private static void writeArray(JsonWriter writer, Schema elementSchema,
                                   Object value) throws IOException {
    ...
    private static void writeMap(JsonWriter writer,
                                 Map.Entry<Schema, Schema> entrySchema,
                                 Object value) throws IOException {
    ...
    private static void writeRecord(JsonWriter writer, Schema schema,
                                    Object value) throws IOException {
    ...
    private static Schema findUnionSchema(Schema unionSchema,
                                          @Nullable Object value) throws IOException {
        Schema.Type type = getSchemaType(value);

        for (Schema schema : unionSchema.getUnionSchemas()) {
            // Just match the type, not matching the detail schema as it'd be
            // too expensive.
