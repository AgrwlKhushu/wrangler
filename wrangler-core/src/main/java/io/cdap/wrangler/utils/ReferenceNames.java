...
public class ReferenceNames {
    ...
    public static void validate(String referenceName) {
        if (!DATASET_PATTERN.matcher(referenceName).matches()) {
            throw new IllegalArgumentException(
                    String.format("Invalid reference name '%s'. Supported " +
    ...
    /**
     * Cleanse the given reference name. This method will remove all the
     * disallowed characters in the given reference
     * name. For example, 111-22-33(1).csv will get convert to 111-22-331.csv
     * . If no valid characters, the method will
     * return "sample".
     *
