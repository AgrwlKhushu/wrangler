...
public class StructuredToRowTransformer {
    ...
    public static Object getValue(StructuredRecord input, String fieldName) {
        ...
        fieldSchema = fieldSchema.isNullable() ?
                fieldSchema.getNonNullable() : fieldSchema;
    ...
    @Nullable
    private static Object processValue(@Nullable Object val, Schema schema,
                                       String fieldName) {
        if (val == null) {
            return null;
        }
        switch (schema.getType()) {
            // if the type is bytes, need to make sure the value is byte
            // array since byte buffer is not serializable
            case BYTES:
                return val instanceof ByteBuffer ?
                        Bytes.toBytes((ByteBuffer) val) : val;
                ...
                if (val instanceof Iterable) {
                    ...
                    for (Object item : (Iterable<?>) val) {
                        if ((item != null) && (schema.getComponentSchema() != null)) {
                            rowList.add(processValue(item,
                                    schema.getComponentSchema(), fieldName));
                        } else {
                            rowList.add(item);
                        }
                    }
            ...
            default:
                // If the logical type is present in complex types, it will
                // be retrieved as corresponding
