...
public final class ColumnConverter {
    ...
    static {
        ...
        // TODO currently CDAP does not have a SHORT datatype for I32 and
        //  SHORT arguments.
    ...
    /**
     * Renames a column. The renamed column must not exist in the row. The
     ...
     * @throws DirectiveExecutionException when a column matching the target
     *                                     name already exists
     */
    public static void rename(String directiveName, Row row, String column,
                              String toName)
        ...
        // with the original column except the casing
        if (existingColumn == -1 || idx == existingColumn) {
            row.setColumn(idx, toName);
        } else {
            throw new DirectiveExecutionException(
                    directiveName, String.format("Column '%s' already exists." +
                            " Apply the 'drop %s' directive before " +
                            "renaming '%s' to '%s'.",
    ...
    /**
     * Converts the column type into another type. Only target types int,
     * short, long, double, boolean, string, and bytes
     * are supported.
     *
     * @param row    source record to be modified.
     * @param column name of the column within source record.
     * @param toType the target type of the column.
     * @throws DirectiveExecutionException when an unsupported type is
     *                                     specified or the column can not be
     *                                     converted.
     */
    public static void convertType(String directiveName, Row row,
                                   String column, String toType,
        ...
        if (idx != -1) {
            ...
            try {
                Object converted = ColumnConverter.convertType(column, toType
                        , object);
                if (toType.equalsIgnoreCase(ColumnTypeNames.DECIMAL)) {
                    row.setValue(idx,
                            setDecimalScaleAndPrecision((BigDecimal) converted, scale,
                ...
                throw new DirectiveExecutionException(
                        directiveName, String.format("Column '%s' cannot be " +
                        "converted to a '%s'.", column, toType), e);
            }
        }
    }
        ...
        switch (toType) {
            ...
            default:
                throw new DirectiveExecutionException(String.format(
                        "Column '%s' is of unsupported type '%s'. Supported " +
                                "types are: " +
                                "int, short, long, double, decimal, boolean, " +
                                "string, bytes", col, toType));
        }
        throw new DirectiveExecutionException(
                String.format("Column '%s' has value of type '%s' and cannot " +
                                "be converted to a '%s'.", col,
                        object.getClass().getSimpleName(), toType));
    }

    private static BigDecimal setDecimalScaleAndPrecision(BigDecimal decimal,
                                                          Integer scale,
                                                          Integer precision,
                                                          RoundingMode roundingMode)
            throws DirectiveExecutionException {
        ...
        try {
            if (precision == null) {
                ...
                if (validateScaleAndPrecision(scale, precision, decimal)) {
                    ...
                    result = result.round(new MathContext(precision,
                            roundingMode));
                } else {
                    throw new DirectiveExecutionException(String.format(
                            "Cannot set scale as '%s' and precision as '%s' " +
                                    "for value '%s' when"
                                    + "given precision - scale is less than " +
                                    "number of digits"
                                    + " before decimal point ", scale,
                            precision, decimal));
                }
                return result;
            }
            ...
            throw new DirectiveExecutionException(String.format(
                    "Cannot set scale as '%s' and precision '%s' for value " +
                            "'%s' when rounding-mode "
                            + "is '%s'", scale, precision, decimal,
                    roundingMode), e);
        }
    }

    private static Boolean validateScaleAndPrecision(Integer scale,
                                                     Integer precision,
                                                     BigDecimal decimal) {
        int digitsBeforeDecimalPoint = decimal.signum() == 0 ? 1 :
                decimal.precision() - decimal.scale();
        return precision - scale >= digitsBeforeDecimalPoint;
    }

    public static Schema getSchemaForType(String type, Integer scale,
                                          Integer precision) throws DirectiveParseException {
        Schema typeSchema;
        ...
        if (type.equals(ColumnTypeNames.DECIMAL)) {
            ...
            if (!SCHEMA_TYPE_MAP.containsKey(type)) {
                throw new DirectiveParseException(String.format("'%s' is an " +
                        "unsupported type. " +
                        "Supported types are: int, short, long, double, " +
                        "decimal, boolean, string, bytes", type));
            }
            typeSchema =
                    Schema.nullableOf(Schema.of(SCHEMA_TYPE_MAP.get(type)));
        }
        return typeSchema;
    }
