...
/**
 * This class provides functionality to retrieve  {@link Schema} from a row
 * or an object.
 */
public final class SchemaConverter {
     ...
     * @return the schema of this object
     * NOTE: ANY NEWLY SUPPORTED DATATYPE SHOULD ALSO BE REGISTERED IN
     * {@link KryoSerializer}
     */
    @Nullable
    public Schema getSchema(Object value, String name,
                            @Nullable String recordPrefix) throws RecordConvertorException {
        ...
        if (value instanceof String || value instanceof Integer || value instanceof Long || value instanceof Short ||
            ...
            try {
                ...
                throw new RecordConvertorException(
                        String.format("Unable to convert field '%s' to basic " +
                                "type.", name), e);
            }
        }

        if (value instanceof BigDecimal) {
            // TODO CDAP-15361 precision should be derived from the schema of
            //  the row.
            return Schema.nullableOf(Schema.decimalOf(38,
        ...
        // TODO - remove all the instaces of java.util.Date once all the
        //  directives support LogicalType.
        ...
        if (value instanceof Row) {
            ...
            for (Pair<String, Object> field : ((Row) value).getFields()) {
                if (field.getSecond() != null) {
                    fields.add(Field.of(field.getFirst(),
                            getSchema(field.getSecond(), field.getFirst(),
                                    recordTypeName)));
                }
            }
    ...
    private Schema toSchema(String name, JsonArray array,
        ...
        if (sum > 0) {
            ...
            if (types[2] > 0 || types[1] > 0) {
                return Schema.nullableOf(Schema.arrayOf(toComplexSchema(name,
                        child, recordPrefix)));
            } else if (types[0] > 0) {
                return Schema.nullableOf(Schema.arrayOf(toSchema(name,
                        child.getAsJsonPrimitive())));
            }
        }
    ...
    private Schema toSchema(String name, JsonPrimitive primitive) throws RecordConvertorException {
        ...
        try {
            ...
            throw new RecordConvertorException(
                    String.format("Unable to convert field '%s' to basic type" +
                            ".", name), e);
        }
    }
    ...
    private Schema toSchema(String name, JsonObject object,
    ...
    private static final class SimpleSchemaGenerator extends AbstractSchemaGenerator {
        ...
        protected Schema generateRecord(TypeToken<?> typeToken, Set<String> set,
            ...
            // we don't actually leverage this method for types we support,
            // so no need to implement it
            throw new UnsupportedTypeException(String.format("Generating " +
                            "record of type %s is not supported.",
                    typeToken));
        }
    }
}
