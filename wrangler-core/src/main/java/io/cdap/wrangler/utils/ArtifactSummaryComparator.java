...
public class ArtifactSummaryComparator implements Comparator<ArtifactSummary> {
    private static final ArtifactSummaryComparator COMPARATOR =
            new ArtifactSummaryComparator();

    /**
     * Pick up the latest artifact, this method assumes the name of the
     * artifact is same
     */
    public static ArtifactSummary pickLatest(ArtifactSummary artifact1,
                                             ArtifactSummary artifact2) {
        return COMPARATOR.compare(artifact1, artifact2) > 0 ? artifact1 :
                artifact2;
    }
    ...
    public int compare(ArtifactSummary summary1, ArtifactSummary summary2) {
        ...
        int cmp =
                new ArtifactVersion(summary1.getVersion()).compareTo(new ArtifactVersion(summary2.getVersion()));
        if (cmp != 0) {
            return cmp;
        }
