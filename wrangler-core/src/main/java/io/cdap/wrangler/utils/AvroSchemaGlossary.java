...
/**
 * The {@link AvroSchemaGlossary} class is used for management of AVRO data
 * model schema definitions.
 */
public class AvroSchemaGlossary {

    private static final Logger LOG =
            LoggerFactory.getLogger(AvroSchemaGlossary.class);
    private static final String REVISION_PROPERTY = "_revision";

    private AvroSchemaLoader avroSchemaLoader;
    private SetValuedMap<String, Schema> glossary =
    ...
    /**
     * Setter for the {@link AvroSchemaLoader} to use when populating the
     * glossary.
     *
    ...
    /**
     * Configures the {@link AvroSchemaGlossary} with the schemas accessible
     * through the {@link AvroSchemaLoader}.
     *
    ...
    public Schema get(String name, long revision) {
        ...
        for (Schema schema : schemas) {
            try {
                long rev =
                        Long.parseLong(schema.getProp(AvroSchemaGlossary.REVISION_PROPERTY), 10);
                if (rev == revision) {
                    result = schema;
                    break;
                }
            } catch (NumberFormatException e) {
                LOG.error(String.format("unable to parse %s property within " +
                                "schema %s",
                        AvroSchemaGlossary.REVISION_PROPERTY,
                        schema.getFullName()));
            }
        }
        return result;
