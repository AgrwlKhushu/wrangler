...
public class ArithmeticOperations {
     ...
     * Used for operations applied to columns of multiple types, in order to
     * determine the type of the output column.
     * The order of increasing generality is Short < Integer < Long < Float <
     * Double < BigDecimal.
     * Returns null if any input is null.
     * Throws an error if any input of type Byte is given (this is not
     * currently supported for arithmetic ops).
     */
    private static NumberType getOutputType(Number... nums) throws DirectiveExecutionException {
        ...
        for (Number num : nums) {
            if (num instanceof Byte) {
                throw new DirectiveExecutionException("Input cannot be of " +
                        "type 'Byte'.");
            } else if (num == null) {
                return null;
    ...
    /**
     * Arithmetic operation - Find the sum of any number of columns, of any
     * valid types.
     ...
     * Output type is Double, except when there is an input of type
     * BigDecimal, in which case the output is BigDecimal.
    ...
    /**
     * Arithmetic operation - divides one column by another and returns the
     * remainder.
     * Output type is most general of input types, except 2 Shorts should
     * give an Integer.
     * Returns null if any input value is null.
     */
    ...
    public static Number lcm(Number x, Number y) throws DirectiveExecutionException {
        ...
        switch (outputType) {
            ...
            default:
                BigDecimal pow = BigDecimal.valueOf(Math.pow(10,
                        Math.max(bdX.scale(), bdY.scale())));
                BigInteger val1 = bdX.multiply(pow).toBigInteger();
                BigInteger val2 = bdY.multiply(pow).toBigInteger();
                BigDecimal absProduct =
                        new BigDecimal(val1.multiply(val2).abs())
                                .setScale(Math.min(bdX.scale(), bdY.scale()),
                                        BigDecimal.ROUND_HALF_EVEN);
                BigDecimal gcd = new BigDecimal(val1.gcd(val2));
                if (gcd.compareTo(BigDecimal.ZERO) == 0) {
                    return BigDecimal.ZERO;
                }
                return absProduct.divide(gcd.multiply(pow),
                        BigDecimal.ROUND_HALF_EVEN);
        }
    }

    /**
     * Arithmetic operation - Check if all values are equal across any number
     * of Short columns.
     */
    ...
    /**
     * Arithmetic operation - Find the maximum of any number of columns, of
     * any valid types.
     * Output type is most general of input types.
     * Returns null if any input value is null.
     */
    ...
    /**
     * Arithmetic operation - Find the minimum of any number of columns, of
     * any valid types.
     * Output type is most general of input types.
     * Returns null if any input value is null.
     */
     ...
     * Returns Number - BigDecimal if any column is of type BigDecimal,
     * double otherwise.
    ...
    public static Number average(Number... nums) throws DirectiveExecutionException {
        ...
        for (Number num : nums) {
            ...
            if (containsBigDecimal) {
                BigDecimal numer =
                        numberToBigDecimal(num).subtract((BigDecimal) avg);
                BigDecimal denom = BigDecimal.valueOf(t);
                avg = ((BigDecimal) avg).add(numer.divide(denom,
    ...
    // The Number types upon which arithmetic operations can be performed, in
