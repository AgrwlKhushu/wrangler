...
/**
 * Type Interface provides utility functions that allow you to detect the
 * types of data.
 */
public class TypeInference {
    private static final Pattern patternInteger = Pattern.compile("^(\\+|-)" +
            "?\\d+$");

    private static final Pattern patternDouble = Pattern.compile(
            "^[-+]?"// Positive/Negative sign
                    + "("// BEGIN Decimal part
                    + "[0-9]+([,\\.][0-9]+)?|"// Alternative I (w/o grouped
                    // integer part)
