...
public class ELContext implements JexlContext {
    ...
    /**
     * Constructor that extracts the {@link ExecutorContext} internals and
     * turns them into variables.
     * This method extracts the transient variables, runtime arguments,
     * environment it's running in and
     * the context in which it is running as identifiers that can be used
     * within JEXL expression.
     *
     * @param context to be examined to be extracted into JEXL expression
     *                variables.
     */
    ...
    /**
     * Sets the context for EL, includes the required variables in
     * expression, 'this' and 'ctx'.
     *
     * @param context to be examined to be extracted into JEXL expression
     *                variables.
    ...
    public ELContext(ExecutorContext context, EL el, Row row) {
        ...
        // These two steps below has to always happen after el variables are
        // loaded
    ...
    private void init(ExecutorContext context) {
        if (context != null) {
            ...
            set("ctx", new Context(context.getEnvironment().name(),
     ...
     * All properties of this class are public to ensure they can be accessed
     * with dot(.) format.
     */
