...
public final class EL {
    ...
    /**
     * Compiles the given expressions and return an {@link EL} for script
     * execution.
     *
    ...
    public static EL compile(ELRegistration registration, String expression) throws ELException {
        ...
        try {
            ...
            // JexlException.getMessage() uses 'io.cdap.wrangler.expression
            // .EL' class name in the error message.
            // So instead use info object to get information about error
            // message and create custom error message.
            JexlInfo info = e.getInfo();
            throw new ELException(
                    String.format("Error encountered while compiling '%s' at " +
                                    "line '%d' and column '%d'. " +
                                    "Make sure a valid jexl transformation is" +
                                    " provided.",
                            // here the detail can be null since there are
                            // multiple subclasses which extends this
                            // JexlException, not all of them has this detail
                            // information
                            info.getDetail() == null ? expression :
                                    info.getDetail(), info.getLine(),
                            info.getColumn()), e);
        } catch (Exception e) {
    ...
    public ELResult execute(ELContext context) throws ELException {
        try {
            ...
            JexlInfo info = e.getInfo();
            throw new ELException(
                    String.format("Error encountered while executing '%s', at" +
                                    " line '%d' and column '%d'. " +
                                    "Make sure a valid jexl transformation is" +
                                    " provided.",
                            // here the detail can be null since there are
