...
public class GrammarWalker {
    ...
    public <E extends Exception> void walk(String recipe,
        ...
        if (!status.isSuccess()) {
            ...
            String prefix = "Encountered syntax error, please ensure the " +
                    "directive is valid:\n";
        ...
        while (tokenGroups.hasNext()) {
            ...
            if (!root.equals(command) && context.isExcluded(command)) {
                throw new DirectiveParseException(
                        command, String.format("Aliased directive '%s' has " +
                        "been configured as restricted directive and "
                        + "is hence unavailable. Please contact your " +
            ...
            if (context.isExcluded(root)) {
                throw new DirectiveParseException(
                        command, String.format("Directive '%s' has been " +
                                "configured as restricted directive and is " +
                                "hence " +
                                "unavailable. Please contact your " +
                                "administrator",
                        command));
            }
