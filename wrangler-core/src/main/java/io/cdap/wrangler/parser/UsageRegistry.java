...
public final class UsageRegistry implements Serializable {
    private static final Logger LOG =
            LoggerFactory.getLogger(UsageRegistry.class);
    ...
    public void addUsage(Class<? extends Executor> classz) {
        ...
        usages.put(name.value(), new UsageEntry(name.value(), usage.value(),
                description.value()));
        usageList.add(new UsageEntry(name.value(), usage.value(),
                description.value()));
    }
    ...
    /**
     * Loads all the default system directives that are available with the
     * system.
     */
    private void addDefaultDirectives() {
        ...
        Set<Class<? extends Executor>> steps =
                reflections.getSubTypesOf(Executor.class);
        ...
        addUsage("set format", "set format csv <delimiter> <skip empty lines>",
                "[DEPRECATED] Parses the predefined column as CSV. Use " +
                        "'parse-as-csv' instead.");
        addUsage("format-unix-timestamp", "format-unix-timestamp <column> " +
                        "<format>",
                "Formats a UNIX timestamp using the specified format");
        addUsage("filter-row-if-not-matched", "filter-row-if-not-matched " +
                        "<column> <regex>",
    ...
    /**
     * A {@link UsageEntry} defines the information about the directives that
     * are available.
     */
