...
public class MapArguments implements Arguments {
    ...
    public MapArguments(UsageDefinition definition, TokenGroup group) throws DirectiveParseException {
        ...
        int required =
                definition.getTokens().size() - definition.getOptionalTokensCount();
        if ((required > group.size() - 1) || ((group.size() - 1) > definition.getTokens().size())) {
            throw new DirectiveParseException(
                    definition.getDirectiveName(), String.format("Improper " +
                            "usage of directive '%s', usage - '%s'",
                ...
                if (!specification.optional()) {
                    if (!specification.type().equals(token.type())) {
                        if (specification.type() == TokenType.COLUMN_NAME_LIST && token.type() == TokenType.COLUMN_NAME) {
                            ...
                            tokens.put(specification.name(),
                                    new ColumnNameList(values));
                            pos = pos + 1;
                            break;
                            ...
                            throw new DirectiveParseException(
                                    String.format("Expected argument '%s' to " +
                                                    "be of type '%s', but it " +
                                                    "is of type '%s' - %s",
                                            specification.name(),
                                            specification.type().name(),
                                            token.type().name(),
                                            group.getSourceInfo().toString())
                            );
                        }
                    } else {
     ...
     *
     * <p>The optional arguments specified during the
     ...
     * <code>UsageDefinition</code>.
     * There are two reason why the <code>name</code> might not exists in
     * this object :
     *
     * <ul>
     ...
     * {@link TokenType}.
     * When the directive is parsed into token, the type of the token is
     * passed through.
     *
     * @param name associated with the token.
     ...
     * specified
     * the user. It returns the <code>String</code> representation of the
     * directive.
     *
