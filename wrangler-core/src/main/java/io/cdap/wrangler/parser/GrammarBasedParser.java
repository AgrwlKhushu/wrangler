...
public class GrammarBasedParser implements RecipeParser {
    ...
    public List<Directive> parse() throws RecipeException {
        ...
        try {
            ...
            new GrammarWalker(new RecipeCompiler(), context).walk(recipe,
                    (command, tokenGroup) -> {
                        directiveIndex.getAndIncrement();
                        DirectiveInfo info = registry.get(namespace, command);
                        if (info == null) {
                            throw new DirectiveNotFoundException(
                                    String.format("Directive '%s' not found " +
                                            "in system" +
                                            " and user scope. Check the name " +
                                            "of " +
                                            "directive.", command)
                            );
                        }
                        ...
                        try {
                            ...
                            Arguments arguments = new MapArguments(definition,
                                    tokenGroup);
            ...
            return result;
        } catch (DirectiveLoadException | DirectiveNotFoundException |
                 DirectiveParseException e) {
