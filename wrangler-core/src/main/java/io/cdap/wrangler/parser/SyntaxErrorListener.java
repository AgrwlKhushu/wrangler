...
public final class SyntaxErrorListener extends BaseErrorListener {
    ...
    public void syntaxError(Recognizer<?, ?> recognizer,
        ...
        if (offSymbol != null) {
            String charstream =
                    offSymbol.getTokenSource().getInputStream().toString();
        ...
        if (offSymbol.getType() == -1 && thisError == tokens.size() - 1) {
            ...
            String message = "At line " + line + ":" + charPositionInLine +
                    ": " + msg;
            errors.add(new SyntaxError(line, charPositionInLine, message,
                    source));
            return;
        }

        String offSymName =
                DirectivesLexer.VOCABULARY.getDisplayName(offSymbol.getType());
