...
/**
 * This class {@link BinaryAvroDecoder} decodes a byte array of AVRO Json
...
public class BinaryAvroDecoder extends AbstractAvroDecoder {
     ...
     * This method will iterate through each of the AVRO schema fields and
     * translate
     * them into columns within the {@link Row}.
     * <p>
     * If the field is instance of {@link List} or {@link Map} it is
     * converted into JSON
     * representation. In order to flatten or expand such columns other
     * directives need
     * to be used.
     *
     * @param bytes array of bytes that contains binary encoded AVRO record.
     * @return list of {@link Row} that are converted from AVRO encoded
     * binary messages.
     */
    @Override
    public List<Row> decode(byte[] bytes) throws DecoderException {
        ...
        try {
            ...
            throw new DecoderException("Issue creating AVRO binary decoder. " +
                    "Verify the schema.");
        } finally {
            try {
                in.close();
