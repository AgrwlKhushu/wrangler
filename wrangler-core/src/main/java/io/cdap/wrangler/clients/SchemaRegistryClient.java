...
/**
 * This class {@link SchemaRegistryClient} is a client API for the
 * SchemaRegistry service.
 * <p>
 * Client allows only read-only access to service. It doesn't support the
 * ability to mutate
 * the service.
 * <p>
...
public final class SchemaRegistryClient {
    ...
    private int connectionTimeout; // Timeout to connect specified in
    // milliseconds.
    ...
    /**
     * Returns a instance of {@link SchemaRegistryClient} initialized with
     * service url.
     *
    ...
    public static SchemaRegistryClient getInstance(ExecutorContext context) {
        return new SchemaRegistryClient(context.getService("dataprep",
                "service").toString());
    }
     ...
     * @throws URISyntaxException  thrown if there are issue with
     *                             construction of url.
     * @throws IOException         throw when there are issues connecting to
     *                             the service.
     * @throws RestClientException thrown when there are issues with request
    ...
    public byte[] getSchema(String namespace, String id, long version)
        ...
        URL url = concat(new URI(baseUrl),
                String.format("contexts/%s/schemas/%s/versions/%d", namespace
                        , id, version)).toURL();
        Response<SchemaInfo> response = request(url, "GET",
                new TypeToken<Response<SchemaInfo>>() {
                }.getType());
    ...
    /**
     * Retrieves schema provided a schema id. It provides the latest, current
     * version of schema.
     *
     * @param namespace the schema namespace
     * @param id        the schema id
    ...
    public byte[] getSchema(String namespace, String id)
            throws URISyntaxException, IOException, RestClientException {
        URL url = concat(new URI(baseUrl), String.format("contexts/%s/schemas" +
                "/%s", namespace, id)).toURL();
        Response<SchemaInfo> response = request(url, "GET",
                new TypeToken<Response<SchemaInfo>>() {
                }.getType());
        if (response.getCount() == 1) {
    ...
    public List<Long> getVersions(String namespace, String id)
            throws URISyntaxException, IOException, RestClientException {
        URL url = concat(new URI(baseUrl),
                String.format("contexts/%s/schemas/%s/versions", namespace,
                        id)).toURL();
        Response<Long> response = request(url, "GET",
    ...
    private <T> T request(URL url, String method, Type classz) throws IOException, RestClientException {
        return request(url, method, null, new HashMap<String, String>(),
                classz);
    }
    ...
    private <T> T request(URL url, String method, byte[] body,
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                Response message;
                try {
                    message = gson.fromJson(new InputStreamReader(es),
                            Response.class);
                ...
                throw new RestClientException(message.getStatus(),
                        message.getMessage());
            } else {
                InputStream es = connection.getErrorStream();
