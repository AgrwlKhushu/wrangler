...
public final class GeoFences {
     ...
     * Checks if Point is inside any of the given polygonal geofences based
     * on the winding number algorithm.
     *
     ...
     */
    @Deprecated
    public static Boolean inFence(double latitude, double longitude,
                                  String geofences) {
    ...
    /**
     * Static method to be used with jexl
     * Checks if the coordinate is inside any of the given polygonal
     * geofences based on the winding number algorithm.
    ...
    public static Boolean InFence(Double latitude, Double longitude,
        ...
        try {
            featureCollection = GSON.fromJson(geofences,
                    FeatureCollection.class);
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException(String.format("String %s is " +
                            "not a valid geoJson representation of fence",
                    geofences), e);
        } catch (JsonSyntaxException e) {
            throw new IllegalArgumentException(String.format("String %s is " +
                    "not a valid Json string", geofences), e);
        }
    ...
    private static Boolean isPointInside(Feature feature,
                                         Coordinates location) {
        Polygon polygon = (Polygon) feature.geometry();
        List<SinglePosition> positions =
        ...
        for (int i = 0; i < positions.size() - 1; i++) {
            if (positions.get(i).coordinates().getLat() <= location.getLat()) {
                if (positions.get(i + 1).coordinates().getLat() > location.getLat()) {
                    if (isLeft(positions.get(i).coordinates(),
                            positions.get(i + 1).coordinates(), location) > 0.0) {
                        ++wn;
                    }
                }
            } else {
    ...
    private static double isLeft(Coordinates vertex0, Coordinates vertex1,
                                 Coordinates gpC) {
