...
public final class DDL {
     ...
     * <p>
     * This method is added if a user makes a mistake of parsing the schema
     * again.
     *
    ...
    /**
     * Given a {@link StructuredRecord} returns the same
    ...
    /**
     * Given a {@link StructuredRecord} and the name of the field, it checks,
     * if the
    ...
    private static Schema drop(Schema schema, @Nullable String path,
                               String fullPath) {
        if (path == null) {
            return schema;
        }
        ...
        if (bracketIndex > 0) {
            if (!recordField.endsWith("]")) {
                throw new IllegalArgumentException(
                        String.format("Invalid field '%s' in path '%s'. An " +
                                        "array index start with '[' and must " +
                                        "end with a ']'",
                                recordField, fullPath));
            }
            String indexStr = recordField.substring(bracketIndex + 1,
                    recordField.length() - 1);
            try {
                arrayIndex = Integer.parseInt(indexStr);
            } catch (NumberFormatException e) {
                throw new IllegalArgumentException(
                        String.format("Invalid array index '%s' for field " +
                                        "'%s' in path '%s'. Must be a valid " +
                                        "integer.",
        ...
        for (Schema.Field field : schema.getFields()) {
            if (field.getName().equals(recordField)) {
                ...
                Schema.Type fieldtype = isNullable ?
                        fieldSchema.getNonNullable().getType() :
                        fieldSchema.getType();
                if (arrayIndex >= 0) {
                    if (fieldtype != Schema.Type.ARRAY) {
                        throw new IllegalArgumentException(
                                String.format("Array syntax for nested field " +
                                                "'%s' in path '%s' was " +
                                                "specified, but '%s' is of " +
                                                "type %s.",
                                        recordField, fullPath, recordField,
                                        fieldtype));
                    }
                    ...
                    Schema.Type componentType = componentNullable ?
                            componentSchema.getNonNullable().getType() :
                            componentSchema.getType();
                    if (nextPath != null && componentType != Schema.Type.RECORD) {
                        throw new IllegalArgumentException(
                                String.format("Nested field '%s' in path '%s'" +
                                                " must be of type '%s', but " +
                                                "is '%s'.",
                                        recordField, fullPath,
                                        Schema.Type.RECORD, componentType));
                    }

                    Schema componentSchemaWithout =
                            drop(componentNullable ?
                                    componentSchema.getNonNullable() :
                                    fieldSchema, nextPath, fullPath);
                    componentSchemaWithout =
                            componentNullable ?
                                    Schema.nullableOf(componentSchemaWithout) : componentSchemaWithout;

                    Schema arraySchema = Schema.arrayOf(componentSchemaWithout);
                    arraySchema = isNullable ?
                            Schema.nullableOf(arraySchema) : arraySchema;
                    fields.add(Schema.Field.of(field.getName(), arraySchema));
                    continue;
                } else if (nextPath != null && fieldtype != Schema.Type.RECORD) {
                    throw new IllegalArgumentException(
                            String.format("Nested field '%s' in path '%s' " +
                                            "must be of type '%s', but is " +
                                            "'%s'.",
                                    recordField, fullPath, Schema.Type.RECORD
                                    , fieldtype));
                }
                fields.add(Schema.Field.of(field.getName(),
                        drop(field.getSchema(), nextPath, fullPath)));
            } else {
                fields.add(field);
            }
        }
    ...
    private static Schema select(Schema schema, String path,
        ...
        if (bracketIndex > 0) {
            if (!fieldName.endsWith("]")) {
                throw new IllegalArgumentException(
                        String.format("Invalid field '%s'. An index start '['" +
                                " must end with a ']'", fieldName));
            }
        ...
        if (field == null) {
            throw new IllegalArgumentException(
                    String.format("Nested field '%s' in path '%s' does not " +
                            "exist", fieldName, path));
        }
        ...
        if (key != null) {
            if (fieldType == Schema.Type.ARRAY) {
                try {
                    ...
                    if (arrayIndex < 0) {
                        throw new IllegalArgumentException(
                                String.format("Invalid array index '%d' for " +
                                                "field '%s' in path '%s. Must" +
                                                " be at least 0.",
                    ...
                    throw new IllegalArgumentException(
                            String.format("Invalid array index '%s' for field" +
                                            " '%s' in path '%s'. Must be a " +
                                            "valid int.",
                                    key, fieldName, path));
                }

                fieldSchema = isNullable ?
                        fieldSchema.getNonNullable().getComponentSchema() :
                        fieldSchema.getComponentSchema();
                ...
                Map.Entry<Schema, Schema> mapSchema =
                        fieldSchema.getMapSchema();
                Schema keySchema = mapSchema.getKey();
                Schema valSchema = mapSchema.getValue();

                Schema.Type keyType = keySchema.isNullable() ?
                        keySchema.getNonNullable().getType() :
                        keySchema.getType();
                if (keyType != Schema.Type.STRING) {
                    throw new IllegalArgumentException(
                            String.format("Only map keys of type string are " +
                                            "supported. Field '%s' in path " +
                                            "'%s' has keys of type '%s'.",
                ...
                throw new IllegalArgumentException(
                        String.format("Array syntax for nested field '%s' in " +
                                        "path '%s' was specified, but '%s' is" +
                                        " of type %s.",
        ...
        if (fields.hasNext()) {
            if (fieldType != Schema.Type.RECORD) {
                throw new IllegalArgumentException(
                        String.format("Nested field '%s' in path '%s' must be" +
                                        " of type '%s', but is '%s'.",
                                fieldName, path, Schema.Type.RECORD,
                                fieldType));
            }

            return select(isNullable ? fieldSchema.getNonNullable() :
                    fieldSchema, path, fields);
        }
        return fieldSchema;
    }
    ...
    public static <T> T select(StructuredRecord record, String path) {
        return recursiveGet(record, path,
                Splitter.on('.').split(path).iterator());
    }

    private static <T> T recursiveGet(StructuredRecord record, String path,
                                      Iterator<String> fields) {
        String fieldName = fields.next();
            ...
            if (fieldType == Schema.Type.ARRAY) {
                try {
                    ...
                    try {
                        ...
                        throw new IllegalArgumentException(
                                String.format("Index '%d' for nested field " +
                                                "'%s' in path '%s' is out of " +
                                                "bounds.",
                                        arrayIndex, fieldName, path));
                    }
                } catch (NumberFormatException e) {
