...
/**
 * This class {@link SchemaRegistry} is responsible for managing the schema
 * registry store.
 * <p>
 * A schema is uniquely identified by an ID and contains other metadata about
 * the schema, such as a name, description,
 * type, etc. By itself, a schema does not contain an actual schema object.
 * It is more like a group of schema entries.
 * <p>
 * A schema entry contains the actual schema bytes and is uniquely identified
 * by the schema it belongs to and a
 * version number. A schema entry is added to a schema and can also be
 * removed. In addition, each schema keeps track
 * of the latest schema entry.
 * <p>
 * Schema information is stored in two tables, one for the schema metadata
 * and one for the schema entries.
 * <p>
 * The schema metadata table contains nine columns:
 * <p>
 * namespace, id, name, description, created, updated, type, auto, current
 * <p>
 * The namespace and id columns form the primary key for the metadata table.
 * The current column points to the latest
 * schema entry version, and the auto column is used for generating version
 * numbers for any new schema entries.
 * <p>
 * The schema entry table contains four columns:
 * <p>
 * namespace, id, version, schema
 * <p>
 * The namespace, id, and version columns form the primary key, while the
 * schema column contains the actual schema.
 */
public final class SchemaRegistry {
    private static final String NAMESPACE_COL = "namespace";
    private static final String GENERATION_COL = "generation";
    private static final String ID_COL = "id";
    private static final StructuredTableId META_TABLE_ID =
            new StructuredTableId("schema_registry_meta");
    public static final StructuredTableSpecification META_TABLE_SPEC =
            new StructuredTableSpecification.Builder()
                    .withId(META_TABLE_ID)
                    .withFields(new FieldType(NAMESPACE_COL,
                                    FieldType.Type.STRING),
                            new FieldType(GENERATION_COL, FieldType.Type.LONG),
                            new FieldType(ID_COL, FieldType.Type.STRING),
                            new FieldType(MetaColumn.NAME,
                                    FieldType.Type.STRING),
                            new FieldType(MetaColumn.DESC,
                                    FieldType.Type.STRING),
                            new FieldType(MetaColumn.CREATED,
                                    FieldType.Type.LONG),
                            new FieldType(MetaColumn.UPDATED,
                                    FieldType.Type.LONG),
                            new FieldType(MetaColumn.TYPE,
                                    FieldType.Type.STRING),
                            new FieldType(MetaColumn.AUTO_VERSION,
                                    FieldType.Type.LONG),
                            new FieldType(MetaColumn.CURRENT_VERSION,
                                    FieldType.Type.LONG))
                    .withPrimaryKeys(NAMESPACE_COL, GENERATION_COL, ID_COL)
                    .build();
    private static final StructuredTableId ENTRY_TABLE_ID =
            new StructuredTableId("schema_registry_entries");
    public static final StructuredTableSpecification ENTRY_TABLE_SPEC =
            new StructuredTableSpecification.Builder()
                    ...
                    .withFields(new FieldType(NAMESPACE_COL,
                            ...
                            new FieldType(GENERATION_COL, FieldType.Type.LONG),
                            new FieldType(ID_COL, FieldType.Type.STRING),
                            new FieldType(EntryColumn.VERSION,
                                    FieldType.Type.LONG),
                            new FieldType(EntryColumn.SCHEMA,
    ...
    public SchemaRegistry(StructuredTable metaTable,
                          StructuredTable entryTable) {
    ...
    public static SchemaRegistry get(StructuredTableContext context) {
        try {
            ...
            throw new IllegalStateException(String.format(
                    "System table '%s' does not exist. Please check your " +
                            "system environment.", e.getId().getName()), e);
        }
    }

    /**
     * Writes an entry in the schema registry. If the schema already exists,
     * it is overwritten.
     *
     * @param schemaDescriptor information about the schema to write
     * @throws IOException if there was an error reading from or writing to
     *                     the storage system
     */
    ...
    public void delete(NamespacedId id) throws IOException {
        ...
        try (CloseableIterator<StructuredRow> rowIter = entryTable.scan(range
                , Integer.MAX_VALUE)) {
            while (rowIter.hasNext()) {
                StructuredRow row = rowIter.next();
                Namespace namespace =
                        new Namespace(row.getString(NAMESPACE_COL),
                                row.getLong(GENERATION_COL));
                NamespacedId entryId = new NamespacedId(namespace,
     ...
     * @throws IOException             if there was an error reading from or
    ...
    public long add(NamespacedId id, byte[] specification) throws IOException {
        ...
        if (existing == null) {
            throw new SchemaNotFoundException(String.format("Schema '%s' does" +
                    " not exist.", id.getId()));
        }
     ...
     * <p>
     * TODO: (CDAP-14661) update latest version pointer if the latest version
     * was removed
     *
    ...
    /**
     * Checks if there is a schema entry, its not necessary that there are
     * any version of schema registered.
     *
     ...
     * @throws SchemaNotFoundException if the schema or its latest entry
     *                                 could not be found
     * @throws IOException             if there was an error reading from or
     *                                 writing to the storage system
     */
    public SchemaEntry getEntry(NamespacedId id) throws IOException {
        ...
        if (version == null) {
            return new SchemaEntry(id, schemaRow.getDescriptor().getName(),
                    schemaRow.getDescriptor().getDescription(),
                    schemaRow.getDescriptor().getType(),
    ...
    private SchemaEntry getEntry(SchemaRow schemaRow, long version) throws IOException {
        ...
        Optional<StructuredRow> row = entryTable.read(getEntryKey(id, version));
        if (!row.isPresent()) {
            throw new SchemaNotFoundException(String.format("Schema '%s' " +
                    "version '%d' does not exist.", id.getId(), version));
        }
    ...
    private List<Field<?>> getMetaKey(NamespacedId id) {
        ...
        fields.add(Fields.stringField(NAMESPACE_COL,
                id.getNamespace().getName()));
        fields.add(Fields.longField(GENERATION_COL,
                id.getNamespace().getGeneration()));
    ...
    private List<Field<?>> getEntryKey(NamespacedId schemaId, long version) {
        ...
        fields.add(Fields.stringField(NAMESPACE_COL,
                schemaId.getNamespace().getName()));
        fields.add(Fields.longField(GENERATION_COL,
                schemaId.getNamespace().getGeneration()));
    ...
    private List<Field<?>> toFields(SchemaRow schemaRow) {
        ...
        fields.add(Fields.stringField(NAMESPACE_COL,
                schemaRow.getDescriptor().getId().getNamespace().getName()));
        fields.add(Fields.longField(GENERATION_COL,
                schemaRow.getDescriptor().getId().getNamespace().getGeneration()));
        fields.add(Fields.stringField(ID_COL,
                schemaRow.getDescriptor().getId().getId()));
        fields.add(Fields.stringField(MetaColumn.NAME,
                schemaRow.getDescriptor().getName()));
        fields.add(Fields.stringField(MetaColumn.DESC,
                schemaRow.getDescriptor().getDescription()));
        fields.add(Fields.stringField(MetaColumn.TYPE,
                schemaRow.getDescriptor().getType().name()));
        fields.add(Fields.longField(MetaColumn.CREATED,
                schemaRow.getCreated()));
        fields.add(Fields.longField(MetaColumn.UPDATED,
                schemaRow.getUpdated()));
        fields.add(Fields.longField(MetaColumn.AUTO_VERSION,
                schemaRow.getAutoVersion()));
        if (schemaRow.getCurrentVersion() != null) {
            fields.add(Fields.longField(MetaColumn.CURRENT_VERSION,
                    schemaRow.getCurrentVersion()));
        }
        return fields;
    }
    ...
    private SchemaRow fromRow(StructuredRow row) {
        Namespace namespace = new Namespace(row.getString(NAMESPACE_COL),
                row.getLong(GENERATION_COL));
        NamespacedId id = new NamespacedId(namespace, row.getString(ID_COL));
        SchemaDescriptor descriptor = new SchemaDescriptor(id,
                row.getString(MetaColumn.NAME),
