...
public class RecipePageRequest extends PageRequest<Recipe> {
    ...
    protected RecipePageRequest(Integer pageSize, String pageToken,
                                String sortBy,
    ...
    public void validateSortBy(String sortBy) {
        if (sortBy != null && !sortByMap.containsKey(sortBy)) {
            throw new IllegalArgumentException(
                    String.format("Invalid sortBy '%s' specified. sortBy must" +
                                    " be one of: '%s' or '%s'",
    ...
    public Range getScanRange() {
        Collection<Field<?>> begin = getNamespaceKeys(NAMESPACE_FIELD,
                GENERATION_COL, namespace);
        Collection<Field<?>> end = getNamespaceKeys(NAMESPACE_FIELD,
                GENERATION_COL, namespace);

        // If pageToken has a value, add the respective field (column, value)
        // to the range to filter results
    ...
    public String getNextPageToken(Recipe recipe) {
        ...
        switch (getSortBy()) {
            ...
            default:
                throw new IllegalArgumentException(
                        String.format("Invalid sortBy field '%s' is not " +
                                        "mapped to any field in Recipe to " +
                                        "return as a pageToken.",
                                getSortBy()));
        }
    ...
    private void validatePageTokenDataType(String pageToken, String sortBy) {
        if (pageToken != null && sortBy.equals(UPDATE_TIME_COL)) {
            try {
                ...
                throw new IllegalArgumentException(
                        "pageToken value is of invalid data type: expected " +
                                "'long' value for sortBy 'updated'");
            }
        }
    }
    ...
    private Field<?> getSortByField() {
        ...
        switch (getSortBy()) {
            case RECIPE_NAME_FIELD:
                sortByField = Fields.stringField(RECIPE_NAME_FIELD,
                        getPageToken());
                break;
            case UPDATE_TIME_COL:
                sortByField = Fields.longField(UPDATE_TIME_COL,
                        Long.parseLong(Objects.requireNonNull(getPageToken())));
                break;
            default:
                throw new IllegalArgumentException(
                        String.format("Invalid sortBy field '%s' is not " +
                                        "mapped to any Recipe store column " +
                                        "name.",
                                getSortBy()));
        }
        return sortByField;
    ...
    public static class Builder {
        ...
        public RecipePageRequest build() {
            return new RecipePageRequest(pageSize, pageToken, sortBy,
                    sortOrder, namespace);
        }
    }
}
