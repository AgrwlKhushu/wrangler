...
/**
 * An interface to implement a page request. Pagination utility class that
 * contains relevant fields and methods
...
public abstract class PageRequest<T> {
    ...
    /**
     * Token to identify the page to retrieve. Either received from a
     * previous page or left empty to fetch the first page.
     */
    @Nullable
    ...
    protected PageRequest(Integer pageSize, @Nullable String pageToken,
                          String sortBy, String sortOrder) {
        this.pageToken = pageToken;

        validateSortBy(sortBy);
        this.sortBy = sortBy == null ? getDefaultSortBy() :
                getSortByColumnName(sortBy);
        ...
        this.sortOrder =
                (sortOrder == null || sortOrder.equals(SORT_ORDER_ASC)) ?
    ...
    protected void validateSortOrder(String sortOrder) {
        if (sortOrder != null && !(sortOrder.equals(SORT_ORDER_ASC) || sortOrder.equals(SORT_ORDER_DESC))) {
            throw new IllegalArgumentException(
                    String.format("Invalid sortOrder '%s' specified. " +
                                    "sortOrder must be one of: '%s' or '%s'",
