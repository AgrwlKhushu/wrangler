 ...
 * It manages the lifecycle of the {@link Connection} including all CRUD
 * operations.
 * <p>
 * The store is backed by a table with namespace, id, type, name,
 * description, properties, created, and updated columns.
...
public class ConnectionStore {
    ...
    public static final StructuredTableSpecification TABLE_SPEC =
            new StructuredTableSpecification.Builder()
                    ...
                    .withFields(new FieldType(NAMESPACE_COL,
                            ...
                            new FieldType(PRECONFIGURED_COL,
                                    FieldType.Type.STRING))
    ...
    /**
     * Creates an entry in the {@link ConnectionStore} for object
     * {@link Connection}.
     * <p>
     * This method creates the id and returns if after successfully updating
     * the store.
     *
    ...
    public NamespacedId create(Namespace namespace, ConnectionMeta meta,
        ...
        NamespacedId id = new NamespacedId(namespace,
                getConnectionId(meta.getName()));
        Connection existing = read(id);
        if (existing != null) {
            throw new ConnectionAlreadyExistsException(
                    String.format("Connection named '%s' with id '%s' already" +
                            " exists.", meta.getName(), id.getId()));
        }
    ...
    public Connection get(NamespacedId id) throws ConnectionNotFoundException
        ...
        Connection existing = read(id);
        if (existing == null) {
            throw new ConnectionNotFoundException(String.format("Connection " +
     ...
     * @throws ConnectionNotFoundException if the specified connection does
     *                                     not exist
     */
     ...
     */
    public boolean connectionExists(Namespace namespace,
                                    String connectionName) throws IOException {
        return read(new NamespacedId(namespace,
                getConnectionId(connectionName))) != null;
    }
     ...
     */
    public List<Connection> list(Namespace namespace,
                                 Predicate<Connection> filter) throws IOException {
    ...
    private List<Field<?>> toFields(Connection connection, long generation) {
        ...
        fields.add(Fields.stringField(PROPERTIES_COL,
                GSON.toJson(connection.getProperties())));
        ...
        fields.add(Fields.stringField(PRECONFIGURED_COL,
                String.valueOf(connection.isPreconfigured())));
    ...
    private Connection fromRow(StructuredRow row) {
        ...
        return Connection.builder(new NamespacedId(namespace,
                ...
                .setProperties(GSON.fromJson(row.getString(PROPERTIES_COL),
                        MAP_TYPE))
