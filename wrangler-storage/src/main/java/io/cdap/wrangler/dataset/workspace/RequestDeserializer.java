...
public class RequestDeserializer implements JsonDeserializer<Request> {
    @Override
    public Request deserialize(JsonElement json, Type type,
        ...
        if (!object.has("version")) {
            throw new JsonParseException(
                    String.format("Version field is not specified in the " +
                            "request.")
            );
        }
        ...
        if (version == 1) {
            Workspace workspace = context.deserialize(object.get("workspace")
                    , Workspace.class);
            Recipe recipe = context.deserialize(object.get("recipe"),
                    Recipe.class);
            Sampling sampling = context.deserialize(object.get("sampling"),
                    Sampling.class);
            JsonObject properties = context.deserialize(object.get(
                    "properties"), JsonObject.class);
