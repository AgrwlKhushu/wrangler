 ...
 * it has primary key as [namespace][namespace-generation][entity-type] and
 * value [upgrade-ts-millis][upgrade-state]
 * upgrade state is a serialized string which represents the overall upgrade
 * information and status
 */
public class UpgradeStore {
    private static final NamespaceSummary SYSTEM_NS = new NamespaceSummary(
            "system", "", 0L);
    ...
    /**
     * Initialize the upgrade timestamp and state for the given upgrade types
     * if not there and
     * return the upgrade timestamp. This method should be called before any
     * upgrade starts.
    ...
    public long initializeAndRetrieveUpgradeTimestampMillis(UpgradeEntityType type, long timestampMillis,
        ...
        return TransactionRunners.run(transactionRunner, context -> {
            ...
            fields.add(Fields.stringField(UPGRADE_STATE_COL,
     ...
     */
    public void setEntityUpgradeState(UpgradeEntityType type,
     ...
     */
    public void setEntityUpgradeState(NamespaceSummary namespace,
     ...
     */
    public UpgradeState getEntityUpgradeState(NamespaceSummary namespace,
                                              UpgradeEntityType type) {
        return TransactionRunners.run(transactionRunner, context -> {
    ...
    // visible for testing, storage do not have guava dependency so cannot
    // add annotation
    void clear() {
        TransactionRunners.run(transactionRunner, context -> {
            StructuredTable table = context.getTable(TABLE_ID);
    ...
    private void setComplete(NamespaceSummary namespace,
                             StructuredTableContext context,
                             UpgradeEntityType type,
                             UpgradeState upgradeState) throws IOException {
        StructuredTable table = context.getTable(TABLE_ID);
    ...
    @Nullable
    private UpgradeState getEntityUpgradeState(NamespaceSummary namespace,
                                               StructuredTableContext context,
        ...
        return GSON.fromJson(row.get().getString(UPGRADE_STATE_COL),
                UpgradeState.class);
    }

    private Collection<Field<?>> getPrimaryKeys(NamespaceSummary namespace,
                                                UpgradeEntityType entityType) {
