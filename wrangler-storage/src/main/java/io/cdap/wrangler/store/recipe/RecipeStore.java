...
public class RecipeStore {
    ...
    public static final StructuredTableSpecification RECIPE_TABLE_SPEC =
            new StructuredTableSpecification.Builder()
                    ...
                    .withPrimaryKeys(NAMESPACE_FIELD, GENERATION_COL,
                            RECIPE_ID_FIELD)
    ...
    public void createRecipe(RecipeId recipeId, RecipeRow recipeRow) {
        TransactionRunners.run(transactionRunner, context -> {
            ...
            RecipeRow oldRecipeWithName = getRecipeByName(table, recipeName,
                    recipeId.getNamespace(), false);
            if (oldRecipeWithName != null) {
                throw new RecipeAlreadyExistsException(String.format("recipe " +
                        "with name '%s' already exists", recipeName));
            }
    ...
    private void saveRecipe(RecipeId recipeId, RecipeRow recipeRow,
                            boolean failIfNotFound, StructuredTable table)
            throws IOException, RecipeAlreadyExistsException {
        RecipeRow oldRecipe = getRecipeInternal(table, recipeId,
                failIfNotFound);
        if (oldRecipe != null) {
    ...
    private void upsertRecipe(RecipeId recipeId, RecipeRow recipeRow,
                              StructuredTable table) throws IOException {
        Collection<Field<?>> fields = getRecipeKeys(recipeId);
        fields.add(Fields.stringField(RECIPE_NAME_FIELD,
                recipeRow.getRecipe().getRecipeName()));
        fields.add(Fields.longField(CREATE_TIME_COL,
                recipeRow.getRecipe().getCreatedTimeMillis()));
        fields.add(Fields.longField(UPDATE_TIME_COL,
                recipeRow.getRecipe().getUpdatedTimeMillis()));
    ...
    /**
     * Get the Recipe associated with given recipe name if it exists, else
     * throws {@link RecipeNotFoundException}
     *
     * @param namespace
     * @param recipeName
     ...
     */
    public Recipe getRecipeByName(NamespaceSummary namespace,
                                  String recipeName) throws RecipeNotFoundException {
        return TransactionRunners.run(transactionRunner, context -> {
            StructuredTable table = context.getTable(TABLE_ID);
            RecipeRow recipeRow = getRecipeByName(table, recipeName,
                    namespace, true);
            return recipeRow.getRecipe();
        }, RecipeNotFoundException.class);
    }
     ...
     *
     * @param request {@link RecipePageRequest} that contains necessary query
     *                parameters
     * @return {@link RecipeListResponse} that contains a page of results and
     * nextPageToken
     */
    public RecipeListResponse listRecipes(RecipePageRequest request) {
        return TransactionRunners.run(transactionRunner, context -> {
            ...
            try (CloseableIterator<StructuredRow> iterator = table.scan(range
    ...
    private RecipeRow getRecipeInternal(StructuredTable table,
                                        RecipeId recipeId,
        ...
        if (!row.isPresent()) {
            ...
            throw new RecipeNotFoundException(String.format("recipe with id " +
                    "'%s' does not exist", recipeId.getRecipeId()));
        }
        return GSON.fromJson(row.get().getString(RECIPE_INFO_COL),
                RecipeRow.class);
    }
    ...
    private RecipeRow getRecipeByName(StructuredTable table, String recipeName,
        ...
        Collection<Field<?>> keys = getNamespaceKeys(NAMESPACE_FIELD,
                GENERATION_COL, summary);
        ...
        if (!iterator.hasNext()) {
            ...
            throw new RecipeNotFoundException(String.format("recipe with name" +
                    " '%s' does not exist", recipeName));
        }
        return GSON.fromJson(iterator.next().getString(RECIPE_INFO_COL),
                RecipeRow.class);
    }
