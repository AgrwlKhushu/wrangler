...
public class WorkspaceStore {
    ...
    // this field is to ensure the workspace information is correctly fetched
    // if a namespace is recreated
    ...
    public WorkspaceDetail getWorkspaceDetail(WorkspaceId workspaceId) throws WorkspaceNotFoundException {
        return TransactionRunners.run(transactionRunner, context -> {
            ...
            Optional<StructuredRow> row =
                    table.read(getWorkspaceKeys(workspaceId));
            if (!row.isPresent()) {
            ...
            Workspace workspace =
                    GSON.fromJson(row.get().getString(WORKSPACE_INFO_COL),
                            Workspace.class);
            List<Row> rows = new ArrayList<>();
            byte[] sample = row.get().getBytes(SAMPLE_COL);
            if (sample != null) {
                try (ObjectInputStream ois =
                             new ObjectInputStream(new ByteArrayInputStream(sample))) {
     ...
     */
    public void saveWorkspace(WorkspaceId workspaceId,
                              WorkspaceDetail workspace) {
        saveWorkspace(workspaceId, workspace.getWorkspace(),
                workspace.getSampleAsBytes(), false);
    }
    ...
    // clean up all workspaces, only usable by tests, do not add
    // @VisibleForTesting to not
    ...
    private void saveWorkspace(WorkspaceId workspaceId, Workspace workspace,
                               @Nullable byte[] sample,
                               boolean failIfNotFound) {
        TransactionRunners.run(transactionRunner, context -> {
            StructuredTable table = context.getTable(TABLE_ID);
            Workspace oldWorkspace = getWorkspaceInternal(table, workspaceId,
                    failIfNotFound);
            ...
            fields.add(Fields.longField(CREATED_COL,
                    newWorkspace.getCreatedTimeMillis()));
            fields.add(Fields.longField(UPDATED_COL,
                    newWorkspace.getUpdatedTimeMillis()));
            fields.add(Fields.stringField(WORKSPACE_INFO_COL,
                    GSON.toJson(newWorkspace)));
    ...
    // internal get method so the save, delete and get operation can all
    // happen in single transaction
    @Nullable
    private Workspace getWorkspaceInternal(
        ...
        if (!row.isPresent()) {
            ...
            throw new WorkspaceNotFoundException(String.format("Workspace %s " +
                    "does not exist", workspaceId.getWorkspaceId()));
        }

        return GSON.fromJson(row.get().getString(WORKSPACE_INFO_COL),
                Workspace.class);
    }

    private Collection<Field<?>> getWorkspaceKeys(WorkspaceId workspace) {
        List<Field<?>> keys =
                new ArrayList<>(getNamespaceKeys(workspace.getNamespace()));
        keys.add(Fields.stringField(WORKSPACE_ID_FIELD,
