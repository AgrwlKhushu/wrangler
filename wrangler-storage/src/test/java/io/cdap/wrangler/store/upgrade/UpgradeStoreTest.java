...
public class UpgradeStoreTest extends SystemAppTestBase {
    ...
    public void testUpgradeTimestampDoesNotChange() throws Exception {
        ...
        Tasks.waitFor(true, () -> System.currentTimeMillis() > upgradeTs, 5,
                TimeUnit.MILLISECONDS);
        long actual = store.initializeAndRetrieveUpgradeTimestampMillis(
                UpgradeEntityType.CONNECTION, System.currentTimeMillis(),
                new UpgradeState(1L));
        Assert.assertEquals(upgradeTs, actual);
        Assert.assertEquals(new UpgradeState(0L),
                store.getEntityUpgradeState(UpgradeEntityType.CONNECTION));
    }
    ...
    public void testUpgradeStore() throws Exception {
        ...
        namespaces.forEach(ns -> {
            store.setEntityUpgradeState(ns, UpgradeEntityType.CONNECTION,
                    upgraded);
            Assert.assertEquals(upgraded, store.getEntityUpgradeState(ns,
        ...
        namespaces.forEach(ns -> {
            store.setEntityUpgradeState(ns, UpgradeEntityType.WORKSPACE,
                    upgraded);
            Assert.assertEquals(upgraded, store.getEntityUpgradeState(ns,
