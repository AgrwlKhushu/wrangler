...
public class RecipeStoreTest extends SystemAppTestBase {
    ...
    public void testSaveAndGetNewRecipe() {
        ...
        Recipe savedRecipeRowByName = store.getRecipeByName(summary, "dummy " +
                "name");
    ...
    public void testGetRecipeDoesNotExist() {
        ...
        RecipeId recipeId = RecipeId.builder(summary).setRecipeId("non" +
                "-existent-recipe-id").build();
    ...
    public void testListRecipesDefault() {
        ...
        RecipeRow recipeRow1 =
                RecipeRow.builder(Recipe.builder(recipeId1).setRecipeName(
                        "xyz").build()).build();
        RecipeRow recipeRow2 =
                RecipeRow.builder(Recipe.builder(recipeId2).setRecipeName(
                        "abc").build()).build();
        ...
        RecipePageRequest pageRequest =
                RecipePageRequest.builder(summary).build();
    ...
    public void testListRecipesPagination() {
        ...
        RecipeRow recipeRow1 =
                RecipeRow.builder(Recipe.builder(recipeId1).setRecipeName(
                        "recipe 1").build()).build();
        RecipeRow recipeRow2 =
                RecipeRow.builder(Recipe.builder(recipeId2).setRecipeName(
                        "recipe 2").build()).build();
        RecipeRow recipeRow3 =
                RecipeRow.builder(Recipe.builder(recipeId3).setRecipeName(
                        "recipe 3").build()).build();

        store.createRecipe(recipeId1, recipeRow1);
        store.createRecipe(recipeId2, recipeRow2);
        store.createRecipe(recipeId3, recipeRow3);

        RecipePageRequest pageRequest1 =
                RecipePageRequest.builder(summary).setPageSize(2).build();
        ...
        Assert.assertEquals(page1.getNextPageToken(),
                recipeRow3.getRecipe().getRecipeName());

        // Fetching the next page
        RecipePageRequest pageRequest2 =
                RecipePageRequest.builder(summary).setPageToken(page1.getNextPageToken()).build();
    ...
    public void testDeleteRecipe() {
        ...
        RecipeId recipeId = RecipeId.builder(summary).build();
        RecipeRow recipeRow =
                RecipeRow.builder(Recipe.builder(recipeId).setRecipeName(
                        "deleted").build()).build();
        store.createRecipe(recipeId, recipeRow);
    ...
    public void testSaveUpdateRecipe() {
        ...
        Assert.assertEquals("description-after-edit",
                newRecipe.getDescription());
        Assert.assertEquals(ImmutableList.of("dir1", "dir2", "dir3"),
                newRecipe.getDirectives());
