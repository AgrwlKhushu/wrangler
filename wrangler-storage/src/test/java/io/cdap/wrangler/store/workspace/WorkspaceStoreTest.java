...
public class WorkspaceStoreTest extends SystemAppTestBase {
    ...
    public void testNotFoundExceptions() throws Exception {
        WorkspaceId workspace = new WorkspaceId(new NamespaceSummary("default"
                , "", 10L), "workspace");
        try {
        ...
        try {
            store.updateWorkspace(workspace, Workspace.builder("dummy",
                    workspace.getWorkspaceId()).build());
            Assert.fail();
        } catch (WorkspaceNotFoundException e) {
            // expected
        }
    ...
    public void testCRUD() throws IOException {
        ...
        SampleSpec dummySpec = new SampleSpec("conn", "dummy", "/tmp",
                ImmutableSet.of());
        ...
        WorkspaceDetail detail1 = new WorkspaceDetail(meta1,
                Collections.singletonList(new Row(ImmutableList.of("k1",
                        "k2"))));
        ...
        WorkspaceDetail detail2 = new WorkspaceDetail(meta2,
                Collections.singletonList(new Row(ImmutableList.of("k3",
                        "k4"))));
        ...
        meta1 = Workspace.builder("newname1", id1.getWorkspaceId())
                ...
                .build();
        detail1 = new WorkspaceDetail(meta1,
                Collections.singletonList(new Row(ImmutableList.of("k5",
                        "k6"))));
        store.saveWorkspace(id1, detail1);

        // creation time should not change
        Workspace expected =
                Workspace.builder(meta1).setCreatedTimeMillis(100L).build();
        Assert.assertEquals(expected, store.getWorkspace(id1));
        Assert.assertEquals(new WorkspaceDetail(expected,
                detail1.getSample()), store.getWorkspaceDetail(id1));
        ...
        Assert.assertEquals(expected, store.getWorkspace(id1));
        Assert.assertEquals(detail1.getSample(),
                store.getWorkspaceDetail(id1).getSample());
        Assert.assertEquals(new WorkspaceDetail(expected,
                detail1.getSample()), store.getWorkspaceDetail(id1));

        // test lists don't include from other namespaces
        Assert.assertEquals(Collections.singletonList(meta1),
                store.listWorkspaces(ns1));
        Assert.assertEquals(Collections.singletonList(meta2),
                store.listWorkspaces(ns2));
        ...
        WorkspaceDetail detail3 = new WorkspaceDetail(meta3,
                Collections.singletonList(new Row(ImmutableList.of("k7",
                        "k8"))));
        store.saveWorkspace(id3, detail3);
        // order can be different because of different id
        Assert.assertEquals(ImmutableSet.of(expected, meta3),
                new HashSet<>(store.listWorkspaces(ns1)));
    ...
    public void testNamespaceGenerations() {
        ...
        Workspace meta1 = Workspace.builder("name1", id1.getWorkspaceId())
                ...
                .build();
        store.saveWorkspace(id1, new WorkspaceDetail(meta1,
