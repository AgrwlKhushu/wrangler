...
public class SchemaRegistryTest extends SystemAppTestBase {
    ...
    public void testNotFoundExceptions() throws Exception {
        getTransactionRunner().run(context -> {
            ...
            try {
                ...
                Assert.fail("removing an entry from a non-existent schema did" +
            ...
            try {
                ...
                Assert.fail("getting a non-existent schema did not throw an " +
                        "exception");
            } catch (SchemaNotFoundException e) {
                // expected
            }
            ...
            try {
                ...
                Assert.fail("getting versions of a non-existent schema did " +
                        "not throw an exception");
            } catch (SchemaNotFoundException e) {
                // expected
            }

            SchemaDescriptor descriptor = new SchemaDescriptor(id, "some " +
                    "name", "desc", SchemaDescriptorType.AVRO);
            registry.write(descriptor);

            try {
                registry.getEntry(id, 1L);
                Assert.fail("getting a non-existent schema entry did not " +
                        "throw an exception");
            } catch (SchemaNotFoundException e) {
                // expected
            }
    ...
    public void testCRUD() {
        ...
        SchemaDescriptor descriptor = new SchemaDescriptor(id, "some name",
                "desc", SchemaDescriptorType.AVRO);
        SchemaEntry expected = new SchemaEntry(id, descriptor.getName(),
        ...
        expected = new SchemaEntry(id, expected.getName(),
                expected.getDescription(), expected.getType(),
        ...
        Assert.assertEquals(expected, v1Actual);
        Assert.assertEquals(expectedVersions,
                call(registry -> registry.getVersions(id)));
        ...
        Assert.assertEquals(Collections.singleton(version2),
                call(registry -> registry.getVersions(id)));
    ...
    public void testNamespaceIsolation() {
        ...
        SchemaDescriptor descriptor1 = new SchemaDescriptor(id1, "name1",
                "desc1", SchemaDescriptorType.AVRO);
        SchemaEntry expected1 = new SchemaEntry(id1, descriptor1.getName(),
                descriptor1.getDescription(),
                descriptor1.getType(), Collections.emptySet(), null, null);
        SchemaDescriptor descriptor2 = new SchemaDescriptor(id2, "name2",
                "desc2", SchemaDescriptorType.AVRO);
        SchemaEntry expected2 = new SchemaEntry(id2, descriptor2.getName(),
        ...
        Assert.assertEquals(expected1,
                call(registry -> registry.getEntry(id1)));
        Assert.assertEquals(expected2,
                call(registry -> registry.getEntry(id2)));
        ...
        Assert.assertEquals(Collections.singleton(v1),
                call(registry -> registry.getVersions(id1)));
        Assert.assertEquals(Collections.singleton(v2),
                call(registry -> registry.getVersions(id2)));
    ...
    public void testNamespaceGenerations() {
        ...
        SchemaDescriptor descriptor = new SchemaDescriptor(id, "name", "desc"
                , SchemaDescriptorType.AVRO);
        run(registry -> registry.write(descriptor));
        long v1 = call(registry -> registry.add(id, new byte[]{1}));

        // test that fetching with a different generation doesn't include the
        // connection
        try {
            run(registry -> registry.getEntry(new NamespacedId(nsGen2,
                    id.getId())));
            Assert.fail("schema with a different generation should not be " +
                    "visible.");
        } catch (SchemaNotFoundException e) {
            // expected
        }
        ...
        try {
            run(registry -> registry.getVersions(new NamespacedId(nsGen2,
                    id.getId())));
            Assert.fail("schema entry with a different generation should not " +
                    "be visible.");
        } catch (SchemaNotFoundException e) {
            // expected
        }
