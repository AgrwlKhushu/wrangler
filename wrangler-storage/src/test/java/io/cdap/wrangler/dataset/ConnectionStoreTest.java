...
public class ConnectionStoreTest extends SystemAppTestBase {
    ...
    public void testNotFoundException() throws Exception {
        getTransactionRunner().run(context -> {
            ...
            try {
                ...
                Assert.fail("Getting a non-existent connection did not throw " +
                        "an exception.");
            ...
            try {
                connectionStore.update(id, ConnectionMeta.builder().setName(
                        "name").setType(ConnectionType.FILE).build());
                Assert.fail("Getting a non-existent connection did not throw " +
                        "an exception.");
            } catch (ConnectionNotFoundException e) {
                // expected
    ...
    public void testAlreadyExistsException() throws Exception {
        getTransactionRunner().run(context -> {
            ...
            ConnectionMeta meta =
                    ConnectionMeta.builder().setName("name").setType(ConnectionType.FILE).build();
            connectionStore.create(namespace, meta);
            try {
                connectionStore.create(namespace, meta);
                Assert.fail("Creating a duplicate connection did not throw an" +
                        " exception.");
    ...
    public void testCRUD() {
        ...
        Assert.assertTrue(run(connectionStore -> connectionStore.list(ns,
                x -> true)).isEmpty());
        ...
        ConnectionMeta expected = ConnectionMeta.builder()
                ...
                .putProperty("k1", "v1")
                .build();
        NamespacedId id = run(connectionStore -> connectionStore.create(ns,
                expected));
        ...
        List<Connection> filterList = run(connectionStore ->
                connectionStore.list(ns,
                        c -> c.getType() == ConnectionType.DATABASE));
        Assert.assertTrue(filterList.isEmpty());
        // text scan
        List<Connection> list =
                run(connectionStore -> connectionStore.list(ns, x -> true));
    ...
    public void testNamespaceIsolation() {
        ...
        ConnectionMeta expected1 = ConnectionMeta.builder()
                ...
                .putProperty("k1", "v1")
                .build();
        NamespacedId id1 = run(connectionStore -> connectionStore.create(ns1,
                expected1));
        ConnectionMeta expected2 = ConnectionMeta.builder()
                ...
                .putProperty("k2", "v2")
                .build();
        NamespacedId id2 = run(connectionStore -> connectionStore.create(ns2,
                expected2));
        ...
        List<Connection> list1 =
                run(connectionStore -> connectionStore.list(ns1, x -> true));
        Assert.assertEquals(1, list1.size());
        assertConnectionMetaEquality(expected1, list1.iterator().next());
        List<Connection> list2 =
                run(connectionStore -> connectionStore.list(ns2, x -> true));
        ...
        Assert.assertTrue(run(connectionStore -> connectionStore.list(ns1,
                x -> true)).isEmpty());
        try {
            run(connectionStore -> connectionStore.get(id1));
            Assert.fail("Connection store did not properly delete connection " +
                    "1");
        } catch (ConnectionNotFoundException e) {
            // expected
        }
    ...
    public void testNamespaceGenerations() {
        ...
        ConnectionMeta expected1 = ConnectionMeta.builder()
                ...
                .build();
        NamespacedId id1 =
                run(connectionStore -> connectionStore.create(nsGen1,
                        expected1));
        ...
        try {
            ...
            Assert.fail("connection with a different generation should not be" +
                    " visible.");
        } catch (ConnectionNotFoundException e) {
            // expected
        }

        // test that listing with a different generation doesn't include the
        // connection
        Assert.assertTrue(run(connectionStore -> connectionStore.list(nsGen2,
                x -> true)).isEmpty());
    }
    ...
    public void testPreconfiguredConnections() {
        ...
        NamespacedId preconfigured =
                run(connectionStore -> connectionStore.create(nsGen1,
                        preconfiguredMeta, true));
        NamespacedId manuallyCreated =
                run(connectionStore -> connectionStore.create(nsGen1,
                        manuallyCreatedMeta));
        ...
        // test that a manually created connection returns that it is not
        // built-in
        Assert.assertFalse(manuallyCreatedConnection.isPreconfigured());
    }

    private void assertConnectionMetaEquality(ConnectionMeta expected,
                                              Connection actual) {
        // can't just compare objects since the store sets created, id, and
        // updated fields
