...
public class RecipePageRequestTest {
    ...
    public void testGetRangeForFirstPage() {
        ...
        RecipePageRequest pageRequest =
                RecipePageRequest.builder(namespace).build();
        Range range = pageRequest.getScanRange();
        Collection<Field<?>> fields = getNamespaceKeys(NAMESPACE_FIELD,
                GENERATION_COL, namespace);
        Assert.assertEquals(range, Range.create(fields, INCLUSIVE, fields,
                INCLUSIVE));
    }
}
