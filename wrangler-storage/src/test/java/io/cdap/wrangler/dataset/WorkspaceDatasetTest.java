...
public class WorkspaceDatasetTest extends SystemAppTestBase {
    ...
    public void testNotFoundExceptions() throws Exception {
        getTransactionRunner().run(context -> {
            ...
            try {
                ws.updateWorkspaceData(new NamespacedId(namespace, "id"),
                        DataType.TEXT, new byte[]{0});
            ...
            try {
                ws.updateWorkspaceProperties(new NamespacedId(namespace,
            ...
            try {
                ws.updateWorkspaceRequest(new NamespacedId(namespace, "id"),
                        null);
                Assert.fail("Updating a non-existing workspace should fail.");
            } catch (WorkspaceNotFoundException e) {
                // expected
    ...
    public void testScopes() {
        ...
        run(ws -> ws.writeWorkspaceMeta(new NamespacedId(namespace,
                id1.getId()), meta1));
        run(ws -> ws.writeWorkspaceMeta(new NamespacedId(namespace,
                id2.getId()), meta2));

        Assert.assertEquals(Collections.singletonList(id1),
                call(ws -> ws.listWorkspaces(namespace, scope1)));
        Assert.assertEquals(Collections.singletonList(id2),
                call(ws -> ws.listWorkspaces(namespace, scope2)));
    ...
    public void testNamespaceIsolation() {
        ...
        Assert.assertEquals(Collections.singletonList(new WorkspaceIdentifier(id1.getId(), meta1.getName())),
                call(ws -> ws.listWorkspaces(id1.getNamespace(),
                        WorkspaceDataset.DEFAULT_SCOPE)));
        Assert.assertEquals(Collections.singletonList(new WorkspaceIdentifier(id2.getId(), meta2.getName())),
                call(ws -> ws.listWorkspaces(id2.getNamespace(),
                        WorkspaceDataset.DEFAULT_SCOPE)));
        ...
        Assert.assertEquals(Collections.singletonList(new WorkspaceIdentifier(id1.getId(), meta1.getName())),
                call(ws -> ws.listWorkspaces(id1.getNamespace(),
                        WorkspaceDataset.DEFAULT_SCOPE)));
        Assert.assertTrue(call(ws -> ws.listWorkspaces(id2.getNamespace(),
                WorkspaceDataset.DEFAULT_SCOPE).isEmpty()));
    ...
    public void testNamespaceGenerations() {
        ...
        try {
            ...
            Assert.fail("workspace with a different generation should not be " +
                    "visible.");
        } catch (WorkspaceNotFoundException e) {
            // expected
        }
        ...
        // connection
        Assert.assertTrue(call(ws -> ws.listWorkspaces(nsGen2,
                WorkspaceDataset.DEFAULT_SCOPE)).isEmpty());
    }
    ...
    public void testCRUD() {
        ...
        WorkspaceIdentifier workspaceId = new WorkspaceIdentifier(id.getId(),
                meta1.getName());
        ...
        Assert.assertEquals(Collections.singletonList(workspaceId),
                call(ws -> ws.listWorkspaces(ns, "default")));
        ...
        io.cdap.wrangler.proto.Workspace requestWorkspace =
                new io.cdap.wrangler.proto.Workspace(meta1.getName(), 10);
        Recipe recipe = new Recipe(Collections.singletonList("parse-as-csv " +
                "body"), true, "recipeName");
        Sampling sampling = new Sampling("random", 0, 10);
        Request request = new RequestV1(requestWorkspace, recipe, sampling,
                new JsonObject());
